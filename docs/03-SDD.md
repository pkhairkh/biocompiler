...
The Type System performs static verification of biological correctness properties on mRNA sequences. It evaluates each declared type predicate against the mRNA and produces a three-valued verdict: PASS (guaranteed correct), FAIL (guaranteed incorrect), or UNCERTAIN (cannot determine). Note: the full Python system uses five-valued logic (adding LIKELY_PASS and LIKELY_FAIL); the three-valued system is the Lean4 formal subset. The Type System is the safety-critical component of BioCompiler; its soundness (no false PASS) is the most important invariant of the entire system.
...
ALGORITHM TypeCheck:
  ...
  2. For each TypePredicate P in predicates:
     ...
     Else:  # UNCERTAIN
       gap = P.get_knowledge_gap()      # What is needed to resolve
       results.append(TypeCheckResult(P, UNCERTAIN, None, None, gap))

  3. Compose results using three-valued logic (five-valued in the full Python system; three-valued in the Lean4 formal model):
     overall = PASS
