...
A probabilistic compiler is, therefore, a category error. It collapses the distinction between engineering and statistics — between knowing something will hold and believing it probably will. No one would trust a compiler that produces executables which "probably will not segfault." Yet this is exactly what a probabilistic gene design framework offers: sequences that "probably" splice correctly, "probably" fold well, "probably" express.
...
**"You do not need a deterministic system to get deterministic answers. You need to ask the right questions — questions whose answers are deterministic even when the system is not."**
...
One might object: computing intervals instead of point estimates merely renames the concept. An interval [0.85, 1.0] for exon inclusion is really just a confidence interval by another name.
...
| Solution quality | A score (continuous) | Binary: satisfies or does not |
...
- **Transitions**: at each potential splice site, either splice or do not splice (subject to biological constraints — splice sites must be recognized, exons must have minimum length, etc.)
...
The logical connectives of three-valued logic are defined to preserve soundness (the full Python system extends to five-valued logic with LIKELY_PASS and LIKELY_FAIL; the three-valued connectives are the Lean4 formal subset):
...
The primary limitation of three-valued logic is its coarseness. It cannot distinguish between "almost certainly yes" and "barely maybe." (The full Python system addresses this partially with five-valued logic adding LIKELY_PASS and LIKELY_FAIL.) For quantitative questions — "what fraction of cells will express this gene?" — probabilities are necessary. But for the yes/no questions that arise in design verification — "will this gene ever produce a truncated protein?" — three-valued logic is both sufficient and more honest than probability.
...
Gene therapy constructs are safety-critical medical devices. They must be verified to be safe before being administered to patients. A probabilistic guarantee ("97% chance of correct splicing") is not sufficient for regulatory approval — the FDA requires evidence that the construct will not produce harmful proteins, not merely that it probably will not. The deterministic framework provides exactly this kind of evidence: a proof that the construct satisfies safety properties in all possible cellular contexts within a specified class.
