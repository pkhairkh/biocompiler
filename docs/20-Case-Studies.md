...
1. **EnforceTranslation**: ✓ The sequence encodes the target protein
...
**Types compose, constraints don't.** The checklist approach checks each
...
|---|---|---|---|
| A | ...GAT | GAT (Asp, D) | ✓ PASS |
| B | CTT... | CTT (Leu, L) | ✓ PASS |
| A+B | ...GAT**C**TT... | Junction: GATC | ✗ **FAIL!** |
...
The fundamental issue: checklist items are checked independently. There's
...
**Composition requires re-verification.** You can't just AND two
...
The optimizer changed AAA→AAG thinking it was "just a synonymous
...
checks. You don't have to trust the optimizer — you can VERIFY its
output.
...
Output files are saved to `download/case_studies/`:
...
The case studies are implemented as a self-contained Python script at:
- `src/biocompiler/case_studies.py` (package copy)
- `case_studies.py` (project root copy, recommended for running)

The script is self-contained and includes all necessary BioCompiler
definitions inline, so it can run without installing the full package.
This ensures maximum portability and reproducibility.
