...
### 1.1 Objective — Build BioCompiler v12.0.0

The objective of the BioCompiler project is to design, implement, verify, and deliver **BioCompiler v12.0.0**, a compiler-framework software system that applies compiler-engineering design patterns — staged transformation, typed intermediate representations, composable passes, and type-based verification — to the formalizable stages of gene-to-protein processing, specifically splicing and translation. The system SHALL provide deterministic, machine-checkable guarantees for designed mRNA sequences without relying on probabilistic models.

BioCompiler v12.0.0 SHALL deliver the following capabilities at release:
...
| 1.1.3 | Type System Prototype | `type_system/` with three-valued verdict logic, 33 type predicates (13 core + 20 SLOT-dependent), subtyping, conjunction algebra | 4 PW | 1.1.2 | REQ-FUNC-030–035 |
...
| **Total** | **64 PW** | **21** | **M4: v12.0.0 release** |
...
| **M4** | Month 21 | Phase 4 complete — all acceptance criteria met | Quality Gate QG-4 passed (Section 7); all Phase 4 work packages (1.4.1–1.4.4) completed; all system-level acceptance criteria AC-01 through AC-07 met (DOC-05 Section 4.1); SARS-CoV-2 pilot study reviewed by domain expert; v12.0.0 release tagged | Mutation explorer, ORF analyzer, SARS-CoV-2 pilot study report, full system V&V report, v12.0.0 release package with all documentation |
...
| `v12.0.0` | End of Phase 4 | Full BioCompiler release. All acceptance criteria met. | M4 passed; all acceptance criteria AC-01 through AC-07 met; all three pilot studies reviewed by domain experts; full system V&V complete |

**Version Numbering**: The project follows Semantic Versioning 2.0.0. Pre-release versions use `-alpha` (internal use, no stability guarantees) and `-beta` (feature-complete, external review, no stability guarantees for APIs). The `v12.0.0` release commits to backward compatibility for the IR schema, CLI interface, and Python API.
...
|---|---|---|
| **BL-1** (Requirements Baseline) | Project start | DOC-01 (SRS), DOC-05 (SVVP), DOC-07 (this document) — all at version 12.0.0, baselined after initial review |
...
| **BL-4** (Product Baseline) | M4 (Month 21) | All documentation (DOC-00 through DOC-10), all source code and tests, `v12.0.0` release package, all pilot study reports, full V&V report |
