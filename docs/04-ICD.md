...
> **WARNING:** **NOTE: The IR Bus and Protocol Buffer schemas described in this document were NOT implemented. The actual codebase uses Python dataclasses in `shared/types.py`. The FFI Manager was replaced by the `engines/` package. The Verdict enum has 5 values (PASS/LIKELY_PASS/UNCERTAIN/LIKELY_FAIL/FAIL), not 3. `TypeCheckResult.predicate` is `str`, not `TypePredicate`. `TypeCheckResult.violation` is `str | None`, not `Violation`. The main API function is `evaluate_all_predicates()`, not `type_check()`. The exception `UnknownPredicateError` is actually `UnknownPredicateError`. Interfaces IF-08, IF-09, IF-10 are [NOT IMPLEMENTED].**
...
**Interface ID:** IF-05
**Description:** Performs static verification of biological correctness properties on mRNA sequences. Evaluates each declared type predicate against the mRNA and produces a five-valued verdict: PASS (guaranteed correct with derivation trace), LIKELY_PASS (strongly indicated correct), UNCERTAIN (cannot determine, with knowledge gap specification), LIKELY_FAIL (strongly indicated incorrect), or FAIL (guaranteed incorrect with violation identification). This is the safety-critical component; its soundness (no false PASS) is the most important invariant of the entire system.
...
| **Output** | `TypeCheckResult`: For each predicate, a verdict (PASS/LIKELY_PASS/UNCERTAIN/LIKELY_FAIL/FAIL) with derivation trace, violation identification, or knowledge gap specification. |
| **Error Conditions** | `UnknownPredicateError`: A type predicate is malformed or references an unknown property. [MissingPrerequisiteError removed — not in actual implementation] (e.g., SpliceCorrect requires isoform IDs). |
...
| **Performance** | Time: O(n × p) where n = sequence length, p = number of predicates. ≤ 2 seconds for n ≤ 10,000 with 17 core predicates; ≤ 10 seconds with all 43 predicates. Space: O(n) for token scanning within predicates. ≤ 2 MB. |
...
def evaluate_all_predicates(
    ...
    Each predicate is evaluated independently, producing a five-valued verdict:
    ...
    five-valued AND logic: overall PASS only if all predicates PASS.
    ...
    Raises:
        UnknownPredicateError: If a predicate is malformed or references an
            unknown property.
...
class CompositionCheck:
    ...
    verdict: Verdict                   # Three-valued verdict (PASS/LIKELY_PASS/UNCERTAIN/LIKELY_FAIL/FAIL)
...
def verify_composition(
    ...
    Each check produces a five-valued verdict (PASS/LIKELY_PASS/UNCERTAIN/LIKELY_FAIL/FAIL) with evidence.
    The overall circuit verdict is composed from individual check verdicts using
    five-valued AND logic.
...
### 3.9 IF-09: Mutation Explorer Interface [NOT IMPLEMENTED]
...
### 3.10 IF-10: ORF Analyzer Interface [NOT IMPLEMENTED]
...
*End of DOC-04: Interface Control Document (ICD) — Version 0.9.0 — Status: Current*
