...
| Technical Reference | [15-Reference.md](15-Reference.md) | 43-predicate tables, integrated optimizer API, TCB, honest limitations |
...
| CSP Solver | [16-CSP-Solver.md](16-CSP-Solver.md) | Constraint satisfaction solver (OR-Tools CP-SAT; Z3 backend removed in 0.9.1) for gene optimization |
...
| Doc | File | Purpose |
|---|---|---|
| IR Compiler Architecture | [21-IR-Compiler.md](21-IR-Compiler.md) | Five-level IR compiler (L0→L4), lowering passes, codegen, formal verification, test-suite summary |
...
- **Type system decomposition**: Monolith `type_system.py` → `type_system/` package (codon_tables, checks, predicates, registry)
- **Optimizer decomposition**: Monolith `optimization.py` → `optimizer/` subpackage (integrated_optimizer, pipeline, mutagenesis, assembly, cai, objectives)
- **Strict mode + safety-by-default**: Protein verification, provenance tracking, and sliding-window GC enabled by default
- **~14,200 test functions**: Comprehensive test suite including biosecurity, CAI validation, tAI, and cross-implementation tests
- **Integrated optimizer**: Unified `integrated_optimize` pipeline (greedy multi-step → CAI hill climbing), invoked via `optimize_sequence`
- **Performance**: CAI 0.999 for GFP (E. coli), 14× faster than DNAchisel
