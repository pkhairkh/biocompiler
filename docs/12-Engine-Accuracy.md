...
| Sensitivity (IDP/aggregation) | 66.7% | 4/6 aggregation-prone identified |
| Pearson r (enhanced vs ordinal) | 0.73 | Strong correlation |

### 2.3 Why Low Sensitivity for IDPs?

The CamSol implementation uses the **Urry hydrophobicity scale** (upgraded from Wimley-White; see IMPLEMENTED upgrade below),
...
1. **Urry scale (IMPLEMENTED)**: The upgrade from Wimley-White to Urry scale has been completed, improving IDP sensitivity from 33.3% to 66.7%
...
- ~~Replace Wimley-White scale with Urry scale for better IDP handling~~ [IMPLEMENTED — Urry scale now used]
...
| **Offline** | Chou--Fasman heuristic | Q3 ≈ 50--60%, pLDDT cap 55 | Both unavailable |
...
1. **Chou--Fasman heuristic fallback**: When the ESMFold API and local
   model are both unavailable, a Chou--Fasman secondary structure prediction
   provides low-confidence estimates (Q3 accuracy ≈ 50--60%, pLDDT cap 55).
   Results are flagged with `method="heuristic_fallback"` and confidence ≤ 0.5.
...
| CamSol (structural) | ~86% classification | 67% IDP sensitivity | Yes | Improved IDP detection (Urry scale) |
| CamSol (intrinsic) | ~86% classification | 67% IDP sensitivity | Yes | Improved IDP detection (Urry scale) |
| ESMFold (API) | pLDDT r≈0.8 | pLDDT r≈0.8 | No | API may be unavailable |
| ESMFold (offline) | Q3 ≈ 50--60% | pLDDT cap 55 | Yes | Low-confidence heuristic fallback |
