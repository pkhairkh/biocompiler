...
BioCompiler therefore uses a **three-tier solver hierarchy**:
...
├─────────────────────────────────┤
│  Tier 2: Z3 SMT                 │  ← Fallback: continuous constraints
│  (MaxEntScan-heavy problems)     │
├─────────────────────────────────┤
│  Tier 3: Greedy optimizer       │  ← Last resort: timeout / unavailable
│  (with warning, no guarantees)  │
└─────────────────────────────────┘
```

The primary backend (OR-Tools CP-SAT) handles 99% of real-world problems in
under 10 seconds. Z3 is invoked only when the problem involves a high density
of MaxEntScan threshold constraints that benefit from real-arithmetic reasoning.
The greedy optimizer is the emergency fallback when both formal solvers time
out or are unavailable at runtime; it produces a best-effort result with an
explicit `SOLVER_FALLBACK` warning.
...
**Z3** is an SMT solver supporting quantifier-free nonlinear real arithmetic
(QF_NRA) and quantifier-free bit-vector logic (QF_BV). It serves as the
fallback backend for problems where continuous constraints dominate.

**When Z3 is preferred:**
...
When both CP-SAT and Z3 fail (timeout, library unavailable, or internal error),
the system falls back to the greedy multi-step optimizer from ADR-0008.
...
| Condition | Action |
|---|---|
| CP-SAT times out (default 60s) | Try Z3 with same timeout |
| Z3 times out | Fall back to greedy |
| CP-SAT not installed (`ImportError`) | Try Z3 |
| Neither solver available | Greedy with `SOLVER_UNAVAILABLE` warning |
| CP-SAT returns internal error | Try Z3, then greedy |
...
def solve_with_csp(
    ...
    Args:
        ...
        objective: Scalar objective to maximize (default: CAI).
...
The CSP solver is the default as of v2.0. Existing code using the greedy
...
**Before (greedy optimizer):**

```python
from biocompiler.optimizer.greedy import greedy_optimize

result = greedy_optimize(
    ir_seq=ir_seq,
    restriction_enzymes=["EcoRI", "XhoI"],
    target_gc=0.55,
    maxentscan_threshold=3.0,
)
# result.optimized_sequence, result.warnings
```

**After (CSP solver):**

```python
from biocompiler.optimizer.csp_solver import solve_with_csp, SolverConfig
from biocompiler.optimizer.variable_domain import Constraint, Objective

constraints = [
    Constraint(name="NoRestrictionSite", constraint_type="restriction",
               parameters={"enzymes": ["EcoRI", "XhoI"]}),
    Constraint(name="GCInRange", constraint_type="gc_content",
               parameters={"lo": 0.40, "hi": 0.60}),
    Constraint(name="NoCrypticSpliceDonor", constraint_type="splicing",
               parameters={"threshold": 3.0}),
    Constraint(name="NoCrypticSpliceAcceptor", constraint_type="splicing",
               parameters={"threshold": 3.0}),
]
...
result = solve_with_csp(
    ...
    objective=Objective(name="CAI", direction="maximize"),
    config=SolverConfig(),
)
...
| Protein | Length (AA) | CP-SAT Time | Z3 Time | Greedy Time | CAI (CSP) | CAI (Greedy) |
|---|---|---|---|---|---|---|
...
| Constraint | CP-SAT Encoding | Z3 Encoding | Greedy Handling |
|---|---|---|---|
| CorrectTranslation | Domain construction | Domain construction | Domain construction |
| NoRestrictionSite | Forbidden assignment table | Implication chain | Sequential removal |
| NoInstabilityMotif | Aho-Corasick automaton | Implication chain | Sequential removal |
| GCInRange | Linear inequality | Real arithmetic | Target-and-adjust |
| NoCrypticSpliceDonor | Aho-Corasick automaton (forbidden 9-mers) | Real arithmetic (MaxEntScan) | Context disruption |
| NoCrypticSpliceAcceptor | Aho-Corasick automaton (forbidden 23-mers) | Real arithmetic (MaxEntScan) | Context disruption |
| NoPrematureStop | Domain construction | Domain construction | Domain construction |

## Appendix C: Revision History

| Version | Date | Author | Changes |
|---|---|---|---|
| 1.0.0-draft | 2026-03-05 | Subagent C8 | Initial draft |

---

*This document is part of the BioCompiler documentation suite. Cross-references
use the DOC-XX naming convention established in the project documentation
index. For the theoretical motivation behind constraint satisfaction over
optimization, see DOC-10 §5. For the original ADR on CSP-based gene design, see
ADR-0004 (superseded by ADR-0008, which is in turn superseded in part by this
document). For the type-directed mutagenesis engine that handles amino acid
substitution when the CSP is infeasible at the codon level, see ADR-0009.*
