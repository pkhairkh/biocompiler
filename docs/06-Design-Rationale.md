...
## 2. Background: Why the Original Idea Does Not Work
...
This is a moderate flaw because it does not invalidate the pipeline architecture — the IR levels are still useful and well-motivated — but it does undermine the claim that grammar induction provides a principled method for discovering the architecture. The BioCompiler fix is simply to acknowledge the source of the IR levels: they come from biological knowledge, not from algorithmic discovery. This is consistent with the broader theme of replacing automatic discovery with curated knowledge (Flaw 3, Flaw 4). The pipeline architecture (AD-01) is still justified by the engineering benefits of modularity, testability, and extensibility — it just is not justified by the claim of emergent structure.
...
Non-deterministic finite-state automata (NFAs) and transducers (NDFSTs) model processes that have multiple possible outcomes without assigning probabilities to those outcomes. An NFA accepts a set of strings; an NDFST produces a *set-valued* function mapping each input string to a set of possible output strings. The computation is deterministic (the same input always produces the same set of outputs), but the modeled process is non-deterministic (the "actual" output is some element of the set, but we do not know which one).

This is exactly the right model for alternative splicing. Given a pre-mRNA sequence, the NDFST produces the set of all possible splice isoforms consistent with the splicing grammar and the specified cellular context. We do not know which isoform will be produced in any given cell at any given time — that depends on stochastic splicing factor concentrations — but we *do* know that the produced isoform will be an element of the computed set. This is a guarantee: "the actual isoform is guaranteed to be in this set." It is not a probabilistic statement ("the actual isoform has probability 0.7 of being isoform A"), because we do not have a calibrated probability model for splicing factor concentrations.
...
### 9.4 Interaction Between These Decisions

The three decisions form a coherent improvement to the optimizer-mutagenesis pipeline:
...
*End of DOC-06: Design Rationale — Version 0.9.0*
