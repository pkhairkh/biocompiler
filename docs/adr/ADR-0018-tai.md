...
3. **Wobble pairing rules**: Configurable wobble efficiency weights based on dos Reis et al. (2004). Default weights:
   ...
   - G:U wobble: 0.5
   - Inosine (I) wobble: I:U=0.35, I:C=0.65, I:A=0.15
   - Other modified wobble: organism-specific

4. **Per-codon tRNA efficiency weights**: `compute_codon_weights(organism)` — returns the tRNA-based weight for each codon, analogous to codon adaptiveness values but derived from tRNA data.
...
6. **Result field**: The tAI value is included in optimization results when tAI is computed, but there is no dedicated `OptimizationResult.tai` field.
