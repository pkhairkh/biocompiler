...
The implementation adds a `_gt_free_codons_for_aa(aa)` helper that returns the set
of synonymous codons for an amino acid that do not contain the GT dinucleotide, and
an `is_gt_mandatory(aa)` function that returns True only for Valine (the sole amino
