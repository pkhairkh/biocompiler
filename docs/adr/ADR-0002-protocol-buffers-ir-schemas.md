...
> **Note**: This ADR was accepted but NOT implemented. Python dataclasses in `shared/types.py` are used instead of Protocol Buffers.
...
Protocol Buffers were never implemented. Python dataclasses in `shared/types.py` are used instead for IR schema definitions, providing simpler integration with the type system and no build-step dependency.
