...
- Negative: (1) Cannot rank isoforms by likelihood: the set is unranked, which is less informative than a probability distribution for exploratory analysis. (2) Conservative: the isoform set may include rare isoforms that are biologically unlikely but grammatically valid, leading to more UNCERTAIN verdicts. (3) Regular language limitation: finite-state transducers cannot express arbitrary long-range dependencies, requiring approximations for exon definition interactions across large introns. (4) Set size can grow exponentially for genes with many alternative exons (mitigated by symbolic set representation using BDDs).
