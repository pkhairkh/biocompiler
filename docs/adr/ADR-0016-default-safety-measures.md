...
- **Positive**: Translation verification as an always-on check provides a critical correctness guarantee. A sequence that does not translate to the input protein is a bug, not a valid result — this check catches such bugs at the output boundary.
...
- **Migration**: This change targets **v1.0.0**. The major version bump is justified because:
  - `strict_mode` default change breaks any code that calls `optimize_sequence()` and does not handle `OptimizationError`
