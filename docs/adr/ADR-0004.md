...
This ADR specified z3 CSP as the primary optimization strategy. In practice, z3 doesn't scale beyond ~100 amino acids and handles multi-codon constraints poorly. ADR-0008 replaces z3 with a greedy multi-phase optimizer as default, keeping z3 as an optional backend for short proteins. The type-directed mutagenesis engine (ADR-0009) extends the approach to handle mathematical impossibility at the codon level.
