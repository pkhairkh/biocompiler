...
2. **Soft scoring certificates** — Assign a numerical score (0-100) based on how many predicates pass. Quantitative. But: scores are arbitrary; 80/100 doesn't tell the user WHICH predicates failed; cannot be independently verified in a meaningful way.

3. **Graduated certificates with full documentation** — Generate certificates for any design, documenting all predicate results (PASS, FAIL, or UNCERTAIN) and any mutagenesis applied. The certificate is a complete record of what was verified and what wasn't. Downstream users can make informed decisions about partial compliance.
