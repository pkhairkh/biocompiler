...
1. **GT-mandatory** (Valine): No codon swap can eliminate GT from any Valine codon
   ...
   do not contain GT (e.g., Isoleucine: ATT, ATC, ATA).

2. **Optimizer weakness**: The amino acid has GT-free codons but the optimizer
   did not use them. For example, Alanine has GT-free codons (GCA, GCC, GCT) but
...
> **Note:** Functions `is_gt_mandatory()`, `diagnose_optimizer_weakness()`, and `type_directed_mutagenesis()` described in this ADR are NOT yet implemented. `GT_MANDATORY_AAS = {"V"}` and `find_unrepairable_cryptic_donors()` are implemented.
