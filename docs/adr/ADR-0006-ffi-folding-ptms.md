...
4. **Foreign Function Interface (FFI) with SLOT-filling** — Invoke external tools (ESMFold, FoldX, CamSol, ViennaRNA, etc.) through a defined adapter interface. The core pipeline treats FFI outputs as non-deterministic SLOT fields in the IR. The FFI boundary isolates non-determinism from the deterministic core.
