...
2. **Risk classification**:
   - **Critical/High-risk**: Exact or near-exact matches (>95% identity over >80% length) to known hazardous sequences. Optimization is **blocked** — the function raises a `BiosecurityError` with details about the matched signature and its risk category.
   - **Medium-risk**: Partial matches or signatures from regulated but lower-concern organisms. Optimization proceeds but emits a `BiosecurityWarning` that is included in the result metadata and all export formats. The user must acknowledge the warning by setting `acknowledge_biosecurity_risk=True` to proceed.
   - **Low-risk/No match**: No action required. Normal optimization proceeds.
