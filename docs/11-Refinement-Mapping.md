...
|---|---|---|
| `BioCompiler.Verdict` (3 values: PASS, FAIL, UNCERTAIN) | `biocompiler.shared.types.Verdict` (5 values: PASS, LIKELY_PASS, UNCERTAIN, LIKELY_FAIL, FAIL) | Python has 2 extra values (LIKELY_PASS, LIKELY_FAIL). The 3-valued Lean4 model is a conservative abstraction: every Python PASS/LIKELY_PASS maps to Lean4 PASS; every Python FAIL/LIKELY_FAIL maps to Lean4 FAIL. See Gap §3.1. |
...
| `BioCompiler.TypePredicate` (36 constructors) | `PREDICATE_NAMES` list + `check_*` functions | Lean4 uses an inductive type with 36 constructors (17 core + 19 SLOT). Python uses string-based dispatch and a registry pattern. |
...
| 26 | `hasInstabilityMotif_complete` (contrapositive) | N/A (Python does not have a negative completeness check) | Python has no explicit "no motif exists" verification | Could add property-based test |
...
The central module: defines all 36 type predicates and proves `type_soundness`. 1 main theorem + 1 per-predicate proof, 0 sorry.
...
| `InFrame rf boundaries` | `readingFrameConsistent ∧ ¬hasPrematureStop → PASS` | `check_valid_coding_seq()` partially covers this | Python does not have a separate InFrame check; frame consistency is implicit |
...
| 33 | `dinucleotide_no_compose` — constraints do not compose (counterexample) | `_remove_site_multicodon()` handles junction-crossing sites | Python correctly handles this via multi-codon coordinated solving | Integration test: verify junction-crossing sites are detected |
...
| 43 | `extension_cannot_remove_gt` | Python's optimization does not rely on this property explicitly | The theorem states a monotonicity property: adding nucleotides can only add GT sites, never remove them | Could be used to optimize the Python optimizer (early termination) |
...
| 50 | `synonymous_safe_for_SLOT_predicates` | SLOT predicates are trivially preserved (both sides UNCERTAIN in Lean4; in Python, SLOT checks are independent of sequence) | Gap: Python SLOT predicates DO depend on sequence, but Lean4 says they are always UNCERTAIN. In practice, the Python SLOT predicates that depend on external tools (ESMFold, etc.) ARE non-deterministic, so the Lean4 model is conservative. | N/A (conservative abstraction) |
| 51 | `synonymous_unsafe_for_dna_predicates` | `_greedy_optimize()` shows that GT elimination can introduce new GTs at other positions | Python handles this via iterative re-scanning after each substitution | Integration test: verify optimizer does not create new violations |
...
| # | Lean4 Theorem | Python Counterpart | Refinement Gap | Gap Closure Strategy |
|---|---|---|---|---|
| 55 | `evaluate_slot_independent` — evaluation does not take SLOT values as arguments | Python's `check_*` functions do not take SLOT values directly | Direct correspondence for core predicates | N/A |
| 56 | `property_slot_independent` | Same | Same | Same |
| 57 | `certificate_slot_independent` | `verify_certificate()` does not use SLOT values | Direct correspondence | N/A |
...
| 59 | `full_slot_independence` — combined guarantee | Python's certificate system does not have this guarantee explicitly | Need to add SLOT-aware certificate mode | Implement `formal_mode=True` in certificate generation that treats SLOT predicates as UNCERTAIN |
| 60 | `predicate_is_core_or_slot` | `PREDICATE_NAMES` list categorizes predicates | Python has 43 predicates (36 formalized in Lean4 + 7 extended diagnostic); Lean4 has 36. The core+SLOT predicate lists are aligned. |

---

### 2.11 Lean4 Library Modules (now superseded by proof/ modules)

> **Note**: The former `lean/BioCompiler/` directory (CodonTable.lean, Predicates.lean, Optimization.lean, Certificate.lean) was a simplified earlier model that has been superseded by the `proof/BioCompiler/` modules. The theorem mappings below remain valid as they describe concepts now covered more rigorously in the proof/ modules.
...
| 63 | `exists_cg_free_codon` — every non-Stop AA has a CG-free codon | Python does not have an explicit CG-free codon finder, but `AvoidCpG` in the optimizer does this | Correspondence through the optimizer's CpG avoidance step | Integration test |
...
| 76 | `gold_certificate_soundness` | Python certificate generation does not have a formal soundness proof | **Gap**: Python relies on testing, not proof | Cross-validation with independent re-implementation |
...
**Impact**: Python code must validate that strings contain only A/C/G/T. This is done at runtime by `scanner.validate_dna_sequence()`, but there is no compile-time guarantee.
...
2. **Property-based testing**: Generate random sequences, verify scanner completeness (if a real site exists, the scanner finds it) and soundness (if the scanner reports a site, it is real)
...
**Description**: The Lean4 model does not explicitly model reverse complement strands for restriction site checking. Python checks both strands.
...
### 3.7 Number of Predicates: 33 (Lean4) vs. 43 (Python)

**Description**: Lean4 defines 36 `TypePredicate` constructors (17 core + 19 SLOT). Python's `PREDICATE_NAMES` list has 43 entries (36 formalized in Lean4 + 7 extended diagnostic predicates in the DNA/sequence domain).

**Impact**: The Lean4 and Python predicate counts differ by 7 extended diagnostic predicates. These 7 predicates provide practical runtime checks but do not affect the soundness guarantee, which is fully covered by the 36-predicate formal model. The core+SLOT predicate lists (36) are aligned between Lean4 and Python.

**Closure Strategy**: Verify that all 36 Lean4 predicates have corresponding Python checks, even if grouped differently. Add any missing checks.
...
| Type system soundness (36 predicates) | High — each predicate needs independent verification | Critical |
