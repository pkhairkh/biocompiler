...
|---|---|---|---|---|
| TC-U-001 | REQ-FUNC-001 | COMP-01 (Scanner) | Accept valid FASTA-formatted DNA input with header line and nucleotide sequence | Scanner produces valid IR-Seq with correct sequence length and no errors |
| TC-U-002 | REQ-FUNC-001 | COMP-01 (Scanner) | Accept raw nucleotide string (no FASTA header) as input | Scanner produces valid IR-Seq; T→U normalization applied in RNA mode |
...
| TC-U-034 | REQ-FUNC-034 | COMP-05 (Type System) | Conjunction of PASS and UNCERTAIN: type-check with one predicate PASS and another UNCERTAIN | Overall verdict = UNCERTAIN (per three-valued AND truth table) |
| TC-U-035 | REQ-FUNC-034 | COMP-05 (Type System) | Conjunction of FAIL with any other verdict: type-check with one predicate FAIL and another PASS or UNCERTAIN | Overall verdict = FAIL (per three-valued AND truth table: FAIL dominates) |
...
| TC-U-090 | REQ-FUNC-090 | COMP-10 (ORF) | Multi-frame construction: sequence with two overlapping reading frames (frame 0 and frame +1) — construct translation FST per frame | Two IR-Peptide records produced, one per frame; each has correct amino acid sequence for its frame offset |
...
| Criterion | Test Cases | Pass Condition |
|---|---|---|
| Type System produces correct three-valued verdicts | TC-U-030, TC-U-031, TC-U-034, TC-U-035 | All pass |
...
| ORF Analyzer constructs multi-frame translations | TC-U-090 | Two correct IR-Peptide records |
...
| **Dependency Versions** | Z3 solver ≥ 4.12; protobuf ≥ 4.24; pytest ≥ 8.0; hypothesis ≥ 6.90; ruff ≥ 0.4; mypy ≥ 1.10 (all pinned in pyproject.toml) |
