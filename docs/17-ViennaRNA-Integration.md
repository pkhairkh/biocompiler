...
2. **Used per-pair energy weights** — each GC pair contributed −1.5 kcal/mol,
   each AU pair −0.5 kcal/mol, and each GU wobble pair −0.3 kcal/mol.  These
...
### 3.1 Module: `biocompiler.viennarna_wrapper`
...
#### `compute_mfe(sequence, temperature=37.0)`
...
| `backend` | `str` | `"viennarna"` or `"nussinov"` |
| `confidence` | `str` | `"high"`, `"medium"`, or `"low"` |

#### `compute_partition_function(sequence, temperature=37.0)`
...
**Parameters:** Same as `compute_mfe`.

**Returns:** `PartitionFunctionResult`

| Field | Type | Description |
|-------|------|-------------|
| `sequence` | `str` | Input RNA sequence |
| `free_energy` | `float` | Ensemble free energy in kcal/mol |
| `bp_probabilities` | `dict[tuple[int,int], float]` | Base pair probabilities (i, j) → p |
| `ensemble_diversity` | `float` | Average distance between structures in ensemble |
| `backend` | `str` | `"viennarna"` or `"nussinov"` |
| `confidence` | `str` | `"high"`, `"medium"`, or `"low"` |

> **Note:** The Nussinov fallback computes an approximate partition function
> using a simplified energy model.  Base pair probabilities from the fallback
> are much less reliable — treat them as qualitative indicators only.

#### `fold_dna(sequence, temperature=37.0)`

Convenience function that converts DNA to RNA and computes MFE.
...
Converts all T → U before folding.  Useful for the `check_mrna_secondary_structure`
predicate, which receives DNA input.
...
Returns `"viennarna"` if the bindings are available, `"nussinov"` otherwise.

### 3.2 Module: `biocompiler.nussinov`
...
Energy rules:
- GC pair: −2.0 kcal/mol
- AU pair: −1.0 kcal/mol
- GU pair: −0.5 kcal/mol
...
class MFEResult:
    ...
    mfe: float             # kcal/mol
    backend: str           # "viennarna" | "nussinov"
    confidence: str        # "high" | "medium" | "low"

@dataclass
class PartitionFunctionResult:
    sequence: str
    free_energy: float     # ensemble free energy (kcal/mol)
    bp_probabilities: dict # (i,j) -> probability
...
```python
from biocompiler.viennarna_wrapper import (
    VIENNARNA_MFE_ACCURACY,       # 1.5 kcal/mol (MAE vs experiment)
    NUSSINOV_MFE_ACCURACY,        # 4.0 kcal/mol (MAE vs experiment)
    TOY_MODEL_MFE_ACCURACY,       # 15.0 kcal/mol (MAE vs experiment)
...
|----------|--------------------|-----------------------|
| `compute_mfe()` backend | `"viennarna"` | `"nussinov"` |
| `compute_partition_function()` | Full McCaskill pf | Simplified Boltzmann-weighted |
| `MFEResult.confidence` | `"high"` | `"medium"` |
...
```
WARNING:biocompiler.viennarna_wrapper:ViennaRNA Python bindings not found.
...
```bash
export BIOCOMPILER_NO_VIENNARNA=1
```

Or pass the parameter directly:

```python
from biocompiler.viennarna_wrapper import set_backend
set_backend("nussinov")  # Force Nussinov fallback

# Or per-call:
result = compute_mfe(sequence, _backend="nussinov")
```
...
# Explicit: disable ViennaRNA
result = optimize(sequence, ..., use_viennarna=False)

# Explicit: enable ViennaRNA (raises if not installed)
...
```python
from biocompiler.viennarna_wrapper import compute_mfe

# Fold at 30°C (e.g., for psychrophilic organisms)
result = compute_mfe(sequence, temperature=30.0)

# Fold at 42°C (e.g., for thermophilic organisms)
result = compute_mfe(sequence, temperature=42.0)
```
...
```python
from biocompiler.viennarna_wrapper import compute_mfe, fold_dna

# RNA folding
result = compute_mfe("GGGAAACCC")
print(f"Structure: {result.structure}")   # (((...)))
print(f"MFE: {result.mfe:.2f} kcal/mol")  # -2.40 (ViennaRNA) or -2.00 (Nussinov)
print(f"Backend: {result.backend}")        # "viennarna" or "nussinov"
print(f"Confidence: {result.confidence}")  # "high" or "medium"

# DNA folding (convenience: T → U conversion)
result = fold_dna("GGGAAACCC")
...
```python
from biocompiler.viennarna_wrapper import compute_partition_function

result = compute_partition_function("GGGAAACCC")
print(f"Ensemble free energy: {result.free_energy:.2f} kcal/mol")
print(f"Ensemble diversity: {result.ensemble_diversity:.2f}")

# Base pair probabilities
for (i, j), prob in sorted(result.bp_probabilities.items()):
    if prob > 0.5:
        print(f"  Pair ({i},{j}): {prob:.3f}")
```
...
```python
from biocompiler.slot_verification import verify_mrna_secondary_structure
from biocompiler.types import SLOTMode
...
result = optimize(
    ...
    use_viennarna=True,          # Enable ViennaRNA for structure checks
    viennarna_temperature=37.0,  # Physiological temperature
)
print(f"Optimized sequence: {result.sequence}")
print(f"ViennaRNA checks: {result.mrna_structure_checks}")
```
...
## 10. Known Limitations — What ViennaRNA Can't Do
