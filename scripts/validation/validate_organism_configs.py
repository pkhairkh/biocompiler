...
from biocompiler.organisms.escherichia import (
...
def validate_organism_configs() -> dict:
    ...
    for org_name in SUPPORTED_ORGANISMS:
        ...
        if adaptiveness and preferred:
            ...
            if not_one:
                issues.append(
                    f"{org_name}: {len(not_one)} AAs where most frequent codon does not have w=1.0: "
                    f"{not_one[:5]}"
                )
    ...
    if not leu_ok:
        ...
        print(f"    [PASS] Leu CTG frequency check passed ({leu_codons_legacy['CTG']}/k ≈ 51/k)")
    ...
    if not gly_ok:
        ...
        print(f"    [PASS] Gly GGC frequency check passed ({gly_codons_legacy['GGC']}/k ≈ 28/k)")
    ...
    if not ala_ok:
        ...
        print(f"    [PASS] Ala GCC frequency check passed ({ala_codons_legacy['GCC']}/k ≈ 27/k)")
        ...
        if gc_target is not None:
            ...
            if gc_min is not None and gc_max is not None:
                if not (gc_min < gc_target < gc_max):
                    issues.append(f"{name}: GC range [{gc_min}, {gc_max}] does not bracket target {gc_target}")
