...
def naive_backtranslate_cai(protein: str, organism: str) -> float:
    ...
    try:
        from biocompiler.optimization import _back_translate_protein
...
def run_optimization(
    ...
    if decision_trail is not None:
        ...
        try:
            from biocompiler.scanner import max_donor_score, max_acceptor_score
    ...
    if translation_match:
        print(f"  ✓ Translation back to original protein: PASS")
    else:
        print(f"  ✗ Translation back to original protein: FAIL")
    ...
    if native_cai is not None:
        ...
        if cai_improved:
            print(f"  ✓ CAI improved vs native: {native_cai:.4f} → {optimized_cai:.4f} (Δ = {optimized_cai - native_cai:+.4f})")
        else:
            print(f"  ⚠ CAI slightly lower vs native: {native_cai:.4f} → {optimized_cai:.4f} (Δ = {optimized_cai - native_cai:+.4f})")
    ...
    if expected_native_cai is not None and native_cai is None:
        ...
        if cai_improved:
            print(f"  ✓ CAI improved vs published native: {expected_native_cai:.4f} → {optimized_cai:.4f} (Δ = {optimized_cai - expected_native_cai:+.4f})")
        else:
            print(f"  ✗ CAI did NOT improve vs published native: {expected_native_cai:.4f} → {optimized_cai:.4f}")

    # ── Verification 3: GC content within bounds ──
    gc_in_bounds = (0.30 <= optimized_gc <= 0.70)
    result_info["gc_in_bounds"] = gc_in_bounds
    if gc_in_bounds:
        print(f"  ✓ GC content in [0.30, 0.70]: {optimized_gc:.4f}")
    else:
        print(f"  ✗ GC content OUT OF BOUNDS: {optimized_gc:.4f}")
    ...
    if rs_found_configured:
        print(f"  ✗ Configured restriction sites found: {len(rs_found_configured)}")
        for rs in rs_found_configured:
            print(f"    - {rs['enzyme']} ({rs['site']}) at positions {rs['forward_positions'][:3]}")
    else:
        print(f"  ✓ No configured restriction sites found (EcoRI, BamHI, XhoI, HindIII, NotI)")
    if rs_found_all:
...
def main():
    ...
    for r in results:
        ...
        if native_cai is not None:
            ...
            if cai_ok:
                report_lines.append(f"  CAI vs native:       PASS ✓ (Δ = {opt_cai - native_cai:+.4f})" if isinstance(opt_cai, float) else f"  CAI vs native:       PASS ✓")
            else:
                report_lines.append(f"  CAI vs native:       TRADEOFF ⚠ (Δ = {opt_cai - native_cai:+.4f} — constraints reduce CAI)" if isinstance(opt_cai, float) else f"  CAI vs native:       TRADEOFF ⚠")
            ...
            report_lines.append(f"  CAI vs published:    {'PASS ✓' if cai_ok else 'FAIL ✗'}")
            if not cai_ok:
                overall_pass = False
        ...
        report_lines.append(f"  GC in bounds:        {'PASS ✓' if gc_ok else 'FAIL ✗'}")
        if not gc_ok:
            overall_pass = False

        # Translation
        trans_ok = r.get("translation_match", False)
        report_lines.append(f"  Translation:         {'PASS ✓' if trans_ok else 'FAIL ✗'}")
        if not trans_ok:
            overall_pass = False
        ...
        report_lines.append(f"  No configured RS:    {'PASS ✓' if rs_ok else 'FAIL ✗'}")
        if rs_configured:
            for rs in rs_configured:
    ...
    report_lines.append(f"  Translation back:    {'ALL PASS ✓' if translation_pass else 'SOME FAIL ✗'}")
    report_lines.append(f"  GC in bounds:        {'ALL PASS ✓' if gc_pass else 'SOME FAIL ✗'}")
    report_lines.append(f"  No configured RS:    {'ALL PASS ✓' if rs_pass else 'SOME FAIL ✗'}")
