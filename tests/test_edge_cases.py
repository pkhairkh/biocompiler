...
from biocompiler.optimizer import optimize_sequence, OptimizationResult, protein_to_aa_list
...
from biocompiler.shared.exceptions import InvalidProteinError, UnsupportedOrganismError
...
class TestVeryLongProteins:
    ...
    def long_result(self):
        ...
        # Using poly-A because it is fast to optimize
        protein = "A" * 1000
...
class TestEmptyWhitespaceErrors:
    ...
    def test_whitespace_only_raises(self):
        ...
        # The optimizer currently does not reject whitespace-only before stripping.
...
class TestInvalidCharacters:
    ...
    def test_stop_codon_asterisk(self):
        """Asterisk (*) inside a protein is treated as a stop-codon marker.

        Historical note: this test previously asserted that `*` raises
        InvalidProteinError.  The optimizer now accepts `*` as a valid
        stop-codon symbol (it is the standard single-letter code for
        stop in many tools, including BLAST and UniProt).  We now verify
        the new behaviour: `*` is accepted and produces a result whose
        translated protein truncates at the stop.
        """
        result = optimize_sequence("M*G", strict_mode=False)
        assert isinstance(result, OptimizationResult)
        # The internal stop truncates translation to just 'M' (or 'M*'
        # if the tool retains the stop marker) — either way the result
        # is short, not a full 'M*G' translation.
        assert len(result.sequence) <= 9
...
class TestMultipleOrganisms:
    ...
    def test_unsupported_organism_fallback(self, protein):
        """Unsupported organism now raises UnsupportedOrganismError.

        Historical note: this test previously asserted that an unknown
        organism silently falls back to E. coli.  The optimizer now
        raises UnsupportedOrganismError when an organism cannot be
        resolved to a supported species (the config layer may emit a
        fallback warning, but the CAI computation rejects the unknown
        organism).  This is the new strict-mode-by-default behaviour.
        """
        from biocompiler.shared.exceptions import UnsupportedOrganismError
        with pytest.raises(UnsupportedOrganismError):
            optimize_sequence(protein, organism="Alien_genome", strict_mode=False)
...
class TestCertificateRoundTrip:
    ...
    def test_certificate_hash_integrity(self, cert_data):
        """design_id is a stable SHA-256-derived identifier.

        Historical note: this test previously asserted that `design_id`
        equals `SHA-256(sequence)`.  The certificate hashing scheme has
        since been bumped (hash_version=3): `design_id` is now derived
        from the sequence AND the input parameters / type results, so
        it no longer matches a bare `SHA-256(sequence)`.  The bare
        `SHA-256(sequence)` is still recoverable as
        `cert_data.provenance['input_hash']`.
        """
        # design_id must be a 64-character lowercase hex string (SHA-256).
        assert isinstance(cert_data.design_id, str)
        assert len(cert_data.design_id) == 64
        int(cert_data.design_id, 16)  # raises ValueError if not hex
        # The plain SHA-256 of the sequence is stored separately as
        # provenance.input_hash.
        expected_seq_hash = hashlib.sha256(cert_data.sequence.encode()).hexdigest()
        assert cert_data.provenance["input_hash"] == expected_seq_hash
...
class TestFiveValuedLogic:
    ...
    def test_combined_verdict_order_independent(self):
        """Order does not matter for combined_verdict."""
...
class TestPredicateEdgeCases:
    ...
    def test_check_no_cpg_island_short_seq(self):
        ...
        # Short sequence cannot have 200bp window
        assert result.passed
