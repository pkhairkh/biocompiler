...
class TestBuildCspModelReferenceSet:
    ...
    def test_both_reference_sets_cover_all_codons(self):
        ...
        Note: the Sharp-Li reference set only covers the core expression
        hosts (E. coli, human, mouse, yeast, CHO-K1) — the organisms
        Sharp & Li (1987) and their direct extensions tabulated.  Kazusa
        tables cover a much broader set, so we only verify codon
        completeness for organisms that actually appear in the Sharp-Li
        tables.
        """
        ...
        for organism in CODON_ADAPTIVENESS_TABLES:
            ...
            # Sharp-Li is only populated for the core expression hosts;
            # skip organisms for which it is intentionally absent.
            if not sharp_li:
                continue
...
class TestSharpLiCAIValidation:
    ...
    def test_validate_cai_sharp_li_recA(self):
        ...
        # We do not assert strict pass, just that the function runs
    ...
    def test_sharp_li_vs_kazusa_for_published_values(self):
        ...
        # some of the highly expressed E. coli genes.  We do not require
...
class TestSharpLiAdaptivenessTables:
    ...
    # Sharp-Li (1987) only tabulated adaptiveness values for a handful of
    # primary expression hosts.  The current Sharp-Li reference set in
    # biocompiler covers exactly these canonical organisms:
    SHARP_LI_CORE_ORGANISMS = {
        "Escherichia_coli",
        "Homo_sapiens",
        "Mus_musculus",
    ...
    def test_all_supported_organisms_present(self):
        """SHARP_LI_ADAPTIVENESS_TABLES should cover the core expression hosts.

        Sharp-Li tables are only populated for the primary expression
        hosts (E. coli, human, mouse, yeast, CHO-K1).  Other organisms
        in CODON_ADAPTIVENESS_TABLES (Kazusa) intentionally have no
        Sharp-Li counterpart — the original paper did not tabulate them.
        """
        sharp_li_tables = get_sharp_li_adaptiveness_tables()
        for organism in self.SHARP_LI_CORE_ORGANISMS:
            assert organism in sharp_li_tables, (
                f"Sharp-Li table missing for core organism {organism}"
            )
