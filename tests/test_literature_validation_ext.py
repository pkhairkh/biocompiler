...
from biocompiler.validation.literature_validation import (
    # Data classes
    LiteratureCase,
    ValidationResult,
    DomainReport,
...
class TestCitationFormat:
    ...
    def test_article_title_is_quoted(self):
        ...
        for case in ALL_LITERATURE_CASES:
            ...
            # Not all references may include the title — that is acceptable
            # But if a title-like string is present, it should be quoted
            # We just check that if there is a quoted section, it is properly formed
            if has_quoted_title:
    ...
    def test_case_ids_are_sequential_within_domain(self):
        ...
        for domain, letter in domain_letters.items():
            ...
            for c in domain_cases:
                ...
                assert match, f"Case {c.case_id} does not match domain {domain}"
