...
class TestSLOTPredicateSoundness:
    ...
    def test_slot_pass_unsound_when_tool_unavailable(self):
        ...
        sound because the fallback trusted the evidence. Now, if we can't
        confirm the PASS claim, it's marked unsound.
        """
        report = verify_soundness(
...
class TestRecheckCorePredicate:
    ...
    def test_slot_predicate_returns_none(self):
        ...
        result = _recheck_core_predicate("LowImmunogenicity", Verdict.PASS, protein_sequence="MAGIC")
...
class TestNonVacuityDemonstration:
    ...
    is NOT confirmed — it's marked unsound. This forces honest
    reporting.
    """
    ...
    def test_old_vacuous_behavior_would_be_sound(self):
        ...
        # The new behavior: StableFolding requires FoldX, which isn't
        ...
        if report.results[0].recheck_passed is False:
            ...
            # If the tool happens to be installed, it's sound
            assert report.all_sound
