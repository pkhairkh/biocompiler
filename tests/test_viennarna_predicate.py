...
# Several test classes below use ``is_viennarna_available()`` to decide
# whether to skip at runtime; the marker keeps them deselected by default
# alongside other requires_external tests.
...
try:
    from biocompiler.engines.viennarna_fallback import nussinov_fold
except ImportError:
...
class TestFullMRNAScan:
    ...
    def test_current_model_only_first_50(self):
        ...
        # Build a sequence that is PASS for first 50 but FAIL later
