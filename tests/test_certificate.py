...
class TestCertificateRoundTrip:
    ...
    def test_current_hash_version_in_dict(self):
        """Newly generated certificates should include hash_version in dict.

        Since C14 (prior fix) bumped ``_CURRENT_HASH_VERSION`` to 3, freshly
        generated certificates carry ``hash_version=3`` (the expanded param
        set closing the verification-envelope-widening gap). Legacy v1 certs
        omit the field; legacy v2 certs preserve it on round-trip.
        """
        results = [_make_pass_result()]
        cert = generate_certificate("ATGCGT", results, _basic_input_params())
        cert_dict = cert.to_dict()
        assert "hash_version" in cert_dict
        assert cert_dict["hash_version"] == _CURRENT_HASH_VERSION
        assert cert_dict["hash_version"] == 3
