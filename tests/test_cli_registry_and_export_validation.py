...
class TestExportFastaDNAValidation:
    ...
    def test_length_not_multiple_of_3_rejected(self):
        """Sequence length not divisible by 3 should be rejected."""
        with pytest.raises(ValidationError) as exc_info:
            ExportFastaInput(sequence="ATGGC")
        error_str = str(exc_info.value).lower()
        assert "multiple of 3" in error_str
...
class TestExportGenbankDNAValidation:
    ...
    def test_length_not_multiple_of_3_rejected(self):
        """Sequence length not divisible by 3 should be rejected."""
        with pytest.raises(ValidationError) as exc_info:
            ExportGenbankInput(sequence="ATGGC")
        error_str = str(exc_info.value).lower()
        assert "multiple of 3" in error_str
...
class TestBatchExportDNAValidation:
    ...
    def test_length_not_multiple_of_3_rejected(self):
        """Sequence length not divisible by 3 should be rejected."""
        with pytest.raises(ValidationError) as exc_info:
            BatchExportItem(sequence="ATGGC")
        error_str = str(exc_info.value).lower()
        assert "multiple of 3" in error_str
