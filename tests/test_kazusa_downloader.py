...
class TestKazusaHTMLParsing:
    ...
    def test_missing_codons_zero_filled(self):
        ...
        result = _parse_kazusa_html(SAMPLE_KAZUSA_HTML)
        # The sample HTML does not include TTT, TTC, etc.
...
class TestResolveOrDownloadOrganism:
    ...
    def test_resolve_unknown_raises_without_network(self):
        """Should raise ValueError for unknown organisms that cannot be downloaded."""
...
class TestCaching:
    ...
    def test_fetch_from_kazusa_caches_result(self):
        ...
        clear_cache()
        # We cannot actually hit the network, but we can verify the cache
...
class TestGracefulErrorHandling:
    ...
    def test_fetch_from_kazusa_returns_empty_on_network_failure(self):
        ...
        # Since we cannot control the network, we test that the function
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            try:
                ...
                # If it somehow succeeds, that is fine too
                assert isinstance(result, dict)
            except Exception:
                # If it raises instead of returning empty dict, that is a bug
