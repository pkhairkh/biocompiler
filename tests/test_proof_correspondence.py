...
Gaps addressed:
  ...
  §3.7  Number of predicates alignment (32 vs. 28)
...
class TestSLOTIndependence:
    ...
    def test_predicate_classification_exclusive(self):
        ...
        from biocompiler.slot_verification import SLOT_PREDICATES
        core_predicates = {
...
class TestReverseComplement:
    ...
    The Lean4 model doesn't explicitly model reverse complement strands.
...
class TestPredicateAlignment:
    """Verify Python predicate set aligns with the Lean4 32-predicate model.

    Lean4 has 32 TypePredicate constructors (13 core + 19 SLOT).
    Python should have corresponding check functions for all of them.
    """

    def test_predicate_count(self):
        """PREDICATE_NAMES should contain 28+ entries (may differ from 32 due to grouping)."""
        assert len(PREDICATE_NAMES) >= 28, f"Expected >=28 predicates, got {len(PREDICATE_NAMES)}"

    def test_slot_predicate_count(self):
        """SLOT_PREDICATES should contain all 19-20 SLOT predicates from the Lean4 model."""
        # The Lean4 model has 19 SLOT predicates (20 with PopulationCoverageSafe)
...
# ═══════════════════════════════════════════════════════════════════════════
# Cross-cutting: Constraints Don't Compose (Lean4 counterexamples)
# ═══════════════════════════════════════════════════════════════════════════

class TestConstraintsDontCompose:
    ...
    def test_junction_detection_in_python(self):
        ...
        # This specific sequence doesn't have GT, let's construct one that does
