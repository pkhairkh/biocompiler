...
from biocompiler.optimizer.parts import Part, PartLibrary, DEFAULT_PARTS
...
class TestPartLibraryFileLoading:
    ...
    def test_load_json_overlays_defaults(self):
        """JSON file parts overlay but do not replace all defaults."""
        parts_data = [
            {
                "name": "my_custom",
                "part_type": "linker",
    ...
    def test_non_list_data_raises(self):
        """Top-level data that is not a list raises ValueError."""
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".json", delete=False
        ) as f:
