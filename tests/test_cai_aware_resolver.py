...
class TestResolutionChoice:
    ...
    def test_cai_savings_negative(self):
        """cai_savings is negative when alternative was better (shouldn't happen normally)."""
        choice = ResolutionChoice(
            relax_constraint="a",
            keep_constraint="b",
            cai_loss=0.10,
...
class TestEstimateCAIImpact:
    ...
    def test_splice_constraint_no_gt_codons(self):
        ...
        # Alanine codons don't contain GT or AG → zero impact
        assert impact == pytest.approx(0.0, abs=0.001)
...
class TestLightweightVsFullMode:
    """Test the lightweight CAIAwareConstraintResolver from conflict_resolution."""

    def test_lightweight_mode_no_model(self):
        """CAIAwareConstraintResolver works in lightweight mode without model."""
    ...
    def test_rank_resolution_compromise_adjusted(self):
        ...
        # Actually: GC impact = -0.05, REST impact = 0.01
