...
class TestValidCodingSeqImpliesNoStopCodons:
    ...
    last codon CAN be a stop in NoStopCodons (it's allowed), so:
    ...
    Actually, a more precise statement: if ValidCodingSeq returns PASS,
    ...
    def test_valid_coding_implies_no_internal_stops(self, seq):
        ...
        if valid_result.verdict == Verdict.PASS:
            ...
            # Therefore the implication doesn't hold in general.
...
class TestDualThresholdMonotonicitySplice:
    ...
    More precisely:
      ...
      Wait, that's backwards. Lower thresholds are STRICTER:
...
class TestPredicateResultConsistency:
    ...
    def test_three_valued_consistency(self, seq):
        """For three-valued predicates: UNCERTAIN → passed=True (warn but don't block)."""
        result = check_no_cryptic_splice(seq)
        if result.verdict == Verdict.UNCERTAIN:
            assert result.passed is True, (
                f"UNCERTAIN should have passed=True (warn but don't block), "
...
class TestEdgeCases:
    ...
    def test_all_stops_coding_seq(self):
        """All stop codons is a valid coding sequence (they're in CODON_TABLE)."""
