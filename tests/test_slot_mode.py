...
import pytest
from biocompiler.shared.types import Verdict, SLOTMode, TypeCheckResult, Certificate
...
class TestSLOTPredicateClassification:
    ...
    def test_known_slot_predicates(self):
        ...
        CodonOptimality were removed from SLOT_PREDICATES (they are
        now CORE, aligned with the Lean4 formal model). The dedicated
        verify_* functions still exist for direct callers, but
        verify_slot_predicate no longer dispatches to them.
        """
        # NoCrypticSplice / NoCrypticPromoter / CodonOptimality are NOT
        # SLOT predicates any more (C19 fix).
        assert "NoCrypticSplice" not in SLOT_PREDICATES
        assert "NoCrypticPromoter" not in SLOT_PREDICATES
        assert "CodonOptimality" not in SLOT_PREDICATES
        # Remaining SLOT predicates (heuristic / external-tool dependent)
    ...
    def test_is_slot_predicate_function(self):
        ...
        C19 (FIX-W3): NoCrypticSplice and NoCrypticPromoter are now
        CORE (not SLOT), so is_slot_predicate returns False for them.
        """
        assert is_slot_predicate("NoCrypticSplice") is False
        assert is_slot_predicate("NoCrypticPromoter") is False
        assert is_slot_predicate("NoUnexpectedTMDomain") is True
    ...
    def test_is_slot_predicate_handles_parameterized_names(self):
        ...
        assert is_slot_predicate("NoUnexpectedTMDomain(True, 0.68)") is True
    ...
    def test_slot_predicate_count(self):
        ...
        # But the exact count depends on what is classified as SLOT
...
class TestConservativeMode:
    ...
    def test_conservative_core_predicates_unaffected(self, clean_seq):
        ...
        C19 (FIX-W3): NoCrypticSplice / NoCrypticPromoter /
        CodonOptimality are now CORE. Some core predicates are
        heuristic and may return LIKELY_PASS / LIKELY_FAIL; the key
        invariant is that they are NOT forced to UNCERTAIN by the
        SLOT CONSERVATIVE override (which sets a SLOT knowledge_gap).
        """
        results = evaluate_all_predicates(clean_seq, slot_mode=SLOTMode.CONSERVATIVE)
        ...
        for r in core_results:
            slot_override = (
                ...
                and r.knowledge_gap
                and "SLOT" in r.knowledge_gap
            )
            assert not slot_override, (
                f"Core predicate {r.predicate} should not be forced to UNCERTAIN "
                f"by SLOT CONSERVATIVE override, got {r.verdict} "
                f"(knowledge_gap={r.knowledge_gap!r})"
            )
...
class TestVerifiedMode:
    ...
    def test_verified_mode_returns_pass_when_conditions_met(self, clean_seq):
        ...
        # in VERIFIED mode (not all will be PASS, but they should not all be UNCERTAIN)
        slot_results = [r for r in results if is_slot_predicate(r.predicate)]
    ...
    def test_verified_core_predicates_unaffected(self, clean_seq):
        ...
        C19 (FIX-W3): heuristic core predicates (NoCrypticPromoter etc.)
        may return LIKELY_PASS; they should not be forced to UNCERTAIN
        by the SLOT VERIFIED dispatch.
        """
        results = evaluate_all_predicates(clean_seq, slot_mode=SLOTMode.VERIFIED)
        core_results = [r for r in results if not is_slot_predicate(r.predicate)]
        for r in core_results:
            ...
            assert not slot_override, (
                f"Core predicate {r.predicate} should not be forced to UNCERTAIN "
                f"by SLOT VERIFIED override, got {r.verdict} "
                f"(knowledge_gap={r.knowledge_gap!r})"
            )
...
class TestPermissiveMode:
    ...
    def test_permissive_uncertain_promoted_to_pass(self, clean_seq):
        ...
        # (unless there is a FAIL-level violation)
    ...
    def test_permissive_core_unaffected(self, clean_seq):
        ...
        C19 (FIX-W3): heuristic core predicates may return LIKELY_PASS;
        they should not be forced to UNCERTAIN by SLOT PERMISSIVE dispatch.
        """
        ...
        for r in core_results:
            ...
            assert not slot_override, (
                f"Core predicate {r.predicate} should not be forced to UNCERTAIN "
                f"by SLOT PERMISSIVE override, got {r.verdict}"
            )
...
class TestBackwardCompatibility:
    ...
    def test_predicate_result_backward_compatible(self):
        ...
        # Old code that does not pass verification_evidence should still work
    ...
    def test_evaluate_all_predicates_without_slot_mode(self, clean_seq):
        ...
        # This tests backward compatibility: old code that does not pass slot_mode
        results = evaluate_all_predicates(clean_seq)
...
class TestSlotVerificationDispatch:
    ...
    def test_dispatch_known_slot_predicate(self, clean_seq):
        ...
        C19 (FIX-W3): NoCrypticSplice is no longer a SLOT predicate
        (it's CORE now), so we dispatch to a real SLOT predicate
        (NoUnexpectedTMDomain) instead. verify_slot_predicate raises
        ValueError for non-SLOT predicates like NoCrypticSplice.
        """
        # NoCrypticSplice is CORE now -> dispatch raises ValueError.
        with pytest.raises(ValueError, match="not a SLOT predicate"):
            verify_slot_predicate(
                "NoCrypticSplice",
                slot_mode=SLOTMode.CONSERVATIVE,
        ...
        # Real SLOT predicate dispatches normally.
