...
class TestEnforceWithCAIAwareness:
    ...
    def test_fix_prefers_least_cai_damaging(self):
        ...
        # We will use a restriction site that matches a substring in the sequence
        ...
        # Let us use a site like "GCTGC" which would be formed by GCT at pos 0 + GC from pos 1
        # Simpler: just avoid "GCT" itself as a "restriction site" (4-letter site)

        # Let us use a real test: force EcoRI (GAATTC) into the sequence
        # That is hard to construct. Let us instead test with NoTRunConstraint
        ...
        # Let us create: "TTTTTATTTTT" — but that is not a valid protein
        ...
        # Simplest: use a site that is literally a codon. Say "GCT" as a "site".
        ...
        # GCC(w=0.8) at pos 0 would give "GCCGCC" which does not contain "GCTG"
        # GCA(w=0.3) at pos 0 would give "GCAGCC" which does not contain "GCTG"
    ...
    def test_no_valid_alternative_leaves_remaining_violation(self):
        ...
        # restriction site that includes "ATG", there is no alternative.
        protein = "M"
        hard_constraints = [
            TranslationConstraint(protein),
...
class TestProvenanceCAIImpact:
    ...
    def test_provenance_repr_includes_cai_delta_when_set(self):
        """repr should include cai_delta when it is set."""
        record = ConflictProvenance(
            conflicting_constraints=["A", "B"],
            resolution_method="cai_aware",
            winner="A",
    ...
    def test_provenance_repr_omits_cai_delta_when_none(self):
        """repr should omit cai_delta when it is None."""
        record = ConflictProvenance(
            conflicting_constraints=["A", "B"],
            resolution_method="priority_based",
            winner="A",
