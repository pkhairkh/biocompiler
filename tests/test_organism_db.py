...
from biocompiler.organism_db import (
    OrganismDatabase,
    SCHEMA_VERSION,
    KAZUSA_ORGANISM_IDS,
    KAZUSA_API_URL,
...
def tmp_db_path(tmp_path: Path) -> Path:
    """Provide a temporary database path that won't collide with the real DB."""
    ...
    def test_preferred_codon_has_highest_adaptiveness(
        ...
        for aa, codon in preferred.items():
            ...
            assert adapt.get(codon, 0.0) == pytest.approx(max_adapt, abs=1e-9), (
                f"Preferred codon {codon} for AA {aa} doesn't have the highest "
...
class TestUnsupportedOrganismError:
    ...
    def test_fetch_from_kazusa_raises_valueerror_on_empty_parse(
        ...
        """When Kazusa returns HTML that can't be parsed into codon usage,
...
class TestConvenienceFunctions:
    ...
    def test_get_database_returns_instance(self, tmp_db_path: Path) -> None:
        ...
        import biocompiler.organism_db as mod
...
class TestValidateCodonUsageData:
    ...
    def test_corrects_wrong_amino_acid(self) -> None:
        """Should correct an amino acid that doesn't match CODON_TABLE."""
