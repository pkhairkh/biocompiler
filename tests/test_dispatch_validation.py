...
class TestGCViolation:
    ...
    def test_high_gc_sequence_violation(self, default_config, protein_mvskge):
        ...
        Let us use a very tight config to force a violation.
        """
...
class TestRestrictionSiteViolation:
    ...
    def test_ecori_site_violation(self, protein_mvskge):
        ...
        # E=GAA F=TTTC is wrong. Let us think more carefully.
...
class TestSolveWithCsp:
    ...
    def test_returns_solver_result(self, mock_feasibility, protein_mvskge):
        ...
        # Patch the OR-Tools engine so solve_with_csp falls back to the
        # in-tree greedy engine.  (The historical `_z3_engine` attribute
        # no longer exists in dispatch.py — z3 is loaded lazily only when
        # available, so there is no module-level engine object to patch.)
...
class TestGetCspAvailability:
    ...
    def test_has_expected_keys(self):
        result = get_csp_availability()
        # The dispatch layer now exposes the in-tree greedy engine as a
        # first-class backend alongside the optional ortools/z3 backends.
        expected_keys = {"ortools", "z3", "greedy", "any"}
        assert set(result.keys()) == expected_keys

    def test_values_are_bool(self):
        result = get_csp_availability()
        for key in ("ortools", "z3", "greedy", "any"):
    ...
    def test_any_is_logical_or(self):
        ...
        The greedy backend is always available (in-tree pure Python),
        so 'any' is effectively always True in the current dispatch layer.
        """
        result = get_csp_availability()
        assert result["any"] == (
            result["ortools"] or result["z3"] or result["greedy"]
        )
...
class TestCspOptimizeFallback:
    ...
    def test_fallback_when_no_backends(self, protein_mvskge):
        ...
        Historical note: this test previously also patched
        `_z3_engine`, but that attribute no longer exists in
        dispatch.py.  Patching `_ortools_engine` alone is sufficient
        to force the greedy fallback path (the only other backend
        exposed by the current dispatch layer is the always-available
        in-tree greedy engine).
        """
        result = csp_optimize(protein_mvskge)
        assert isinstance(result, SolverResult)
