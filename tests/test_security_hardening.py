...
class TestCORSDefaultDenyAll:
    ...
    def test_cors_default_is_empty(self):
        ...
        with patch.dict(os.environ, env, clear=True):
            # We cannot easily reload without side effects, but we can test
...
class TestRateLimitHeaders:
    ...
    def test_rate_limit_remaining_decreases(self, client):
        ...
        # (we cannot easily clear via TestClient, so just check that the header exists
...
class TestHealthEndpointNoAuthLeak:
    ...
    def test_health_no_auth_leak_source_check(self):
        """Verify the health endpoint source does not use _is_auth_enabled()."""
        import biocompiler.api as api_mod
        import inspect
...
class TestProvenancePathTraversal:
    ...
    def test_load_rejects_non_uuid_string(self, store):
        """load() should reject strings that are not valid UUIDs."""
        with pytest.raises(ValueError, match="Invalid record_id format"):
    ...
    def test_load_accepts_valid_uuid(self, store):
        """load() should accept a valid UUID (even if file does not exist)."""
        # Should raise FileNotFoundError (not ValueError) for a valid UUID that does not exist
    ...
    def test_uuid_regex_in_source(self):
        ...
        import biocompiler.provenance.decision_provenance as dp_mod
        import inspect
...
class TestChunkedEncodingSizeCheck:
    ...
    def test_oversized_request_rejected(self):
        ...
        # but we can at least verify the middleware exists and does not crash
