...
# ═══════════════════════════════════════════════════════════════════════════
# 2. PrecomputedAlleleDatabase tests (REMOVED — prior fix)
# ═══════════════════════════════════════════════════════════════════════════
# The entire precomputed/ subpackage (~20,500 LOC) was deleted in prior fix
# because 25 entries labelled source="known_epitope" had fabricated IC50
# values and 2,075 pssm_predicted entries were stale.  Tests that exercised
# the deleted per-allele databases (TestPrecomputedAlleleDatabase) have been
# removed.  The PrecomputedEntry / PrecomputedAlleleDatabase dataclasses
# remain in mhc_binding_db/__init__.py for backward compatibility, and
# get_database() now always returns None (callers fall through to PSSM
# fallback).  Honest published IC50 values for ~18 peptide-allele pairs
# remain in mhc_binding_db.schema.PRECOMPUTED_BINDERS.


class TestPrecomputedEntryDataclass:
    """Tests for the (still-existing) PrecomputedEntry legacy dataclass.

    The precomputed/ SUBPACKAGE was deleted in prior fix, but the
    PrecomputedEntry / PrecomputedAlleleDatabase dataclasses remain in
    mhc_binding_db/__init__.py for backward compatibility.  These tests
    verify the dataclass behaviour without touching the deleted subpackage.
    """
    ...
    def test_get_database_returns_none_after_w3c(self):
        """After prior fix, get_database() always returns None (subpackage deleted)."""
        # The precomputed/ subpackage was deleted; get_database is a stub.
        result = get_database("HLA-A*02:01")
...
class TestMHCFlurryAdapterOffline:
    ...
    def test_pssm_fallback_for_known_epitope(self):
        """After prior fix, known epitopes fall through to PSSM (precomputed deleted).

        The precomputed/ subpackage was deleted; _lookup_precomputed now
        always returns None, so even known epitopes like GILGFVFTL are
        scored via the runtime PSSM with confidence=0.5.
        """
        ...
        assert result is not None
        # Precomputed lookup is gone — must be PSSM fallback
