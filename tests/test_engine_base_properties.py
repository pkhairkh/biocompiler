...
class TestEngineTimerElapsedTime:
    ...
    def test_elapsed_positive_after_context_with_mock(self):
        ...
        with patch("biocompiler.engines.base.time.perf_counter", side_effect=mock_perf_counter):
    ...
    def test_elapsed_positive_even_with_no_work(self):
        ...
        with patch("biocompiler.engines.base.time.perf_counter", side_effect=mock_perf_counter):
            with EngineTimer() as timer:
                pass  # no-op, time does not advance
            assert timer.elapsed >= 0.0
    ...
    def test_two_timers_ordering_with_mock(self):
        ...
        with patch("biocompiler.engines.base.time.perf_counter", side_effect=mock_perf_counter_short):
        ...
        with patch("biocompiler.engines.base.time.perf_counter", side_effect=mock_perf_counter_long):
...
class TestValidateProteinSequenceRejectsInvalid:
    ...
    def test_rejects_lowercase_not_in_standard(self):
        """Lowercase letters that are not amino acids after upper-casing
