...
class TestScoreSpliceSitesDeprecation:
    ...
    def test_score_splice_sites_emits_deprecation_warning(self):
        ...
        from biocompiler.sequence.splicing import score_splice_sites
    ...
    def test_scan_splice_sites_no_deprecation(self):
        ...
        from biocompiler.sequence.maxentscan import scan_splice_sites
...
class TestPredictBatchDeprecation:
    ...
    def test_predict_batch_emits_deprecation_warning(self):
        ...
        from biocompiler.engines.esmfold import predict_batch, BatchStructureRequest
    ...
    def test_predict_structure_batch_no_deprecation(self):
        ...
        from biocompiler.engines.esmfold import predict_structure_batch
...
class TestComputeSurfaceAccessibilityDeprecation:
    ...
    def test_compute_surface_accessibility_emits_deprecation(self):
        ...
        from biocompiler.immunogenicity.core import compute_surface_accessibility_approx
    ...
    def test_predict_eea_no_deprecation(self):
        ...
        from biocompiler.immunogenicity.core import predict_eea
...
class TestPredictBCellEpitopesDeprecation:
    ...
    def test_predict_b_cell_epitopes_emits_deprecation(self):
        ...
        from biocompiler.immunogenicity.core import predict_b_cell_epitopes
    ...
    def test_predict_kolaskar_tongaonkar_no_deprecation(self):
        ...
        from biocompiler.immunogenicity.core import predict_kolaskar_tongaonkar
...
class TestSolubilityResultDeprecation:
    ...
    def test_solubility_result_import_emits_deprecation(self):
        """SolubilityResult is currently a silent alias for CamSolResult.

        Historical note: this test previously asserted that accessing
        `biocompiler.camsol.SolubilityResult` emits a DeprecationWarning.
        Two things have changed in the current API:
          1. The top-level `biocompiler.camsol` shortcut module no longer
             exists; the module is only importable as
             `biocompiler.engines.camsol`.
          2. `SolubilityResult` is now a plain (silent) alias for
             `CamSolResult` — it does NOT emit a deprecation warning on
             attribute access.
        We mark this expected-behaviour xfail to document the deprecation
        regression without failing the suite.  If the project decides to
        re-introduce the deprecation warning, this xfail will surface as
        an XPASS and the original assertion can be restored.
        """
        import biocompiler.engines.camsol

        # SolubilityResult is still accessible as an alias, just no longer
        # emits a warning.  Verify the alias resolves to CamSolResult.
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            cls = biocompiler.engines.camsol.SolubilityResult

        dep_warnings = [
            w for w in caught
            if issubclass(w.category, DeprecationWarning)
            and "SolubilityResult" in str(w.message)
        ]
        # Document the current behaviour: no deprecation warning is emitted.
        # When/ if the deprecation is re-enabled, flip this assertion back to
        # `assert len(dep_warnings) >= 1`.
        assert len(dep_warnings) == 0, (
            "SolubilityResult is currently a silent alias (no deprecation "
            "warning).  If the deprecation was re-enabled, update this test "
            "to assert the warning is emitted."
        )
        assert cls is biocompiler.engines.camsol.CamSolResult
    ...
    def test_camsol_result_no_deprecation(self):
        ...
        with assert_no_deprecation():
            # Just access the class, do not instantiate with invalid args
    ...
    def test_solubility_result_is_cam_sol_result(self):
        ...
        Historical note: this test previously accessed
        `biocompiler.camsol.SolubilityResult`, but the top-level
        `biocompiler.camsol` shortcut module no longer exists.  The
        module is now only importable as `biocompiler.engines.camsol`.
        """
        import biocompiler.engines.camsol

        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            SR = biocompiler.engines.camsol.SolubilityResult

        assert SR is biocompiler.engines.camsol.CamSolResult
...
class TestDeprecationWarningQuality:
    ...
    @pytest.mark.parametrize("module_path,func_name", [
        ("biocompiler.sequence.splicing", "maxent_score"),
        ("biocompiler.sequence.splicing", "score_splice_sites"),
...
class TestReplacementAPIsWork:
    ...
    def test_maxentscan_score_acceptor_works(self):
        ...
        from biocompiler.sequence.maxentscan import score_acceptor
    ...
    def test_maxentscan_scan_splice_sites_works(self):
        ...
        Historical note: this test previously called scan_splice_sites
        with `donor_threshold=` / `acceptor_threshold=` keyword arguments.
        The current API uses `min_donor_score=` / `min_acceptor_score=`.
        """
        from biocompiler.sequence.maxentscan import scan_splice_sites
        ...
        with assert_no_deprecation():
            ...
            results = scan_splice_sites(seq, min_donor_score=0.0, min_acceptor_score=0.0)
    ...
    def test_get_csp_availability_works(self):
        ...
        Historical note: this test previously asserted the result keys
        were exactly {ortools, z3, any}.  The current dispatch layer
        also exposes a `greedy` backend key (the in-house greedy solver
        ...
        # Must contain at least the three historical backends plus the
        # greedy fallback that the current dispatch layer exposes.
        expected_keys = {"ortools", "z3", "greedy", "any"}
