/-
  ...
  PREDICATE ARCHITECTURE (36 constructors):
  - Core (17): fully evaluable from sequence + context, can return PASS/FAIL/UNCERTAIN
    ...
    ValidCodingSeq, CodonOptimality, NoCrypticPromoter, NoNewGlycosylationSite,
    NoRibosomalFrameshift, NoMiRNABindingSite, NoLongHydrophobicStretch
    NOTE: NoNewGlycosylationSite is a DELEGATED core predicate — it is classified
    as core (no external oracle needed; the check is decidable from sequence alone
    if the original protein were available), but in the Lean4 model it always returns
    PASS because the original protein sequence is not available in the formal model.
    The actual N-glycosylation site comparison is performed in the Python implementation.
    Its soundness proof is therefore vacuously true by design, analogous to how SLOT
    predicate soundness is vacuously true in CONSERVATIVE mode.
    NOTE: NoMiRNABindingSite is likewise a DELEGATED core predicate — the full miRNA
    seed-database matching is deterministic and performed in the Python implementation
    (bundled seed DB, no external oracle); the Lean4 model always returns PASS and
    propertyHolds is True, so soundness is vacuously true.
    NOTE: NoRibosomalFrameshift and NoLongHydrophobicStretch are DETERMINISTIC core
    predicates with real sequence checks AND non-vacuous soundness proofs below.
...
-- ==============================================================================
-- Type Predicates (36 constructors: 17 core + 19 SLOT-dependent)
-- ==============================================================================
...
inductive TypePredicate where
  ...
  | NoNewGlycosylationSite : TypePredicate
  ...
  | NoRibosomalFrameshift : TypePredicate
  | NoMiRNABindingSite : TypePredicate
  deriving Repr
...
def isSLOT : TypePredicate → Bool
  ...
  | TypePredicate.NoNewGlycosylationSite => false
  ...
  | TypePredicate.NoLongHydrophobicStretch _ => false
  ...
  | TypePredicate.NoRibosomalFrameshift => false
  | TypePredicate.NoMiRNABindingSite => false
...
-- ==============================================================================
-- Deterministic Scanner Helpers for Moved Core Predicates
-- ==============================================================================

/-- Slippery sequence: a homopolymeric run of 7 A's (poly-A runs cause
    ribosomal frameshifting, e.g. in viral genomes). DNA form of poly-A. -/
def slipperySeqA : Sequence := List.replicate 7 Nucleotide.A

/-- Slippery sequence: a homopolymeric run of 7 T's (poly-U/T runs cause
    ribosomal frameshifting). DNA form of poly-U. -/
def slipperySeqT : Sequence := List.replicate 7 Nucleotide.T

/-- Check if the sequence contains a slippery (frameshift-prone) homopolymer
    run of length 7 (poly-A or poly-T). These are the most common
    ribosomal frameshift slippery sequences in eukaryotic and viral mRNAs. -/
def hasSlipperySequence (seq : Sequence) : Bool :=
  hasPattern seq slipperySeqA || hasPattern seq slipperySeqT

/-- Check if all codons in a window of `count` codons starting at codon
    index `startCodon` (0-based, codon-aligned) are hydrophobic.
    Each codon is checked via `isHydrophobicCodon`. -/
def allHydrophobicCodonsIn (seq : Sequence) (startCodon : Nat) (count : Nat) : Bool :=
  (List.range count).all fun i =>
    isHydrophobicCodon ((seq.drop (3 * (startCodon + i))).take 3)

/-- Check if the sequence contains a hydrophobic stretch of MORE THAN `maxLen`
    consecutive hydrophobic codons. Returns true iff such a stretch exists. -/
def hasLongHydrophobicStretch (seq : Sequence) (maxLen : Nat) : Bool :=
  if maxLen + 1 > seq.length / 3 then false
  else
    (List.range (seq.length / 3 - (maxLen + 1) + 1)).any fun start =>
      allHydrophobicCodonsIn seq start (maxLen + 1)

/-- THEOREM (hasLongHydrophobicStretch completeness): If a window of
    `maxLen + 1` consecutive hydrophobic codons starting at codon index
    `startCodon` exists (i.e. `allHydrophobicCodonsIn` returns true and the
    window fits within the coding sequence), then `hasLongHydrophobicStretch`
    returns true. -/
theorem hasLongHydrophobicStretch_complete (seq : Sequence) (maxLen : Nat)
    (startCodon : Nat)
    (h_bound : startCodon + (maxLen + 1) ≤ seq.length / 3)
    (h_all : allHydrophobicCodonsIn seq startCodon (maxLen + 1) = true) :
    hasLongHydrophobicStretch seq maxLen = true := by
  unfold hasLongHydrophobicStretch
  have h_not_gt : ¬(maxLen + 1 > seq.length / 3) := by omega
  rw [if_neg h_not_gt]
  rw [List.any_eq_true]
  refine ⟨startCodon, ?_, h_all⟩
  · rw [List.mem_range]
    omega
...
def evaluate [SpliceSiteScanner] [CodonAdaptationIndex] [CpGIslandScanner]
  ...
  | TypePredicate.NoNewGlycosylationSite, seq, _ =>
      -- DELEGATED CORE PREDICATE:
      -- NoNewGlycosylationSite is classified as core because the check is
      -- decidable from sequence alone (scan for N-X-S/T motifs) IF the original
      -- protein sequence were available for comparison. However, the Lean4 formal
      -- model does not carry the original protein sequence, so this predicate
      -- always returns PASS here. The actual N-glycosylation site comparison
      -- (original vs. modified protein) is performed in the Python implementation,
      -- which has access to both sequences. This is analogous to SLOT predicates,
      -- which also delegate the real check to an external system, but unlike SLOT
      -- predicates, NoNewGlycosylationSite does NOT require an external oracle —
      -- only the original protein sequence, which is a matter of model context,
      -- not oracle dependency. The soundness proof for this predicate is vacuously
      -- true by design (PASS → True, since propertyHolds is defined as True for
      -- this predicate).
      PASS

  | TypePredicate.NoRibosomalFrameshift, seq, _ =>
      -- DETERMINISTIC CORE PREDICATE:
      -- Checks for slippery homopolymeric sequences (poly-A or poly-T runs of
      -- length ≥ 7) that cause ribosomal frameshifting. This is a pure
      -- sequence-pattern check — no external tool needed.
      if hasSlipperySequence seq = true then FAIL else PASS

  | TypePredicate.NoMiRNABindingSite, seq, _ =>
      -- DELEGATED CORE PREDICATE:
      -- The full miRNA seed-database matching is deterministic (the seed DB is
      -- bundled with the Python implementation, no external oracle needed).
      -- However, the Lean4 formal model does not carry the miRNA seed database,
      -- so this predicate always returns PASS here. The actual seed-matching
      -- check is performed in the Python implementation. The soundness proof is
      -- vacuously true by design (PASS → True, since propertyHolds = True).
      PASS

  | TypePredicate.NoLongHydrophobicStretch maxLen, seq, _ =>
      -- DETERMINISTIC CORE PREDICATE:
      -- Checks for hydrophobic amino acid stretches longer than `maxLen`
      -- consecutive hydrophobic codons. Pure sequence computation — no external
      -- tool needed. FAIL if such a stretch exists, PASS otherwise.
      if hasLongHydrophobicStretch seq maxLen = true then FAIL else PASS
...
def propertyHolds [SpliceSiteScanner] [CodonAdaptationIndex] [CpGIslandScanner]
  ...
  | TypePredicate.NoNewGlycosylationSite, seq, _ =>
      -- DELEGATED CORE PREDICATE: Since the original protein sequence is not
      -- available in the Lean4 model, we cannot meaningfully check whether new
      -- N-glycosylation sites have been introduced. We define propertyHolds as
      -- True, making the soundness proof vacuously true (PASS → True). The real
      -- check is performed in the Python implementation where both the original
      -- and modified protein sequences are available for comparison.
      True
  | TypePredicate.NoRibosomalFrameshift, seq, _ =>
      -- DETERMINISTIC CORE PREDICATE: No slippery homopolymeric sequence
      -- (poly-A or poly-T run of length ≥ 7) appears anywhere in the sequence.
      (∀ (pos : Nat), pos + slipperySeqA.length ≤ seq.length →
        (seq.drop pos).take slipperySeqA.length ≠ slipperySeqA) ∧
      (∀ (pos : Nat), pos + slipperySeqT.length ≤ seq.length →
        (seq.drop pos).take slipperySeqT.length ≠ slipperySeqT)
  | TypePredicate.NoMiRNABindingSite, seq, _ =>
      -- DELEGATED CORE PREDICATE: The full miRNA seed-database matching is
      -- performed in the Python implementation (bundled seed DB). The Lean4
      -- model defines propertyHolds as True, making soundness vacuously true.
      True
  | TypePredicate.NoLongHydrophobicStretch maxLen, seq, _ =>
      -- DETERMINISTIC CORE PREDICATE: No window of `maxLen + 1` consecutive
      -- codons (codon-aligned) is entirely hydrophobic.
      ∀ (startCodon : Nat),
        startCodon + (maxLen + 1) ≤ seq.length / 3 →
          ¬allHydrophobicCodonsIn seq startCodon (maxLen + 1) = true
...
theorem type_soundness [SpliceSiteScanner] [CodonAdaptationIndex] [CpGIslandScanner]
  ...
  | NoNewGlycosylationSite =>
    -- DELEGATED CORE PREDICATE: The soundness proof is vacuously true by design.
    -- In the Lean4 model, NoNewGlycosylationSite always returns PASS because the
    -- original protein sequence (needed for comparison) is not available. The
    -- propertyHolds function is defined as True for this predicate, so the
    -- implication PASS → True holds trivially. The actual N-glycosylation site
    -- comparison is performed in the Python implementation, which has access to
    -- both the original and modified protein sequences.
    trivial

  | NoRibosomalFrameshift =>
    -- DETERMINISTIC CORE PREDICATE: PASS means hasSlipperySequence = false,
    -- i.e. neither poly-A nor poly-T slippery runs appear. The proof mirrors
    -- NoInstabilityMotif: use hasPattern completeness + Bool.or_false lemmas.
    simp only [evaluate] at h_pass
    have h_not_true : hasSlipperySequence seq ≠ true := by
      intro h; rw [if_pos h] at h_pass; cases h_pass
    have h_false : hasSlipperySequence seq = false :=
      (bool_ne_true_iff_false _).mp h_not_true
    simp only [propertyHolds]
    constructor
    · intro pos h_pos h_match
      have h_has : hasPattern seq slipperySeqA = true :=
        hasPattern_complete seq slipperySeqA pos h_pos h_match
      unfold hasSlipperySequence at h_false
      have : hasPattern seq slipperySeqA = false :=
        Bool.or_false_left (hasPattern seq slipperySeqA) (hasPattern seq slipperySeqT) h_false
      rw [this] at h_has
      cases h_has
    · intro pos h_pos h_match
      have h_has : hasPattern seq slipperySeqT = true :=
        hasPattern_complete seq slipperySeqT pos h_pos h_match
      unfold hasSlipperySequence at h_false
      have : hasPattern seq slipperySeqT = false :=
        Bool.or_false_right (hasPattern seq slipperySeqA) (hasPattern seq slipperySeqT) h_false
      rw [this] at h_has
      cases h_has

  | NoMiRNABindingSite =>
    -- DELEGATED CORE PREDICATE: evaluate always returns PASS and propertyHolds
    -- is True, so soundness holds trivially.
    trivial

  | NoLongHydrophobicStretch maxLen =>
    -- DETERMINISTIC CORE PREDICATE: PASS means hasLongHydrophobicStretch = false.
    -- By the completeness theorem, if any window of (maxLen+1) consecutive
    -- hydrophobic codons existed, hasLongHydrophobicStretch would return true.
    -- The contrapositive gives us the property.
    simp only [evaluate] at h_pass
    have h_not_true : hasLongHydrophobicStretch seq maxLen ≠ true := by
      intro h; rw [if_pos h] at h_pass; cases h_pass
    have h_false : hasLongHydrophobicStretch seq maxLen = false :=
      (bool_ne_true_iff_false _).mp h_not_true
    simp only [propertyHolds]
    intro startCodon h_bound h_all
    have h_complete := hasLongHydrophobicStretch_complete seq maxLen startCodon h_bound h_all
    rw [h_false] at h_complete
    cases h_complete
