/-
  ...
  EXTENDED PREDICATE SET (36 total):
  - 17 core (deterministic) predicates: SpliceCorrect, NoCrypticSplice,
    ...
    CodonOptimality, NoCrypticPromoter, NoNewGlycosylationSite,
  ...
  - 19 SLOT-dependent predicates: ConservationScore, NoUnexpectedTMDomain,
    ...
    ChargeComposition, LowImmunogenicity,
...
def isCorePredicate : TypePredicate → Bool
  ...
  | TypePredicate.NoNewGlycosylationSite => true
  | TypePredicate.NoRibosomalFrameshift => true
  | TypePredicate.NoMiRNABindingSite => true
...
theorem predicate_is_core_or_slot (P : TypePredicate) :
  ...
  | NoLongHydrophobicStretch _ => left; rfl
  ...
  | NoRibosomalFrameshift => left; rfl
  | NoMiRNABindingSite => left; rfl
  | NoNewGlycosylationSite => left; rfl
...
theorem core_not_slot (P : TypePredicate) :
  ...
  | NoNewGlycosylationSite => simp [isSLOT]
  | NoRibosomalFrameshift => simp [isSLOT]
  | NoMiRNABindingSite => simp [isSLOT]
  | NoLongHydrophobicStretch _ => simp [isSLOT]
...
/-- THEOREM (Full SLOT-Independence Guarantee): For the BioCompiler type system:
    ...
    2. Core predicates are SLOT-independent (evaluate does not use SLOT values)
