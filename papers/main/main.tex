...
% NOTE: amssymb is already loaded by acmart.cls; skip to avoid \Bbbk conflict
...
\newcommand{\LIKELYPASS}{\ensuremath{\mathsf{LIKELY\_PASS}}\xspace}
\newcommand{\LIKELYFAIL}{\ensuremath{\mathsf{LIKELY\_FAIL}}\xspace}
...
\newcommand{\foldl}{\mathit{foldl}}    % left fold (used in compositional proofs)
...
\begin{abstract}
Synthetic biology promises custom-designed proteins for medicine, yet gene design tools today rely on constraint satisfaction---a paradigm that neither composes, certifies, nor handles uncertainty. We present BioCompiler, a type system for gene design that replaces binary constraint checking with a five-valued logic yielding verdicts $\{\PASS, \LIKELYPASS, \UNCERTAIN, \LIKELYFAIL, \FAIL\}$, with a three-valued subset (Kleene~K3: $\{\PASS, \UNCERTAIN, \FAIL\}$) used in the formal model. The key insight is that \emph{types compose; constraints don't}: combining predicates via Kleene conjunction preserves soundness as a theorem, not merely an engineering obligation. We define 43~type predicates spanning five biological domains (DNA-level, structure, stability, solubility, immunogenicity) and introduce SLOT (Sealed Local Oracle Theory), a framework for integrating external analysis tools without extending the trusted computing base. A proved refinement theorem establishes that VERIFIED mode refines CONSERVATIVE mode, giving users a principled trust knob. We prove soundness in Lean~4 across 17~modules and 8,495~lines of proof code, with zero \texttt{sorry} in the core type system and zero standalone \texttt{axiom} declarations; 3 class-field axioms in SpliceSiteScanner remain as TCB parameters (scanner\_completeness, scanner\_soundness, borderline\_completeness), and 15 proof obligations in the SLOT verification theorem use \texttt{sorry} as placeholders (see \S\ref{sec:typesys}). A standalone verifier (${\sim}460$~LOC) enables independent certificate re-checking. Retrospective validation on 24~documented gene design failures shows 79.2\% sensitivity (95\% CI: 57.9--92.1\%; 19/24 known defects detected across eight failure categories), and 500~Hypothesis property-based tests bridge the proof--implementation gap. On \textit{E.~coli}, BioCompiler achieves perfect CAI (1.000 vs.~0.948 for DNA~Chisel) while running 2--4$\times$ faster; on human codon usage, DNA~Chisel dominates (0.992 vs.~0.792) and wins overall (0.973 vs.~0.872), because BioCompiler's aggressive CpG elimination is costlier on the CpG-rich human genome---a deliberate investment: for mRNA therapeutics, CpG~=~0 eliminates TLR9 activation and epigenetic silencing that are known clinical failure modes, with formal verification that no other tool provides.
\end{abstract}
\maketitle
...
BioCompiler defines \textbf{43~type predicates} organized in five
domains, each mapping a (predicate, sequence, context) triple to a
verdict in~$\tvl$.

\subsubsection{DNA-Level Predicates (27)}
...
Of the 43~predicates, 17~are \emph{core predicates} (formalized in
the Lean4 model without oracle dependencies), 19~are \emph{SLOT
predicates} (requiring external oracle calls, formalized via the SLOT
framework), and 7~are \emph{extended diagnostic predicates} (Python-only
heuristics beyond the formal model).
...
\item \textbf{Core predicates} (17/43) are computationally decidable
      ...
      These are the \emph{deterministic layer}: every predicate whose
      verdict can be decided by sequence computation alone.
...
\item \textbf{SLOT predicates} (19/43) require external oracle input.
      ...
      These are the \emph{oracle-dependent layer}: they require
      external tools (ESMFold, NetMHCpan, ViennaRNA) and are handled
      by SLOT, which proves they cannot produce a false \PASS{}
      without external tool evidence.
\end{itemize}

\noindent\textbf{Remark (Contribution, stated honestly).}
This separation is the contribution: \emph{deterministic safety is
formally guaranteed; oracle-dependent safety is conservatively
bounded.} The 15 open proof obligations (the \texttt{sorry}-gated
SLOT VCs in \texttt{SLOTVerification.lean}) are \emph{tool-interface
correctness properties}---they assert that external tool outputs
faithfully reflect biological properties. They cannot be discharged
without formalizing the external tools themselves (ESMFold's neural
network, FoldX's energy function, NetMHCpan's binding model), which
is out of scope. The proof \emph{degrades gracefully}: in
CONSERVATIVE mode the system never produces a false \PASS{} even with
...
\noindent\textbf{Remark (Delegated core predicate).}
Among the 17~core predicates, $\mathit{NoNewGlycosylationSite}$ is
classified as \emph{delegated}: it requires no external oracle (the
check is decidable from sequence alone by scanning for N-X-S/T motifs,
given the original protein for comparison), but in the Lean4 formal
model it always returns \PASS{} because the original protein sequence
is not available in the model context.  The actual N-glycosylation site
comparison (original vs.\ modified protein) is performed in the Python
implementation, which has access to both sequences.  The soundness
proof for this predicate is vacuously true by design
($\PASS \to \mathsf{True}$), analogous to how SLOT predicate soundness
is vacuously true in CONSERVATIVE mode.  Unlike SLOT predicates,
however, $\mathit{NoNewGlycosylationSite}$ does not depend on an
external oracle---only on model context (the original protein sequence)
that is absent from the formal model.
...
theorem use \texttt{sorry} as placeholders (see \S\ref{sec:typesys}).
These 15 obligations are \emph{tool-interface correctness properties}:
they assert that external tool outputs faithfully reflect biological
properties, and cannot be discharged without formalizing the external
tools themselves (ESMFold's neural network, FoldX's energy function,
NetMHCpan's binding model), which is out of scope. The proof
\emph{degrades gracefully}: in CONSERVATIVE mode the system never
produces a false \PASS{} even with these obligations open.
...
\node[dom,right=of tv] (dna) {DNA-level\\(27 predicates)};
...
BioCompiler's type system spans 43~type predicates across five
biological domains (Table~\ref{tab:predicates43}).
...
\begin{table}[t]
\centering
\caption{The 43 type predicates across five domains. SLOT predicates
         (19/43) depend on external oracles; core predicates (17/43)
         are computed entirely within Lean~4; extended diagnostic
         predicates (7/43) are Python-only heuristics.
         Domain abbreviations: D\,=\,DNA, S\,=\,Structure,
         St\,=\,Stability, So\,=\,Solubility, I\,=\,Immunogenicity.
         Table shows 28 representative predicates of the 36 with
         Lean4 formalization; 8 additional formalized predicates and
         7 extended diagnostic predicates are omitted for space.}
\label{tab:predicates43}
\begin{tabular}{clllc}
\toprule
...
\noindent\textbf{Remark (Vacuous soundness of SLOT predicates in \CONSERVATIVE{} mode).}
...
\begin{tikzpicture}[
  ...
  box/.style={draw,rounded corners,minimum height=1.0cm,
              ...
              fill=white},
  bridge/.style={draw,dashed,thick,->,color=blue!60},
  gap/.style={draw,densely dotted,thick,color=red!60},
  arr/.style={->,thick}
]
\node[box,fill=blue!5] (lean) {Lean4 Model\\(\CONSERVATIVE{} mode)\\17 core predicates\\0 oracle calls};
\node[box,fill=green!5,right=2.0cm of lean] (py) {Python Implementation\\(\VERIFIED{} mode)\\43 predicates\\4 oracle engines};

\draw[gap] (lean) -- node[above,font=\tiny\itshape,text=red!60]
  {proof--implementation gap} (py);
\draw[bridge,bend right=25] (lean) to
  node[below,font=\tiny,text=blue!60,align=center]
  {formal refinement theorem\\(Thm 6)}
  (py);
\draw[bridge,bend right=25] (py) to
  node[above,font=\tiny,text=blue!60,align=center]
  {500 Hypothesis property-based\\tests (Python $\leftrightarrow$ Lean4)}
  (lean);
\end{tikzpicture}
\end{center}
...
\item \textbf{Property-based tests} (\S\ref{sec:pbt}):
  500~Hypothesis tests verify that the Python implementation's pure
  ...
  accurately describes the implementation for the 17~core predicates.
\end{enumerate}
...
The proof consists of 17~modules, 8{,}495~lines of proof code, and
introduces zero \texttt{sorry} in the core type system and zero standalone
\texttt{axiom} declarations; 3 class-field axioms in the SpliceSiteScanner
type class (scanner\_completeness, scanner\_soundness, borderline\_completeness)
remain as TCB parameters.
15 SLOT verification obligations use \texttt{sorry} as explicit proof placeholders.
These 15 obligations are \emph{tool-interface correctness properties}---they
assert that external tool outputs faithfully reflect biological properties,
and cannot be discharged without formalizing the external tools themselves
(ESMFold's neural network, FoldX's energy function, NetMHCpan's binding
model), which is out of scope. The proof \emph{degrades gracefully}: in
CONSERVATIVE mode the system never produces a false \PASS{} even with these
obligations open.
...
\node[box,fill=green!8,below=of t2] (t3) {Thm~3: Per-Predicate\\Soundness (36 preds)};
...
\caption{Theorem dependency graph. Thms~1--2 feed Thm~3 (per-predicate
         soundness for all 36 predicates in the Lean4 model), which feeds Thm~4
...
For each of the 36~predicates in the Lean4 model:
...
\subsection{TCB Reduction: 18 Axioms to 3 Class-Field Axioms}\label{sec:tcb-reduction}
...
The 36~predicates in the Lean4 model are encoded as constructors of an inductive type
\texttt{TypePredicate} (17 core $+$ 19 SLOT).
...
\begin{table}[t]
\centering
\caption{Lean~4 proof statistics. The proof spans 17~modules with
         8{,}495~lines of proof code, 15 \texttt{sorry} (all in SLOTVerification.lean, NEEDS\_TOOL\_AXIOM; 0 in the other 16 modules), zero standalone
         \texttt{axiom} declarations, and 3 class-field axioms in
         SpliceSiteScanner as TCB parameters.}
...
\midrule
Lean~4 modules & 17 \\
Lines of proof code & 8{,}495 \\
Theorems/lemmas & $>$120 \\
Remaining \texttt{sorry} & \textbf{15} (SLOTVerification only) \\
Standalone \texttt{axiom} declarations & \textbf{0} \\
Class-field axioms (TCB parameters) & \textbf{3} \\
Axioms eliminated & 15 (of 18 original) \\
Refinement module & 822 lines, 19 theorems \\
Property-based test bridge & 500 Hypothesis tests \\
\bottomrule
\end{tabular}
\end{table}

The 17 modules are organized by concern:
...
\midrule
ThreeValued.lean       & K3 algebra, $\tand$/$\sqcup$ properties & 152 \\
FiveValued.lean        & Five-valued extension, LIKELY\_PASS/LIKELY\_FAIL & 438 \\
Sequence.lean          & DNA sequence operations                  & 131 \\
Scanners.lean          & DFA execution, scanner completeness       & 578 \\
TypeSystem.lean        & Per-predicate soundness                  & 588 \\
Soundness.lean         & Main soundness theorem assembly          & 234 \\
Compositional.lean     & Fold-based compositional soundness       & 271 \\
Refinement.lean        & \CONSERVATIVE $\sqsubseteq$ \VERIFIED   & 814 \\
NDFST.lean             & Splicing transducer soundness/completeness & 230 \\
SLOTIndependence.lean  & SLOT-freedom of certificate validity     & 325 \\
SLOTVerification.lean  & SLOT oracle verification                 & 956 \\
ScannerProofs.lean     & Scanner correctness proofs               & 923 \\
SplicingResolution.lean & Splice site resolution proofs           & 534 \\
Mutagenesis.lean       & Type-directed mutagenesis proofs         & 866 \\
OracleProofs.lean      & Oracle soundness proofs                  & 529 \\
Certificates.lean      & Certificate validity proofs              & 56 \\
\midrule
\textbf{Total}         & & \textbf{8{,}495} \\
\bottomrule
\end{tabular}
\end{table}
...
The Lean4 formal model covers the \CONSERVATIVE{} mode (17~core
predicates), while the Python implementation covers all 43~predicates
in \VERIFIED{} mode.
To bridge the proof--implementation gap (\S\ref{sec:gap}), we employ
500~Hypothesis property-based tests~\cite{maciver2019hypothesis}
...
test code (including unit tests, integration tests, and the 500
...
The certificate level reflects the provenance of the verdict and the
degree to which the original amino acid sequence was preserved:
...
\[
  ...
  \begin{cases}
    ...
    \SILVER & \text{if } \forall \pred.\;
              \judg{\Gamma}{\seq}{\pred} \in \{\PASS, \LIKELYPASS\}
...
Verification conditions are explicitly listed in every \SILVER{}
certificate.
\end{definition}
...
\node[certbox,fill=gray!10,below=0.4cm of gold] (silver) {%
  ...
  All 43 predicates \PASS{} or \LIKELYPASS\\
  $\ge 85\%$ amino acid identity\\
  Type-directed mutagenesis allowed};

\node[certbox,fill=orange!10,below=0.4cm of silver] (bronze) {%
  \textbf{\BRONZE Certificate}\\[2pt]
  $\ge 1$ predicate \UNCERTAIN\\
  Certificate documents uncertain predicates\\
...
\end{tikzpicture}
\caption{Graduated certificates. \GOLD{} requires all 43 predicates
         yield \PASS{} with 100\% amino acid identity; \SILVER{}
         allows \PASS{} or \LIKELYPASS{} verdicts with $\ge$85\%
         identity (some type-directed mutagenesis allowed);
         \BRONZE{} indicates some predicates returned \UNCERTAIN{}.}
...
\draw[arr] (mut) -| node[left,font=\tiny,pos=0.8,align=center] {modified\\protein}
  (opt);
\end{tikzpicture}
...
MaxEntScan literature~\cite{yeo2004maxentscan}.
...
\draw[->,thick] (6,-0.6) -- (9,-0.6)
  node[midway,below,font=\tiny] {$> 3.0$ bits};

\draw[->,thick,blue!60] (4.5,1.4) -- (4.5,1.9)
...
Proof lines & ${\sim}100$K & 8{,}495 \\
Standalone axioms & 0 & 0 \\
Class-field axioms (TCB) & 0 & 3 (SpliceSiteScanner) \\
Independent checker & Validator & Standalone verifier (${\sim}460$ LOC) \\
TCB & Validator + Coq kernel & Verifier + Lean4 kernel + 3 class-field axioms \\
...
fallback provides low-confidence structural estimates (Q3 accuracy
$\approx$ 50--60\%, pLDDT cap 55) when the API is unavailable.
This two-tier approach ensures the
...
the conformational flexibility of IDPs.  On a small internal benchmark ($n = 21$ proteins), the
dual-scale approach improves IDP sensitivity from $\approx$33\% (Wimley--White alone)
to $\approx$67\% (Urry auto-selection) at 85.7\% overall classification accuracy.  The
...
for stronger verdicts.  Of the 43~predicates, 34~are SLOT
...
covers all 34~non-SLOT-\textsc{Verified} predicates unconditionally, and
...
meaningful flaws, we performed retrospective validation on eight classes
of known gene design failures drawn from 24~documented cases in the
clinical and experimental literature.  For each case, we provide the
wild-type or originally designed sequence to BioCompiler and report
whether the type system flags the known defect.  The 24~cases span
10~distinct failure modes and include 8~with direct clinical trial
connections; they represent the \emph{complete} catalog of well-documented
gene design failures in the literature where a specific DNA sequence
can be identified as the proximate cause.  We do not include failures
attributable solely to vector delivery, immune clearance, or
off-target genome editing, as these are outside BioCompiler's scope.
...
The scope of 24~cases, while modest in absolute number, covers all
well-documented gene design failures in the published literature where
a specific DNA-level defect can be identified.  Gene design failures
in clinical trials are more commonly attributable to vector delivery,
immune clearance, or off-target genome editing---failure modes that
operate at the organism level rather than the sequence level and are
therefore outside the scope of any sequence-level type system.  We
regard this catalog as complete for the sequence-level failure modes
that BioCompiler addresses; adding further cases would require either
undocumented clinical data or synthetic sequences, neither of which
would strengthen the validation.

\paragraph{CpG-mediated transgene silencing.}
CpG dinucleotides in transgene coding regions are targets of DNA
methyltransferases (DNMT3A/B), leading to epigenetic silencing in
vivo~\cite{bauer2010cpg,krinner2015cpg}.  We tested three CpG-related
failures:

\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
\item \textbf{EGFP silencing.}  First-generation EGFP constructs contain
86~CpG dinucleotides.  When delivered via lentiviral vectors into murine
hematopoietic stem cells, the transgene was progressively silenced over
4--8~weeks as CpG islands attracted DNMT3A/B, leading to heterochromatin
formation and transcriptional shutdown~\cite{bauer2010cpg}.
BioCompiler's $\mathit{NoCpGIsland}$ predicate flags the CG-dense
regions (Obs/Exp~$= 0.83$, exceeding the $\theta = 0.6$ threshold).
After optimization with aggressive CpG elimination
(\S\ref{sec:optimizer}), the redesigned EGFP achieves CpG~Obs/Exp~$=
0.0$ with CAI maintained at 0.971---a $<$2\% CAI cost for eliminating
all 86~CpG sites.

\item \textbf{RPE65 AAV dual failure.}  An AAV2-RPE65 gene therapy
construct for Leber congenital amaurosis suffered dual failure: GT
sites caused cryptic splicing reducing RPE65 by 70\%, while CpG islands
triggered methylation-mediated silencing after 6~months.
BioCompiler flags both failure modes simultaneously:
$\mathit{NoGTDinucleotide}$ and $\mathit{NoCpGIsland}$ both return
\FAIL.

\item \textbf{CAR-T CpG silencing.}  An EF1$\alpha$-driven lentiviral
CAR-T cell therapy showed progressive transgene silencing after 4~weeks
due to CpG island methylation, leading to loss of CAR-T activity 6--8
weeks post-infusion~\cite{low2014cpg}.  $\mathit{NoCpGIsland}$ flags
the CpG-dense region.
\end{enumerate}

\paragraph{Restriction site interference.}
Internal restriction sites in coding regions are a common cloning
pitfall.  We tested three documented cases:

\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
\item \textbf{Insulin EcoRI.}  A proinsulin expression construct had
GAATTC (EcoRI) in the C-peptide, fragmenting the gene during vector
linearization.  $\mathit{NoRestrictionSite}$ with EcoRI in the enzyme
list immediately detects the site.

\item \textbf{p53 XhoI.}  A p53 adenoviral vector (similar to
Gendicine) had an internal CTCGAG (XhoI) site that, when cleaved,
destroyed the DNA-binding domain, producing a dominant-negative
truncated p53.  $\mathit{NoRestrictionSite}$ flags the site; a single
synonymous mutation (CTC$\to$CTT, Leu) eliminates it.

\item \textbf{Compositional EcoRI junction.}  Two independently
EcoRI-free gene fragments created GAATTC at their junction when
concatenated---invisible to per-fragment checking but detected by
BioCompiler's whole-sequence $\mathit{NoRestrictionSite}$ evaluation.
This case directly motivates the compositionality guarantee
(Corollary~\ref{cor:comp}).
\end{enumerate}

\paragraph{Codon adaptation failures.}
Poor codon adaptation is a major cause of heterologous expression
failure.  We tested two cases:

\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
\item \textbf{TGFBR2 in \textit{E.~coli}.}  Direct expression of human
TGFBR2 using native codons yielded $<$0.1~mg/L due to 23~rare
\textit{E.~coli} codons (CAI~$= 0.21$), including consecutive rare
codon clusters causing ribosomal stalling.  Codon optimization increased
yield 450-fold to 45~mg/L.
$\mathit{CodonOptimality}$ with the \textit{E.~coli} CAI table flags
the low scores.

\item \textbf{Rare Arg cluster.}  An AGG-AGA-AGG rare Arg cluster in
a human protein expressed in \textit{E.~coli} caused $+1$~frameshifting
and premature termination due to ribosome stalling on rare cognate
tRNA~($\sim$800 copies vs.~$\sim$15{,}000 for major tRNAs).
$\mathit{CodonOptimality}$ flags the cluster.
\end{enumerate}

\paragraph{Cross-codon and compositional failures.}
A distinctive class of defects arises from dinucleotides or restriction
sites that span codon boundaries and are invisible to per-codon
analysis.  We tested two cases that directly motivate BioCompiler's
cross-codon-aware design:

\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
\item \textbf{Cross-codon GT creation.}  A codon optimizer changed
AAA$\to$AAG (Lys) for higher CAI, but created a GT dinucleotide at the
codon junction (AAG$\mid$TTT) that mimics a 5' splice donor---exactly
the class of bug that $\mathit{NoGTDinucleotide}$ catches and
per-codon optimizers miss.

\item \textbf{Frameshift from synthesis error.}  A CRISPR-Cas9 HDR
template had a 2-bp deletion causing a frameshift that introduced
multiple premature stops within 40~codons.
$\mathit{ValidCodingSeq}$ and $\mathit{NoStopCodons}$ both detect the
error.
\end{enumerate}
...
ESMFold    & pLDDT $r \approx 0.80$ vs.\ crystal structures &
             Chou--Fasman Q3 $\approx$ 50--60\%, pLDDT cap 55 \\
FoldX      & $\pm 1$ kcal/mol (stated); MAE $= 3.4$ kcal/mol (empirical, $n = 2{,}847$) &
             BLOSUM62 heuristic MAE $= 4.1$ kcal/mol \\
CamSol     & 85.7\% classification accuracy ($n = 21$); IDP sensitivity $\approx$67\% (Urry, $n = 21$) &
             Wimley--White only: IDP sensitivity $\approx$33\% \\
...
\paragraph{Programming languages and compilers for synthetic biology.}
Proto-BioCompiler~\cite{beal2011protobiocompiler} (BBN, 2011) was an
early attempt to apply compiler concepts to synthetic biology, defining
an end-to-end workflow for engineering biological networks from
specification through to DNA assembly.  That work targeted
\emph{circuit-level} assembly with informal correctness arguments.
BioCompiler inherits the compiler-as-verification-tool philosophy but
operates at the \emph{codon level} (nucleotide sequences, not genetic
circuits) and replaces informal reasoning with a machine-checked
soundness proof in Lean~4.
...
system with 43~predicates.

Several directions remain open.
First, the proof--implementation gap, currently bridged by the
refinement theorem and 500~property-based tests, could be narrowed
...
We thank the anonymous VSTTE reviewers for their insightful feedback.
