%% BioCompiler (VSTTE submission) — Sections 4--6
%% "BioCompiler: A Type System for Machine-Verified Gene Design with Proven Soundness"
%%
%% This file is \input{} by main.tex.
%% The standalone preamble below is commented out — it was used for
%% independent compilation during drafting but must not be active when
%% this file is included by the main document.

%% --- BEGIN STANDALONE PREAMBLE (commented out) ---
...
% \subtitle{Sections 4--6 (VSTTE Draft)}
% \author{Anonymous}
...
\noindent\textbf{Remark (Delegated core predicates).}
Among the 17~core predicates, $\mathit{NoNewGlycosylationSite}$ is
classified as \emph{delegated}: it requires no external oracle (the check
is decidable from sequence alone by scanning for N-X-S/T motifs, given
the original protein for comparison), but in the Lean4 formal model it
always returns \PASS{} because the original protein sequence is not
available in the model context. The actual N-glycosylation site comparison
(original vs.\ modified protein) is performed in the Python implementation,
which has access to both sequences. The soundness proof for this predicate
is vacuously true by design ($\PASS \to \mathsf{True}$), analogous to
how SLOT predicate soundness is vacuously true in \CONSERVATIVE{} mode.
Unlike SLOT predicates, however, $\mathit{NoNewGlycosylationSite}$ does
not depend on an external oracle---only on model context (the original
protein sequence) that is absent from the formal model.
...
% \end{document}  % Commented out: this file is \input{} by main.tex
