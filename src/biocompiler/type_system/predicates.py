...
from biocompiler.sequence.sliding_gc import evaluate_sliding_gc, check_sliding_gc, SlidingGCResult, WindowViolation
...
def evaluate_no_cryptic_splice(
    ...
    cryptic_threshold: float = 8.0,
    uncertain_lo: float = 3.0,
    organism: str = "",
) -> TypeCheckResult:
    """Evaluate whether the sequence contains cryptic splice sites.

    Uses the MaxEntScan first-order Markov model for scoring.  The default
    threshold of 8.0 matches the calibrated CRYPTIC_SPLICE_THRESHOLD from
    ``maxentscan.py`` (Yeo & Burge, 2004).  A borderline range [3.0, 8.0)
    yields UNCERTAIN verdicts, reflecting the ~30% false-positive rate for
    scores below 8.0.
    """
    ...
    # Use the provided threshold directly — defaults are already aligned
    # with maxentscan.CRYPTIC_SPLICE_THRESHOLD (8.0) and
    # maxentscan.BORDERLINE_SPLICE_THRESHOLD (3.0).
        ...
        if has_iupac:
            for i in range(len(seq) - len(site) + 1):
                ...
                for s_base, p_base in zip(window, site.upper()):
                    from biocompiler.shared.constants import IUPAC_EXPAND
...
def evaluate_mrna_secondary_structure(
    ...
    # Try ViennaRNA engine first — if it returns a verdict_suggestion
    # of UNCERTAIN or uses gc_heuristic_fallback, propagate UNCERTAIN.
    try:
        from biocompiler.engines.viennarna import check_mrna_structure_viennarna
        vr_result = check_mrna_structure_viennarna(
            seq, window_start, window_end, dg_threshold, organism,
        )
        if isinstance(vr_result, dict):
            if vr_result.get("verdict_suggestion") == "UNCERTAIN" or vr_result.get("method") == "gc_heuristic_fallback":
                dg_val = vr_result.get("dg", 0.0)
                return TypeCheckResult(
                    predicate=f"mRNASecondaryStructure({window_start}, {window_end}, {effective_threshold})",
                    verdict=Verdict.UNCERTAIN,
                    violation=(
                        f"mRNA secondary structure from GC heuristic fallback: "
                        f"dG={dg_val:.1f} kcal/mol [UNCERTAIN: heuristic estimate, not ViennaRNA]"
                    ),
                )
            # ViennaRNA succeeded — use its dG directly
            if "dg" in vr_result and vr_result.get("viennarna_used"):
                vr_dg = vr_result["dg"]
                if vr_dg <= effective_threshold:
                    vr_verdict = Verdict.FAIL
                    vr_violation = (
                        f"Strong mRNA secondary structure (ViennaRNA): dG={vr_dg:.1f} kcal/mol "
                        f"<= {effective_threshold}"
                    )
                elif vr_dg <= effective_threshold * _MRNA_MODERATE_DG_RATIO:
                    vr_verdict = Verdict.UNCERTAIN
                    vr_violation = (
                        f"Moderate mRNA secondary structure (ViennaRNA): dG={vr_dg:.1f} kcal/mol "
                        f"<= {effective_threshold * _MRNA_MODERATE_DG_RATIO:.1f}"
                    )
                else:
                    vr_verdict = Verdict.LIKELY_PASS
                    vr_violation = None
                return TypeCheckResult(
                    predicate=f"mRNASecondaryStructure({window_start}, {window_end}, {effective_threshold})",
                    verdict=vr_verdict,
                    violation=vr_violation,
                )
    except (ImportError, AttributeError, RuntimeError):
        pass  # Fall through to the hairpin model below
...
def evaluate_all_predicates(
    ...
    """Evaluate all 43 type predicates against a sequence."""
    ...
    from biocompiler.provenance.slot_verification import is_slot_predicate, verify_slot_predicate
...
# ── Extended Diagnostic Predicates (L7, L10, L12, L3, L4, PolyA, Nucleoside) ──

def evaluate_no_cryptic_orf(
    seq: str,
    min_orf_length: int = 30,
    organism: str = "",
) -> TypeCheckResult:
    """Evaluate whether the sequence contains cryptic open reading frames."""
    result = check_no_cryptic_orf(seq, min_orf_length=min_orf_length, organism=organism)
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate=f"NoCrypticORF(min={min_orf_length})",
            verdict=Verdict.FAIL,
            violation=result.details,
        )
    elif result.verdict == Verdict.UNCERTAIN:
...
def evaluate_no_rqc_trigger(
    seq: str,
    organism: str = "",
    poly_a_min_length: int = 6,
) -> TypeCheckResult:
    """Evaluate whether the sequence contains RQC (Ribosome Quality Control) triggers."""
    result = check_no_rqc_trigger(seq, organism=organism, poly_a_min_length=poly_a_min_length)
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate="NoRQCTrigger",
            verdict=Verdict.FAIL,
            violation=result.details,
        )
    elif result.verdict == Verdict.UNCERTAIN:
...
def evaluate_no_alu_repeat(
    seq: str,
    organism: str = "",
    min_match_score: float = 0.75,
) -> TypeCheckResult:
    """Evaluate whether the sequence contains Alu repeat elements."""
    result = check_no_alu_repeat(seq, organism=organism, min_match_score=min_match_score)
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate=f"NoAluRepeat(threshold={min_match_score})",
            verdict=Verdict.FAIL,
            violation=result.details,
        )
    elif result.verdict == Verdict.UNCERTAIN:
...
def evaluate_no_mirna_binding_site(
    seq: str,
    organism: str = "Homo_sapiens",
    min_seed_match: int = 7,
    tissue: str = "",
) -> TypeCheckResult:
    """Evaluate whether the sequence contains miRNA binding sites."""
    result = check_no_mirna_binding_site(
        seq, organism=organism, min_seed_match=min_seed_match, tissue=tissue,
    )
    tissue_suffix = f",tissue={tissue}" if tissue else ""
    predicate_name = f"NoMiRNABindingSite(seed≥{min_seed_match}{tissue_suffix})"
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
...
def evaluate_no_m6a_site(
    seq: str,
    organism: str = "",
    scan_mode: str = "comprehensive",
) -> TypeCheckResult:
    """Evaluate whether the sequence contains m6A modification sites."""
    result = check_no_m6a_site(seq, organism=organism, scan_mode=scan_mode)
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate="NoM6ASite",
            verdict=Verdict.FAIL,
...
def evaluate_no_polya_signal(
    seq: str,
    organism: str = "",
    scan_cds_only: bool = True,
) -> TypeCheckResult:
    """Evaluate whether the sequence contains premature polyadenylation signals."""
    result = check_no_polya_signal(seq, organism=organism, scan_cds_only=scan_cds_only)
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate="NoPolyASignal",
            verdict=Verdict.FAIL,
            violation=result.details,
        )
    elif result.verdict == Verdict.UNCERTAIN:
...
def evaluate_nucleoside_modification_guidance(
    seq: str,
    organism: str = "",
    modification_type: str = "auto",
) -> TypeCheckResult:
    """Evaluate nucleoside modification guidance for mRNA therapeutics."""
    result = check_nucleoside_modification_guidance(
        seq, organism=organism, modification_type=modification_type
    )
    if result.verdict == Verdict.FAIL:
        return TypeCheckResult(
            predicate="NucleosideModificationGuidance",
            verdict=Verdict.FAIL,
            violation=result.details,
        )
    elif result.verdict == Verdict.UNCERTAIN:
...
# ── Issue #20: New Predicates (N-glycosylation, Ribosomal Frameshift, miRNA) ──

def evaluate_no_new_glycosylation_site(
    original_protein: str,
    modified_protein: str,
) -> TypeCheckResult:
    """Evaluate whether modified protein introduces new N-glycosylation sites.

    N-glycosylation sites follow the pattern N-X-S/T (asparagine, any amino
    acid except proline, followed by serine or threonine).  The check compares
    the set of site positions in the original vs. modified protein sequences
    and FAILs if any new sites were created by the modification.

    This is a **core** (deterministic) predicate — it requires no external
    tools and can return a definite PASS/FAIL verdict.

    Args:
        original_protein: Original amino acid sequence (single-letter codes).
        modified_protein: Modified amino acid sequence (single-letter codes).

    Returns:
        TypeCheckResult with PASS if no new sites, FAIL with positions otherwise.
    """
    pattern = _re.compile(r"N[^P][ST]")

    original_sites = {m.start() for m in pattern.finditer(original_protein.upper())}
    modified_sites = {m.start() for m in pattern.finditer(modified_protein.upper())}

    new_sites = modified_sites - original_sites

    if not new_sites:
        return TypeCheckResult(
            predicate="NoNewGlycosylationSite",
            verdict=Verdict.PASS,
        )

    new_positions = sorted(new_sites)
    return TypeCheckResult(
        predicate="NoNewGlycosylationSite",
        verdict=Verdict.FAIL,
        violation=(
            f"New N-glycosylation site(s) created at position(s): "
            f"{new_positions}. Pattern N-X-S/T found where original "
            f"sequence had none."
        ),
    )


# Known slippery heptamer DNA patterns associated with -1 ribosomal frameshifting.
# These are the DNA equivalents of the canonical RNA slippery heptamers
# (X XXY YYZ motif where X and Y are homogeneous codon groups).
_SLIPPERY_HEPTAMERS_DNA: list[str] = [
    "TTTAAAC",   # U UUA AAC
    "TTTAAAT",   # U UUA AAU
    "TTTAAG",    # U UUA AAG  (6-mer, will match 7-char window)
    "GGGAAAC",   # G GGA AAC
    "AAAAAAC",   # A AAA AAC
]

# GC content threshold above which downstream sequence is considered
# likely to form RNA secondary structure (pseudoknot / stem-loop).
_FRAMESHIFT_GC_STRUCTURE_THRESHOLD: float = 0.60
_FRAMESHIFT_DOWNSTREAM_WINDOW: int = 50


def evaluate_no_ribosomal_frameshift(
    seq: str,
    organism: str = "Homo_sapiens",
) -> TypeCheckResult:
    """Evaluate whether the DNA sequence contains ribosomal frameshift signals.

    A ribosomal frameshift requires both:
    1. A slippery heptamer sequence (known patterns like T TTAAAC, G GGAAAC, etc.)
    2. Downstream RNA secondary structure (heuristic: GC content > 60% in the
       next 50 bases after the heptamer)

    If both are present → FAIL (frameshift likely).
    If slippery heptamer present but structure check uncertain → UNCERTAIN.
    If no slippery heptamers → PASS.

    This is a **SLOT** predicate — the structure heuristic is a simplification
    of actual RNA folding prediction (which requires tools like ViennaRNA).

    Args:
        seq: DNA sequence to evaluate.
        organism: Target organism (for future organism-specific heptamers).
    ...
    slippery_positions: list[tuple[int, str]] = []
    for heptamer in _SLIPPERY_HEPTAMERS_DNA:
        start = 0
        while True:
            idx = seq_upper.find(heptamer, start)
            if idx == -1:
                break
            slippery_positions.append((idx, heptamer))
            start = idx + 1

    if not slippery_positions:
        return TypeCheckResult(
            predicate="NoRibosomalFrameshift",
            verdict=Verdict.PASS,
        )

    # Check downstream structure for each slippery heptamer
    for pos, heptamer in slippery_positions:
        downstream_start = pos + len(heptamer)
        downstream_end = min(downstream_start + _FRAMESHIFT_DOWNSTREAM_WINDOW, len(seq_upper))

        if downstream_end <= downstream_start:
            # Not enough downstream sequence — uncertain
            return TypeCheckResult(
                predicate="NoRibosomalFrameshift",
                verdict=Verdict.UNCERTAIN,
                knowledge_gap=(
                    f"Slippery heptamer '{heptamer}' found at position {pos}, "
                    f"but insufficient downstream sequence for structure check."
                ),
            )

        downstream = seq_upper[downstream_start:downstream_end]
        gc_count = sum(1 for b in downstream if b in ("G", "C"))
        gc_content = gc_count / len(downstream) if downstream else 0.0

        if gc_content > _FRAMESHIFT_GC_STRUCTURE_THRESHOLD:
            return TypeCheckResult(
                predicate="NoRibosomalFrameshift",
                verdict=Verdict.FAIL,
                violation=(
                    f"Ribosomal frameshift signal detected: slippery heptamer "
                    f"'{heptamer}' at position {pos} with GC-rich downstream "
                    f"region ({gc_content:.0%} GC in next {len(downstream)} bp)."
                ),
            )

    # Slippery heptamers found but no strong downstream structure signal
    return TypeCheckResult(
        predicate="NoRibosomalFrameshift",
        verdict=Verdict.UNCERTAIN,
        knowledge_gap=(
            f"Slippery heptamer(s) found at position(s) "
            f"{[p for p, _ in slippery_positions]}, but downstream GC content "
            f"does not indicate strong secondary structure. True structure "
            f"prediction requires RNA folding tools (ViennaRNA)."
        ),
    )
