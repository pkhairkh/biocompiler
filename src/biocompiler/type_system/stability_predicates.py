...
from biocompiler.shared.constants import HYDROPHOBIC_AAS
...
# ── MAE (Mean Absolute Error) of the empirical heuristic by protein size ──
# Used for confidence-aware threshold checks: if the predicted dG is within
# the MAE of the decision threshold, the verdict should be UNCERTAIN.
_EMPIRICAL_MAE_SMALL: float = 1.2      # < 100 aa
_EMPIRICAL_MAE_MEDIUM: float = 3.2     # 100–300 aa
_EMPIRICAL_MAE_LARGE: float = 9.8      # > 300 aa
...
def evaluate_stable_folding(
    ...
    if pdb_string is not None:
        try:
            from ..engines.foldx import empirical_stability as foldx_empirical
    ...
    # Issue #17: For large proteins (>300 aa), the empirical heuristic has
    # MAE ~9.8 kcal/mol — return UNCERTAIN instead of PASS/FAIL regardless
    # of pdb_string availability, since the empirical model does not scale.
    if method == "empirical" and len(protein) > 300:
        return TypeCheckResult(
            predicate=f"StableFolding({stability_threshold})",
            verdict=Verdict.UNCERTAIN,
            derivation=[
                ...
                {"step": "note", "value": "Large protein (>300 aa): empirical heuristic MAE ~9.8 kcal/mol, verdict forced to UNCERTAIN"},
            ],
            violation=(
                f"Empirical heuristic dG={dg:.2f} kcal/mol for {len(protein)}-aa protein "
                f"[UNCERTAIN: heuristic unreliable for proteins >300 aa, MAE ~9.8 kcal/mol]"
            ),
            knowledge_gap=(
                "Protein exceeds 300 residues; empirical stability heuristic has MAE ~9.8 "
                "kcal/mol for large proteins.  FoldX structure-based analysis required."
            ),
        )

    # Derive verdict
    #
    # Issue #17.5: Confidence-aware threshold check — if the predicted dG
    # is within the known MAE of the threshold, return UNCERTAIN because
    # we cannot be confident that the true value is on the correct side.
    if method in ("empirical", "empirical_fallback"):
        n_residues = len(protein)
        if n_residues < 100:
            mae = _EMPIRICAL_MAE_SMALL
        elif n_residues <= 300:
            mae = _EMPIRICAL_MAE_MEDIUM
        else:
            mae = _EMPIRICAL_MAE_LARGE
        if abs(dg - stability_threshold) <= mae:
            return TypeCheckResult(
                predicate=f"StableFolding({stability_threshold})",
                verdict=Verdict.UNCERTAIN,
                derivation=[
                    ...
                    {"step": "method", "value": method},
                    {"step": "threshold", "value": stability_threshold, "unit": "kcal/mol"},
                    {"step": "mae", "value": mae, "unit": "kcal/mol"},
                    {"step": "note", "value": f"dG within MAE ({mae}) of threshold ({stability_threshold}), verdict forced to UNCERTAIN"},
                ],
                violation=(
                    f"Predicted dG={dg:.2f} kcal/mol is within MAE ({mae}) "
                    f"of threshold ({stability_threshold}) "
                    f"[UNCERTAIN: prediction too close to threshold for confident verdict]"
                ),
                knowledge_gap=(
                    f"Empirical estimate MAE={mae} kcal/mol; predicted dG too close "
                    f"to threshold for a confident verdict.  Structure-based analysis needed."
                ),
            )
    #
    # When pdb_string is None, we are using empirical/heuristic estimates
    # (not FoldX or structure-based analysis). Per Issue #9, these fallback
    # paths should return UNCERTAIN instead of PASS/FAIL to avoid overstating
    # confidence in heuristic estimates.
    #
    if pdb_string is not None:
        # Structure-aware path: verdicts can be PASS/FAIL with higher confidence
        ...
        # Heuristic/empirical path (no FoldX, no PDB): cap verdicts at UNCERTAIN.
        # PASS/FAIL would overstate confidence in the heuristic dG estimate.
        if dg < stability_threshold:
            # UNCERTAIN: heuristic says stable but we cannot verify without structure
            verdict = Verdict.UNCERTAIN
            violation = None
        elif dg < stability_threshold / 2.0:
            verdict = Verdict.UNCERTAIN
            ...
            if confidence == "medium":
                ...
                violation = (
                    ...
                    f"[UNCERTAIN: empirical heuristic, not FoldX]"
                )
            else:
                verdict = Verdict.UNCERTAIN
                violation = (
            ...
            # UNCERTAIN: heuristic says unstable but we cannot verify without structure;
            # LIKELY_FAIL would overstate confidence in heuristic dG.
            verdict = Verdict.UNCERTAIN
            violation = (
                f"Predicted unstable: dG={dg:.2f} kcal/mol (>= 0) "
                f"[UNCERTAIN: empirical heuristic, not FoldX]"
            )
        else:
            # UNCERTAIN: even strongly heuristic-unstable should not be FAIL
            # without structure validation
            verdict = Verdict.UNCERTAIN
            violation = (
                f"Strongly unstable: dG={dg:.2f} kcal/mol (>= {_CLEARLY_UNSTABLE_DG}) "
                f"[UNCERTAIN: empirical heuristic, not FoldX]"
            )
...
def evaluate_no_destabilizing_mutation(
    ...
    # Verdict
    #
    # When pdb_string is None, ddG is estimated from BLOSUM62 heuristic (not
    # FoldX). Per Issue #9, heuristic PASS/FAIL verdicts should be UNCERTAIN
    # to avoid overstating confidence in approximate ddG estimates.
    #
    if not all_mutations:
        # No mutations at all → PASS is correct (identical sequences)
        return TypeCheckResult(
            predicate=f"NoDestabilizingMutation({max_ddg})",
            verdict=Verdict.PASS,
    ...
    if not destabilizing_positions:
        if pdb_string is not None:
            ...
            # UNCERTAIN: heuristic says no destabilizing mutations, but
            # BLOSUM62-based ddG is approximate; cannot be certain without structure.
            verdict = Verdict.UNCERTAIN
        violation = None
        ...
        if pdb_string is not None:
            ...
            # UNCERTAIN: heuristic says 1 destabilizing mutation, but
            # BLOSUM62 ddG estimate has high uncertainty.
            verdict = Verdict.UNCERTAIN
        ...
        if pdb_string is not None:
            ...
            # UNCERTAIN: heuristic says multiple destabilizing mutations, but
            # FAIL would overstate confidence in BLOSUM62 ddG estimates.
            verdict = Verdict.UNCERTAIN
        violation = (
    ...
    #
    # Note: Per Issue #9, when pdb_string is None, verdicts are already capped
    # at UNCERTAIN above. This BLOSUM62 refinement block can still upgrade
    # UNCERTAIN → LIKELY_PASS for clearly conservative substitutions, but
    # should NOT upgrade to PASS since we lack structural validation.
    if pdb_string is None and verdict in (Verdict.UNCERTAIN,):
        ...
        if worst_blosum >= 0:
            ...
            # but still UNCERTAIN because we lack structural data.
            # (Do NOT upgrade to LIKELY_PASS without structure validation.)
            derivation.append({
                "step": "blosum62_heuristic_note",
                "value": True,
                "original_verdict": original_verdict.value,
                "reason": f"worst BLOSUM62={worst_blosum} >= 0 (conservative substitutions), "
                          f"but still UNCERTAIN without structural validation",
            })
        elif worst_blosum >= -2:
            derivation.append({
                "step": "blosum62_heuristic_note",
                ...
                "reason": f"worst BLOSUM62={worst_blosum} >= -2 (mildly non-conservative), "
                          f"verdict remains UNCERTAIN without structural validation",
            })
        else:
            derivation.append({
                "step": "no_structure_note",
                "value": True,
                "original_verdict": original_verdict.value,
                "reason": f"worst BLOSUM62={worst_blosum} < -2 (radical substitution, need structure), "
                          f"verdict remains UNCERTAIN",
            })
...
def evaluate_disulfide_bond_integrity(
    ...
    # Intracellular proteins: disulfide bonds do not form in the reducing
...
def evaluate_hydrophobic_core_quality(
    ...
    #
    # When pdb_string is None, core quality is assessed from sequence only
    # (hydrophobic fraction, no burial verification). Per Issue #9, heuristic
    # PASS/FAIL verdicts should be UNCERTAIN to avoid overstating confidence.
    #
    if pdb_string is not None:
        # Structure-aware path: verdicts can be PASS/FAIL with higher confidence
        ...
        # Heuristic/sequence-only path: cap verdicts at UNCERTAIN.
        # PASS/FAIL would overstate confidence without structural validation.
        if core_quality_score > _CORE_QUALITY_PASS_THRESHOLD:
            # UNCERTAIN: composition looks good but burial is not verified
            verdict = Verdict.UNCERTAIN
            violation = None
        elif core_quality_score > 0.5:
            verdict = Verdict.UNCERTAIN
            violation = (
                f"Core quality score {core_quality_score:.3f} slightly below "
                f"PASS threshold ({_CORE_QUALITY_PASS_THRESHOLD}) "
                f"[UNCERTAIN: sequence-only heuristic, no structure]"
            )
        elif core_quality_score > 0.4:
            verdict = Verdict.UNCERTAIN
            violation = (
            ...
            # UNCERTAIN: even very low core quality should not be FAIL
