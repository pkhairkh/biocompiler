...
from biocompiler.solver.types import CodonVariable, ConstraintPriority, ConstraintSpec, SolverConfig, ConstraintStrictness, ConstraintType
from biocompiler.shared.constants import AA_TO_CODONS, CODON_TABLE, RESTRICTION_ENZYMES, INSTABILITY_MOTIF
...
from biocompiler.sequence.maxentscan import CRYPTIC_SPLICE_THRESHOLD, score_donor, score_acceptor
...
DEFAULT_EUKARYOTIC_SPLICE_THRESHOLD = CRYPTIC_SPLICE_THRESHOLD
CAI_DG_SCALE = 10.0
...
class NoCrypticSpliceConstraint(HardConstraint):
    ...
    For **eukaryotic organisms**, only sites with MaxEnt score > threshold
    (default ``CRYPTIC_SPLICE_THRESHOLD`` = 8.0 under the Markov model) are
    constrained.  Lower thresholds (e.g. 3.0 from the old PWM model) are too
    strict and flag many weak sites that are not functional splice signals.
...
class PatternAvoidanceConstraint(HardConstraint):
    ...
    **AvoidPattern** constraint (DNA-Chisel terminology).
    ...
    def check(self, sequence: str) -> bool:
        ...
        from biocompiler.sequence.pattern_enforcement import check_pattern
...
class MinimizeCodonPairBias(SoftConstraint):
    ...
    def __init__(self, organism: str) -> None:
        from biocompiler.expression.codon_pair_scoring import get_codon_pair_data
        self._organism = organism
...
class MinimizeMRNADG(SoftConstraint):
    ...
    def compute_dg(self, sequence: str) -> float:
        ...
        try:
            from biocompiler.engines.viennarna import compute_5prime_dg, is_viennarna_available
...
def _extract_constraint_params(constraint: HardConstraint | SoftConstraint) -> dict[str, Any]:
    ...
    """
    from biocompiler.solver.types import ConstraintType as CT
