...
class ConstraintSpec:
    ...
    def check(self, sequence: str) -> bool:
        ...
        if self.ctype in (
            ...
            if organism and not _is_eukaryotic(organism):
                return True
            threshold = self.params.get("threshold", CRYPTIC_SPLICE_THRESHOLD)
...
class SatisfiedConstraint:
    """A constraint that is satisfied in a solution.

    Mirrors :class:`ConstraintViolation` for constraints that pass,
    enabling the verdict integration layer to produce a complete
    mapping of every constraint to a :class:`Verdict`.

    Attributes:
        name: Human-readable identifier (e.g. 'no_EcoRI_site').
        strictness: Whether this was a hard or soft constraint.
    """
    ...
    def __hash__(self) -> int:
        return hash((self.name, self.strictness))

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, SatisfiedConstraint):
            return NotImplemented
        return self.name == other.name and self.strictness == other.strictness
...
class InfeasibilityReport:
    ...
    def from_feasibility_report(cls, feasibility_report: "FeasibilityReport") -> InfeasibilityReport:  # noqa: F821
        ...
        return cls(
            ...
            unsat_core=None,  # Pre-solve check does not compute MUS
...
class SolverConfig:
    ...
    Attributes:
        ...
        cryptic_splice_threshold: MaxEnt score threshold for cryptic splice
            ...
            Default is ``CRYPTIC_SPLICE_THRESHOLD`` from the MaxEntScan
            module (8.0 under the Markov model).
...
# ===========================================================================
# Verdict integration — bridge between solver constraints and type-system
# ===========================================================================

# Mapping from (ConstraintStrictness, satisfied: bool) → Verdict string.
# When a SOFT constraint is violated, it maps to UNCERTAIN (not FAIL)
# because soft constraints are preferences, not requirements.
SOLVER_VERDICT_MAP: dict[tuple[ConstraintStrictness, bool], str] = {
    (ConstraintStrictness.HARD, True): "PASS",       # Hard constraint satisfied
    (ConstraintStrictness.HARD, False): "FAIL",      # Hard constraint violated
    (ConstraintStrictness.SOFT, True): "PASS",        # Soft constraint satisfied
    (ConstraintStrictness.SOFT, False): "UNCERTAIN",  # Soft constraint violated → UNCERTAIN
}


def enforcement_to_verdicts(
    violations: list[ConstraintViolation],
    satisfied: list[SatisfiedConstraint],
) -> dict[str, "Verdict"]:
    """Convert solver enforcement results to type-system verdicts.

    Maps:
    - Satisfied hard constraints → PASS
    - Violated hard constraints → FAIL
    - Satisfied soft constraints → PASS
    - Violated soft constraints → UNCERTAIN (not FAIL, because soft
      constraints are preferences, not requirements)

    Args:
        violations: List of :class:`ConstraintViolation` from enforcement.
        satisfied: List of :class:`SatisfiedConstraint` that passed.

    Returns:
        Dict mapping constraint name to :class:`Verdict`.
    """
    ...
    for violation in violations:
        key = (violation.constraint_type, False)
        verdict_str = SOLVER_VERDICT_MAP.get(key, "UNCERTAIN")
        verdicts[violation.constraint_name] = Verdict(verdict_str)

    for constraint in satisfied:
        key = (constraint.strictness, True)
        verdict_str = SOLVER_VERDICT_MAP.get(key, "PASS")
        verdicts[constraint.name] = Verdict(verdict_str)
