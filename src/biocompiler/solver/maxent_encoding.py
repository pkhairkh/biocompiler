...
.. note::
   ...
   Splice scores (``score_donor`` / ``score_acceptor``) now use the
   **first-order Markov model** from Yeo & Burge 2004 rather than the
   earlier position-weight matrix (PWM).  The Markov model captures
   dinucleotide dependencies — including those that span codon
   boundaries — producing higher absolute scores (donor max ~12 bits,
   acceptor max ~14 bits) compared to the old PWM (donor max ~7.3 bits,
   acceptor max ~5–7 bits).  All thresholds and quantisation parameters
   in this module are now derived from ``CRYPTIC_SPLICE_THRESHOLD``
   (defined in ``biocompiler.sequence.maxentscan``) to stay consistent
   with the new score distribution.
...
ACCEPTOR_CONTEXT_UPSTREAM: int = 18   # bases before A position in k-mer window
ACCEPTOR_CONTEXT_DOWNSTREAM: int = 5  # exclusive end offset after A (window: [p-18, p+5))
...
# DEFAULT_QUANTIZE_THRESHOLD is derived from the authoritative
# CRYPTIC_SPLICE_THRESHOLD in maxentscan.py so that quantisation bins
# span the biologically meaningful range under the Markov model.
DEFAULT_QUANTIZE_BINS: int = 20
DEFAULT_QUANTIZE_THRESHOLD: float = CRYPTIC_SPLICE_THRESHOLD
...
def build_splice_constraint_table(
    ...
    Example
    -------
    >>> config = SolverConfig()  # default threshold derived from CRYPTIC_SPLICE_THRESHOLD
    >>> data = precompute_splice_scores("MV", config)
...
def quantize_maxent_scores(
    ...
    about -50 to ~12 for donors and ~14 for acceptors under the Markov
    model) into integer bin indices. This allows CSP solvers to reason
    about splice scores in their objective function without continuous
    variables.
    ...
    and very strong sites do not need fine-grained distinction.

    Under the Markov model the default threshold equals
    ``CRYPTIC_SPLICE_THRESHOLD`` (8.0), so bins span [0, 8.0]. Scores
    above 8.0 (strong cryptic sites) all collapse to the maximum bin,
    while scores below 0 (non-sites) collapse to bin 0.
    ...
    Args:
        ...
        threshold: The splice site threshold; scores above this are
            all mapped to the maximum bin (default ``CRYPTIC_SPLICE_THRESHOLD``).
    ...
    Example
    -------
    >>> quantize_maxent_scores([0.0, 4.0, 8.0, 12.0], n_bins=10, threshold=8.0)
    [0, 5, 10, 10]
    >>> quantize_maxent_scores([-5.0, 0.3], n_bins=20, threshold=8.0)
    [0, 0]
    """
    if n_bins < 2:
...
def encode_cross_codon_splice_context(
    ...
    for the greedy optimizer because fixing one codon alone does not
