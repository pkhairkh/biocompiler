...
- **EnforcementResult**: Detailed result of enforcement including
  violation list, satisfied constraints, soft score, and optional conflict
  resolution.
- **ConflictResolution**: Diagnosis of conflicting hard constraints with
  human-readable relaxation suggestions.
- **enforcement_to_verdicts**: Converts enforcement results (violations +
  satisfied constraints) into type-system :class:`Verdict` values.  This
  bridges the solver's binary HARD/SOFT constraint system with the
  five-valued logic of the type checker (PASS / LIKELY_PASS / UNCERTAIN /
  LIKELY_FAIL / FAIL).  Key mapping: violated soft constraints → UNCERTAIN
  (not FAIL), because soft constraints are preferences, not requirements.
...
Typical usage::
    ...
    # Convert to type-system verdicts
    from biocompiler.solver.enforcement import enforcement_to_verdicts
    verdicts = enforcement_to_verdicts(result.violations, result.satisfied)
"""
...
class EnforcementResult:
    ...
    violation breakdown, the list of satisfied constraints, the aggregate
    soft-constraint score, and an optional conflict resolution if hard
    constraints are violated.

    Attributes:
        all_hard_satisfied: True if every hard constraint passes.
        violations: List of all constraint violations (hard and soft).
        satisfied: List of all constraints that were satisfied, with their
            strictness.  Populated by :meth:`ConstraintEnforcer.enforce` so
            that :func:`enforcement_to_verdicts` can map every constraint to
            a :class:`Verdict`.
    ...
    satisfied: list[SatisfiedConstraint] = field(default_factory=list)
...
class ConstraintEnforcer:
    ...
    def enforce(self, model: CSPModel, sequence: str) -> EnforcementResult:
        ...
        # --- Soft constraints (always considered satisfied for verdict
        #     purposes — they are preferences, not requirements) ---
    ...
    def _generate_relaxations(
        ...
        for hc in violated:
            ...
            if name == "GCRangeConstraint":
                ...
                suggestions.append(
                    "Raise cryptic splice threshold (e.g. from 8.0 to 10.0) "
