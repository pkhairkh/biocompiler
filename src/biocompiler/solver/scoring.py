...
# Used by ConstraintScorer when a ConstraintSpec does not carry an explicit
    ...
    def score_solution(
        ...
        for spec in constraints:
            ...
            if priority == ConstraintPriority.CRITICAL:
                ...
                # Do not break — we want to log all violations
...
#   over-optimising for stability that does not persist.
...
class SoftConstraintScorer:
    ...
    def adjust_weights_for_organism(self, organism: str) -> None:
        ...
        10.0
        """
        from biocompiler.organisms.config import get_organism_config
    ...
    def score_sequence(self, model: CSPModel, sequence: str) -> ScoringResult:
        ...
        Raises:
            ValueError: If the sequence length does not match the model's
