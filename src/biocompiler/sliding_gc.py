...
from .types import TypeCheckResult, Verdict

# ── NUMBA fast path with fallback ─────────────────────────────────────
try:
    from .numba_kernels import fast_gc_window as _fast_gc_window_numba
    from .numba_kernels import seq_to_bytes as _seq_to_bytes
    from .numba_kernels import HAS_NUMBA as _HAS_NUMBA
except ImportError:
...
def fix_sliding_gc_violations(
    ...
    for aa in set(protein):
        ...
        from .type_system import AA_TO_CODONS as _AA_TO_CODONS
        ...
        for violation in result.violations:
            ...
            for ci in range(first_codon, last_codon + 1):
                ...
                for alt in sorted_codons.get(aa, []):
                    ...
                    cai_score = usage.get(alt, 0.0) if usage else 0.0
