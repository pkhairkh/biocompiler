...
from biocompiler.export.sbol_export import (
    SBOL3_NS,
    RDF_NS,
    PROV_NS,
    SO_NS,
...
from biocompiler.shared.exceptions import FileFormatError
...
def sbol_to_genespecs(components: list[SBOLComponent]) -> list[Any]:
    ...
    or :func:`~biocompiler.optimizer.optimize_sequence`.
    ...
    """
    # The ``biocompiler.optimizer.multigene`` module (which provided
    # ``GeneSpec``) was deleted in v0.9.1 (see CHANGELOG).  Conversion of SBOL
    # components to GeneSpec objects is therefore no longer supported.  We
    # return an empty list and emit a warning rather than crashing with
    # ``ModuleNotFoundError`` so that SBOL import / round-trip workflows that
    # do not depend on GeneSpec continue to function.  Callers that need
    # per-component coding sequences can read ``comp.sequence`` directly from
    # the returned :class:`SBOLComponent` objects (see :func:`import_sbol`).
    import warnings

    warnings.warn(
        "sbol_to_genespecs() is a no-op: the biocompiler.optimizer.multigene "
        "module (GeneSpec) was deleted in v0.9.1. Returning an empty list. "
        "Use SBOLComponent.sequence directly from import_sbol() to access "
        "the coding DNA.",
        DeprecationWarning,
        stacklevel=2,
    )
    logger.warning(
        "sbol_to_genespecs() is a no-op (multigene module deleted in "
        "v0.9.1); returning empty list for %d component(s).",
        len(components),
    )
    return []
