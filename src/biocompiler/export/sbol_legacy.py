"""
DEPRECATED — This module is superseded by ``sbol3_export.py`` and ``sbol_export.py``.

``sbol_export.py`` is the canonical SBOL3 pure-Python XML generator with SO ontology
and PROV-O provenance.  ``sbol3_export.py`` provides pure-Python RDF/XML generation
with BioPAX annotations.  New code should use one of those modules instead.

This module is retained only for backward compatibility and will be removed in
BioCompiler 2.0.
...
def export_sbol3(
    ...
    Example::
        ...
        from biocompiler.export.sbol_legacy import export_sbol3
