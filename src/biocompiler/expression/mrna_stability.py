...
Three organism models are provided:
  ...
  - *Human / mammalian*:  ARE subtypes per Chen & Shyu 1995:
      Class I (ATTTA = scattered AUUUA in AU-rich context);
      Class II (ATTTATTTATTTA = 3+ overlapping AUUUA repeats);
...
from biocompiler.shared.constants import AA_TO_CODONS, CODON_TABLE
...
_HUMAN_MOTIFS: dict[str, list[tuple[str, str, str]]] = {
    ...
    "destabilizing": [
        ...
        # Single AUUUA motif (ATTTA in DNA).  Class I AREs contain one or
        # two isolated AUUUA motifs within a U-rich region.  This pattern
        # detects the core AUUUA pentamer; the U-rich context is captured
        # separately by the Class III pattern below.
        (
            r"ATTTA",
            "ARE Class I (scattered AUUUA in AU-rich context)",
            "Shaw & Kamen 1986; Chen & Shyu 1995",
        ),
        # ---- ARE Class II: 3+ overlapping AUUUA repeats ----
        # Class II AREs contain three or more overlapping AUUUA motifs
        # (AUUUAUUUAUUUA in RNA, ATTTATTTATTTA in DNA).  These are the
        # most potent mRNA-destabilizing elements and drive rapid decay
        # via AUF1/tristetraprolin-mediated pathways.
        # Reference: Chen & Shyu 1995, Trends Biochem Sci 20:465-470.
        (
            r"ATTTATTTATTTA",
            "ARE Class II (3+ overlapping AUUUA repeats - rapid decay)",
            "Chen & Shyu 1995",
        ),
        # ---- ARE Class III: U-rich without AUUUA (3' UTR context required) ----
        # Class III AREs are U-rich regions that lack the AUUUA pentamer
        # but still promote mRNA destabilisation through binding of
        # heterogeneous nuclear ribonucleoproteins (hnRNPs).
        # IMPORTANT: Class III elements are only functional in the 3' UTR
        # context (Chen & Shyu 1995). When gene structure annotation is
        # available, only T{6,} patterns within 3' UTR boundaries should
        # be flagged. Without annotation, a heuristic (last 30% of sequence)
        # is used, but such hits are annotated with a context warning.
        # Reference: Chen & Shyu 1995; Barreau et al. 2005.
        (
            r"T{6,}",
            "ARE Class III (U-rich element without AUUUA, 3' UTR context required)",
            "Chen & Shyu 1995; Barreau et al. 2005",
        ),
...
def score_mrna_stability(
    dna: str,
    organism: str,
    utr_context: dict[str, tuple[int, int]] | None = None,
) -> MRNAStabilityScore:
    ...
    **Motif-based scoring**: modifier component
      ...
      5. **Class III ARE context**: ARE Class III (U-rich) elements are
         only flagged when they occur in the 3' UTR.  When *utr_context*
         provides 3' UTR boundaries, only matches within those boundaries
         are counted.  Without *utr_context*, a heuristic (last 30% of
         sequence) is used and hits are annotated with a context warning.
    ...
    Args:
        ...
        utr_context: Optional mapping of UTR names to ``(start, end)``
            0-based positions, e.g. ``{"3utr": (800, 1000)}``.  When
            provided, ARE Class III (T{6,}) motifs are only flagged if
            they fall within the 3' UTR boundaries.  When *None*, the
            heuristic of last 30% of the sequence is used instead.
    ...
    # Resolve 3' UTR boundaries for Class III ARE filtering
    three_utr_start: int | None = None
    three_utr_end: int | None = None
    utr_context_confirmed = False
    if utr_context and "3utr" in utr_context:
        three_utr_start, three_utr_end = utr_context["3utr"]
        utr_context_confirmed = True
    else:
        # Heuristic: last 30% of the sequence as 3' UTR proxy
        three_utr_start = int(seq_len * 0.7)
        three_utr_end = seq_len
    ...
    for effect in ("stabilizing", "destabilizing"):
        for compiled, description, source in compiled_motifs.get(effect, []):
            for m in compiled.finditer(dna):
                ...
                # Class III ARE (U-rich) 3' UTR context restriction:
                # Only flag T{6,} patterns that fall within the 3' UTR.
                # Per Chen & Shyu 1995, Class III elements are only
                # functional in the 3' UTR context.
                is_class_iii = "Class III" in description
                if is_class_iii:
                    match_end = m.end()
                    # Skip if the match does not overlap with 3' UTR
                    if match_end <= three_utr_start or pos >= three_utr_end:
                        continue
                ...
                # Annotate Class III hits when 3' UTR context is unconfirmed
                if is_class_iii and not utr_context_confirmed:
                    detail["note"] = (
                        "3' UTR context unconfirmed; flagged based on "
                        "heuristic (last 30% of sequence)"
                    )
...
def _compute_cai(dna: str, organism: str) -> float:
    ...
    This is a lightweight internal CAI computation that does not depend on
...
def predict_mrna_stability(
    ...
    penalty that downgrades by at most one level:
      ...
      - **STABLE**: CAI >= 0.8 and no ATTTA motifs (or organism
        does not check them).  The mRNA is expected to have a long
