...
from biocompiler.shared.types import Certificate, TypeCheckResult, Token, SpliceIsoform, Verdict, combined_verdict
...
from biocompiler.organisms.config import get_organism_config, is_eukaryotic_organism
from biocompiler.provenance.certificate import compute_uncertainty_summary, compute_certificate as _compute_cert_level
...
__all__ = [
    ...
    "_compute_uncertainty_from_type_results",
    "_cert_level_with_uncertainty_cap",
]
...
def _compute_uncertainty_from_type_results(
    type_results: list[TypeCheckResult],
) -> dict[str, object]:
    """Compute an uncertainty summary from TypeCheckResult objects.

    This adapts the :func:`compute_uncertainty_summary` from certificate.py
    to work with :class:`TypeCheckResult` objects (which have ``.predicate``
    and ``.verdict`` attributes) instead of :class:`PredicateResult` objects.

    Args:
        type_results: List of TypeCheckResult objects.

    Returns:
        Dict with uncertain_count, uncertain_predicates, confidence_score,
        definite_count, likely_pass_count, likely_fail_count,
        slot_uncertain_count, and total_predicates.
    """
    ...
    for r in type_results:
        ...
        if v == Verdict.PASS:
            ...
            definite += 1
            total_confidence += 0.0
        elif v == Verdict.LIKELY_PASS:
            likely_pass += 1
            total_confidence += 0.75
        elif v == Verdict.LIKELY_FAIL:
            likely_fail += 1
            ...
            # Unknown verdict — treat as uncertain
            uncertain_preds.append(r.predicate)
            total_confidence += 0.5
    ...
    return {
        ...
        "slot_uncertain_count": 0,  # Not determinable from TypeCheckResult alone
    }


def _cert_level_with_uncertainty_cap(
    uncertainty: Optional[dict[str, object]],
) -> str:
    """Compute a certificate level string with uncertainty cap explanation.

    Applies the uncertainty enforcement rules from Issue #10:
    - Any UNCERTAIN predicate caps at SILVER
    - Multiple UNCERTAIN predicates cap at BRONZE
    - LIKELY_PASS/LIKELY_FAIL do not cap the level

    Returns:
        A human-readable string like "SILVER (capped from GOLD: 1 UNCERTAIN predicate)"
        or just the level name if no cap applies.
    """
    if uncertainty is None:
        return ""

    uncertain_count = uncertainty.get("uncertain_count", 0)
    if uncertain_count == 0:
        return "GOLD"

    if uncertain_count >= 2:
        return "BRONZE (capped: ≥2 UNCERTAIN predicates prevent SILVER/GOLD)"
    else:
        return "SILVER (capped: 1 UNCERTAIN predicate prevents GOLD)"
...
def _build_html(
    ...
    if type_results:
        ...
        # Compute uncertainty summary from type-check results
        uncertainty_summary = _compute_uncertainty_from_type_results(type_results)
        # Compute certificate level with uncertainty cap explanation
        cert_level_display = _cert_level_with_uncertainty_cap(uncertainty_summary)
...
<section class="section">
    ...
    <div class="stats-grid">
        <div class="stat-card" data-tooltip="Number of predicates with UNCERTAIN verdict (insufficient evidence for definite PASS/FAIL)">
            <div class="stat-value">{uncertainty_summary['uncertain_count']}</div>
            <div class="stat-label">UNCERTAIN Predicates</div>
        </div>
        <div class="stat-card" data-tooltip="Weighted average confidence: PASS=1.0, LIKELY_PASS=0.75, UNCERTAIN=0.5, LIKELY_FAIL=0.25, FAIL=0.0">
            <div class="stat-value">{uncertainty_summary['confidence_score']:.3f}</div>
            <div class="stat-label">Confidence Score</div>
        </div>
        <div class="stat-card" data-tooltip="Predicates with definite verdicts (PASS or FAIL)">
            <div class="stat-value">{uncertainty_summary['definite_count']}</div>
            <div class="stat-label">Definite Verdicts</div>
        </div>
        <div class="stat-card" data-tooltip="Certificate level after applying uncertainty cap rules">
            <div class="stat-value">{html.escape(cert_level_display)}</div>
            <div class="stat-label">Certificate Level</div>
        </div>
    </div>
    {f'''<h3>UNCERTAIN Predicates</h3>
    <table>
        <thead>
            <tr><th>#</th><th>Predicate</th><th>Reason</th></tr>
        </thead>
        <tbody>
            {"".join(f"""<tr class="uncertain"><td>{i + 1}</td><td><code>{html.escape(p)}</code></td><td>Insufficient evidence for definite verdict</td></tr>""" for i, p in enumerate(uncertainty_summary["uncertain_predicates"])) if uncertainty_summary["uncertain_predicates"] else '<tr><td colspan="3">No uncertain predicates</td></tr>'}
        </tbody>
    </table>''' if uncertainty_summary.get("uncertain_predicates") else '<p style="color:var(--text-secondary);font-size:0.9rem;">No predicates have UNCERTAIN verdicts.</p>'}
    <div style="margin-top:1rem;padding:0.75rem;background:var(--surface);border-radius:6px;font-size:0.85rem;color:var(--text-secondary);">
        <strong>Uncertainty Cap Rules:</strong>
        If any predicate is UNCERTAIN, the certificate level cannot exceed SILVER.
        If two or more predicates are UNCERTAIN, the level is capped at BRONZE.
        LIKELY_PASS and LIKELY_FAIL verdicts do not cap the level — they express calibrated uncertainty.
    </div>
</section>''' if uncertainty_summary is not None else ''}
