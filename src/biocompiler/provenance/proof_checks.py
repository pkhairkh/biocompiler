...
Usage:
    from biocompiler.provenance.proof_checks import assert_conservative_safe, assert_verified_evidence
...
from biocompiler.shared.types import Verdict, SLOTMode, five_valued_and, combined_verdict
from biocompiler.shared.five_valued_logic import (
    FiveValuedVerdict,
    certificate_level_with_uncertainty,
)
from ..shared.types import _VERDICT_ORDER
...
def assert_and_pass_iff(verdicts: list[Verdict], combined: Verdict) -> None:
    ...
    Raises:
        AssertionError: If combined PASS does not imply all individual PASS.
    """
...
def assert_no_slot_in_pass_list(
    ...
    Raises:
        AssertionError: If SLOT predicate UNCERTAIN verdicts do not prevent PASS.
    """
    if combined == Verdict.PASS:
...
def assert_synonymous_preserves_translation(original_aa: str, new_codon: str) -> None:
    ...
    Raises:
        AssertionError: If the new codon does not encode the same amino acid.
    """
...
# ═══════════════════════════════════════════════════════════════════════════
# 8. Uncertainty capping soundness (UncertaintyCapping.lean)
# ═══════════════════════════════════════════════════════════════════════════

def check_uncertainty_capping_sound(results: list) -> List[str]:
    """Verify that UNCERTAIN predicates correctly cap the certificate.

    Checks:
    1. Any UNCERTAIN verdict → certificate cannot be GOLD
    2. Multiple UNCERTAIN → certificate cannot be SILVER
    3. LIKELY_PASS/LIKELY_FAIL do NOT cap

    Returns:
        List of error messages. Empty means all checks pass.
    """
    errors: List[str] = []

    # Convert results to FiveValuedVerdicts for the certificate-level function
    verdicts: list[FiveValuedVerdict] = []
    for r in results:
        v = getattr(r, "verdict", None)
        if v is None:
            errors.append(
                f"Result {r!r} has no 'verdict' attribute; cannot check uncertainty capping"
            )
            continue
        # Convert Verdict (shared.types) to FiveValuedVerdict if needed
        if isinstance(v, FiveValuedVerdict):
            verdicts.append(v)
        else:
            try:
                verdicts.append(FiveValuedVerdict(v.value))
            except (ValueError, AttributeError):
                errors.append(
                    f"Result {r!r} has unrecognized verdict {v!r}; cannot check uncertainty capping"
                )
                continue
    ...
    cert_level = certificate_level_with_uncertainty(verdicts)

    # Check 1: Any UNCERTAIN → certificate cannot be GOLD
    has_uncertain = any(v == FiveValuedVerdict.UNCERTAIN for v in verdicts)
    if has_uncertain and cert_level == "GOLD":
        errors.append(
            "PROOF VIOLATION: uncertainty_capping_sound — certificate is GOLD "
            "but at least one verdict is UNCERTAIN. UNCERTAIN verdicts must cap "
            "the certificate to at most SILVER."
        )

    # Check 2: Multiple UNCERTAIN → certificate cannot be SILVER
    uncertain_count = sum(1 for v in verdicts if v == FiveValuedVerdict.UNCERTAIN)
    if uncertain_count >= 2 and cert_level == "SILVER":
        errors.append(
            f"PROOF VIOLATION: uncertainty_capping_sound — certificate is SILVER "
            f"but {uncertain_count} verdicts are UNCERTAIN. Two or more UNCERTAIN "
            f"verdicts must cap the certificate to BRONZE."
        )

    # Check 3: LIKELY_PASS/LIKELY_FAIL do NOT cap
    # If the only non-definite verdicts are LIKELY_*, certificate should be GOLD
    has_uncertain_or_fail = any(
        v in (FiveValuedVerdict.UNCERTAIN, FiveValuedVerdict.FAIL)
        for v in verdicts
    )
    has_likely = any(
        v in (FiveValuedVerdict.LIKELY_PASS, FiveValuedVerdict.LIKELY_FAIL)
        for v in verdicts
    )
    if has_likely and not has_uncertain_or_fail and cert_level != "GOLD":
        errors.append(
            "PROOF VIOLATION: uncertainty_capping_sound — certificate is "
            f"{cert_level} but only LIKELY_PASS/LIKELY_FAIL verdicts are present "
            "(no UNCERTAIN or FAIL). LIKELY_PASS/LIKELY_FAIL should not cap "
            "the certificate; it should remain GOLD."
        )
...
# This ensures the codon table has not been corrupted
try:
    assert_valine_gt_invariant()
    logger.debug("Lean4 proof cross-check: all_valine_codons_have_gt: verified")
