...
**Lean4 formal model (SLOTVerification.lean):**
  ...
  (MOVE-PRED: was 22; NoRibosomalFrameshift, NoMiRNABindingSite, NoLongHydrophobicStretch
  moved to CORE as deterministic predicates)
...
**Why this gap is deliberate:**
  ...
  always says "UNCERTAIN" for 19 of 36 predicates is formally impeccable but
...
# NoCrypticSplice, NoCrypticPromoter, CodonOptimality.
#
# NOTE (C19 fix, FIX-W3): NoCrypticSplice, NoCrypticPromoter, and
# CodonOptimality were previously misclassified as SLOT in this Python
# set but as CORE in the Lean4 formal model (`TypeSystem.lean :: isSLOT`).
# They are now aligned with Lean4 (CORE). The dedicated verification
# functions (verify_no_cryptic_splice, verify_no_cryptic_promoter,
# verify_codon_optimality) remain available for direct callers, but
...
SLOT_PREDICATES = {
    ...
    # MOVE-PRED: NoRibosomalFrameshift, NoMiRNABindingSite, and
    # NoLongHydrophobicStretch moved from SLOT to CORE (deterministic,
    # no external oracle needed). They are now aligned with the Lean4
    # formal model (isSLOT = false). See worklog MOVE-PRED.
}
...
def _check_foldx_available() -> bool:
    ...
    try:
        from biocompiler.engines.foldx import is_foldx_available
...
def _check_mhc_available() -> bool:
    ...
    try:
        from biocompiler.immunogenicity.core import predict_mhc_i_binding
...
def verify_solubility_predicate(
    ...
    try:
        from biocompiler.engines.camsol import compute_intrinsic_solubility
...
_SOLUBILITY_PREDICATES = {"SolubleExpression", "NoAggregationProneRegion", "ChargeComposition"}  # MOVE-PRED: NoLongHydrophobicStretch moved to CORE
...
def _recheck_predicate(predicate_name: str, verdict: Verdict, **kwargs) -> SoundnessResult:
    ...
    being vacuously true because PASS never happens (as in the Lean4 conservative
    ...
    # For non-definite verdicts, soundness is vacuously true
    # (UNCERTAIN/LIKELY_PASS/LIKELY_FAIL do not make definite claims)
    if verdict not in (Verdict.PASS, Verdict.FAIL):
        result.details = f"Verdict {verdict.value} is non-definite; soundness is vacuously true"
        result.sound = True
        return result
    ...
    try:
        ...
        if verdict == Verdict.PASS:
            ...
            if recheck_verdict in (Verdict.PASS, Verdict.LIKELY_PASS):
                ...
                # Recheck could not verify (tool unavailable) — NOT vacuously sound.
            ...
            if recheck_verdict in (Verdict.FAIL, Verdict.LIKELY_FAIL):
                ...
                # Cannot confirm the violation — not vacuously sound.
        ...
        # Unknown predicate name — cannot recheck
        if verdict == Verdict.PASS:
...
def verify_soundness(
    ...
    **kwargs: Any,
    ...
    Example::
        ...
        from biocompiler.provenance.slot_verification import verify_soundness, verify_codon_optimality
        from biocompiler.shared.types import Verdict, SLOTMode
