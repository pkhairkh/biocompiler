...
def get_database(allele: str) -> Optional[PrecomputedAlleleDatabase]:
    ...
    .. deprecated:: C20 remediation (prior fix)
        ...
        to the current runtime PSSM scoring.  This function now always
        returns ``None``.  Callers (e.g.
        :meth:`biocompiler.immunogenicity.mhcflurry_adapter.MHCflurryClient._lookup_precomputed`)
        fall through to the runtime PSSM fallback, which uses the SAME
        PSSM matrices with current scoring and an honest
        ``confidence=0.5``.  For an in-memory database populated with
        PSSM-scored records, use :func:`generate_fallback_database`;
        for the small curated set of published IC50 values, see
        :data:`biocompiler.mhc_binding_db.schema.PRECOMPUTED_BINDERS`.
    ...
    PrecomputedAlleleDatabase or None
        Always ``None`` since the precomputed subpackage was removed.
    """
    return None
