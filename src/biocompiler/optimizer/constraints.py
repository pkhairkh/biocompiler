...
def _remove_site_multicodon(
    ...
    single-codon swaps fail because changing either codon alone does not eliminate the site.
...
def _back_translate_protein(
    ...
    # sequences where it is fast and produces a better starting point.
    if strategy == "dp":
        use_dp = True
...
def _is_unavoidable_gt(seq: str, pos: int) -> bool:
    ...
    2. It's a cross-codon GT where the next codon's AA has no synonymous
       codon that does not start with T (e.g., Trp=TGG, Cys=TGT/TGC, Tyr=TAT/TAC)
    3. It's a cross-codon GT where the previous codon's AA has no synonymous
       codon that does not end with G
    """
    codon_start = (pos // 3) * 3
    ...
    if pos + 1 < next_codon_start:
        ...
        for alt in AA_TO_CODONS.get(aa, []):
            if "GT" not in alt:
                return False  # There is an alternative without GT
    ...
    if next_cs + 3 > len(seq):
        return True  # Cannot check, assume unavoidable
    ...
    # GT is unavoidable only if BOTH sides cannot avoid it
