...
Usage::
    ...
    from biocompiler.optimizer.objectives import cai_gc_balanced_objective
...
# The ``codon_harmonization`` module was deleted in v0.9.1 (see CHANGELOG).
# The harmonization objective is therefore unavailable.  We keep the symbol
# resolvable (and the registry entry intact for backwards-compat) but guard
# the import so that merely importing this module -- or resolving the
# "harmonization" objective name -- does not crash.  Calling the objective
# raises a clear :class:`RuntimeError` (see ``harmonization_objective``).
try:
    from .codon_harmonization import compute_harmonization_score  # type: ignore[assignment]
except ImportError:
    compute_harmonization_score = None  # type: ignore[assignment]
...
def codon_pair_objective(dna: str, protein: str, organism: str) -> float:
    ...
    from biocompiler.expression.codon_pair_scoring import compute_cpb, get_codon_pair_data
...
def harmonization_objective(
    ...
    *Nucleic Acids Research*, 45(6), 3342–3356. PMID: 28250820.
    ...
    if not dna or len(dna) < 3:
        return 0.0
    if compute_harmonization_score is None:
        raise RuntimeError(
            "harmonization objective removed -- "
            "codon_harmonization module deleted in v0.9.1"
        )
    try:
...
def tai_objective(dna: str, protein: str, organism: str) -> float:
    ...
    tAI measures codon usage bias based on tRNA gene copy numbers and
    wobble pairing efficiency.  It predicts how well a gene's codon
    usage matches the host organism's tRNA pool, making it especially
    useful for heterologous expression optimization.

    Unlike CAI which uses codon frequencies from highly expressed
    genes, tAI uses the actual tRNA gene copy numbers as a proxy for
    tRNA abundance.  This makes tAI more biologically grounded and
    better at predicting expression levels for recombinant proteins.
    ...
    Returns:
        tAI value in [0.0, 1.0]. Returns 0.0 for empty/invalid sequences
        or organisms without tRNA data.
    ...
    *Nucleic Acids Research*, 32(17), 5036-5044.
    ...
    try:
        return compute_tai(dna, organism=organism)
