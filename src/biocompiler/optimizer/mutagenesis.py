"""
BioCompiler Mutagenesis Engine v1.0.0
=======================================
...
def propose_mutagenesis(
    ...
    for pos in constraint_positions:
        ...
        if best is None:
            ...
            # Check if a better BLOSUM score was available but did not resolve
            chose_poorly = False
...
def _resolves_constraints(
    ...
    for cname in constraint_names:
        if cname == "GT":
            ...
            # Fix: previously this branch used
            # `if site in modified:` (a GLOBAL scan of the entire
            # modified sequence), whereas the GT/CG branches above
            # use a local 5-nt window around codon_start.  The global
            # check caused false negatives: a pre-existing RS hit
            # elsewhere in the sequence would falsely mark this codon
            # substitution as "does not resolve the RS constraint",
            # even though the substitution itself didn't touch that
            # distant RS.  Use a local window of size
            # `_CODON_LENGTH + 2 * (len(site) - 1)` centred on the
            # codon being modified — wide enough to catch any RS that
            # overlaps the substituted codon, narrow enough to ignore
            # unrelated RS occurrences elsewhere.
            site = cname[3:]
            if not site:
                continue
            site_len = len(site)
            region_start = max(0, codon_start - (site_len - 1))
            region_end = min(
                len(modified), codon_start + _CODON_LENGTH + (site_len - 1)
            )
            if site in modified[region_start:region_end]:
                return False
