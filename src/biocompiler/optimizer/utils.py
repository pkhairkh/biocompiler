...
from biocompiler.provenance.decision_provenance import OptimizationDecisionTrail
...
MAX_ATTTA_MOTIF_ITERATIONS: int = 500  # increased from 100 — some sequences need more passes
MAX_T_RUN_ITERATIONS: int = 500  # increased from 100
...
# ────────────────────────────────────────────────────────────
# Optimizer timeout / resource caps (Task TIGHTEN-2)
# ────────────────────────────────────────────────────────────
# These prevent the optimizer from hanging or OOMing on pathological
# inputs (e.g. very long proteins that trigger deep backtracking in
# the greedy constraint-fixing loops).  The signal.alarm timeout in
# ``optimize_sequence()`` is the PRIMARY safety net; the iteration
# caps in greedy.py are a SECONDARY defense that fails faster and
# more gracefully.

# Wall-clock timeout (seconds) for the entire optimize_sequence() call.
# Default 30s — generous enough for normal genes (<1s typically) but
# short enough to prevent a hung process from blocking the test suite
# or API for minutes.
OPTIMIZER_TIMEOUT_SECONDS: int = 30

# Hard cap on TOTAL iterations across all constraint-fixing loops in
# the greedy optimizer.  Each inner loop already has its own per-step
# cap (MAX_RESTRICTION_SITE_ITERATIONS etc.), but a pathological input
# can still blow up the cumulative count.  This global budget is a
# backstop: if the sum of all inner-loop iterations exceeds this,
# the greedy optimizer bails out early and returns what it has.
OPTIMIZER_MAX_ITERATIONS: int = 10000

# Soft memory limit (MB) for documentation / future resource_tracer
# integration.  Not enforced directly today (signal.alarm covers the
# time dimension, which is the observed failure mode), but recorded
# here so callers and future agents know the intended envelope.
OPTIMIZER_MAX_MEMORY_MB: int = 500

# Per-constraint inner-loop cap.  All existing MAX_*_ITERATIONS
# constants are already below this; it is exported so that new
# constraint-fixing loops have a single obvious ceiling to reference.
    ...
    def check_convergence(self) -> str | None:
        ...
        if len(window) >= 3:
            ...
            if has_increase and has_decrease:
                # Additional check: the objective has not improved overall in the window
...
class OptimizationResult:
    ...
    # Number of predicates with UNCERTAIN verdict (populated by optimize_sequence)
    uncertain_predicate_count: int = 0
    # IR objects (the compiler pipeline) — populated by optimize_sequence()
    # when it runs the optimized DNA through the BioCompiler IR (L0→L1→L2→L3)
    # to verify protein preservation and produce codegen outputs.
    ir_l0: Optional[object] = None  # IR_L0_GenomicDNA
    ir_l3: Optional[object] = None  # IR_L3_Polypeptide
    ir_verified: bool = False  # True if IR translation matches input protein
    genbank: Optional[str] = None  # GenBank format output
    fasta: Optional[str] = None  # FASTA format output
    sbol3: Optional[str] = None  # SBOL3 format output
    # True if the optimizer hit its time/iteration budget and returned a
    # PARTIAL result rather than raising.  When True, the sequence is the
    # best partial output obtained before the timeout; ``convergence_status``
    # is set to "timeout".  (Task TIGHTEN-2)
    timed_out: bool = False

    @property
    def optimized_sequence(self) -> str:
        """Alias for ``sequence`` — the optimized DNA output.

        Provided so callers can use the more descriptive name
        ``optimized_sequence`` interchangeably with ``sequence``.
        """
...
class _OptConfig:
    ...
    Extracts the relevant optimizer state so sub-module functions do not
