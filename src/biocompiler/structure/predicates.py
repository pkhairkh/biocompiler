...
def evaluate_structure_confidence(
    ...
    # Try ESMFold heuristic fallback — if it returns method ==
    # "heuristic_fallback", return UNCERTAIN per Issue #17.
    try:
        from ..engines.esmfold_fallback import predict_structure_heuristic
        heuristic_result = predict_structure_heuristic(protein)
        if isinstance(heuristic_result, dict) and heuristic_result.get("method") == "heuristic_fallback":
            return TypeCheckResult(
                predicate="StructureConfidence",
                verdict=Verdict.UNCERTAIN,
                derivation=[
                    {"step": "esmfold_heuristic_fallback", "mean_plddt": heuristic_result.get("mean_plddt", 0.0)},
                    {"step": "method", "value": "heuristic_fallback"},
                    {"step": "note", "value": "Heuristic fallback cannot produce confident structure prediction"},
                ],
                violation=(
                    f"Heuristic structure estimate: mean pLDDT={heuristic_result.get('mean_plddt', 0.0):.1f} "
                    f"[UNCERTAIN: heuristic_fallback, not ESMFold]"
                ),
                knowledge_gap=(
                    "Structure prediction from heuristic fallback (not ESMFold); "
                    "confidence too low for definite verdict."
                ),
            )
    except (ImportError, AttributeError, RuntimeError):
        pass  # Fall through to the sequence-based heuristic below
...
def _sequence_based_topology_check(
    ...
    # does not (LIKELY_FAIL). UNCERTAIN is reserved for truly ambiguous
...
def evaluate_no_unexpected_interaction(
    ...
    # If the "interaction" involves >30% of surface residues, it is likely
    ...
    # Only flag as LIKELY_FAIL if there is strong evidence of dimerization
    n_indicators = len(indicators)
    if n_indicators == 0:
        if pdb_string is not None:
            verdict = Verdict.PASS
        else:
            # UNCERTAIN: no indicators found, but without structure data
            # the checks were much less thorough; PASS would overstate
            # confidence in the sequence-only heuristic analysis.
            verdict = Verdict.UNCERTAIN
    elif n_indicators == 1:
        if pdb_string is not None:
            verdict = Verdict.LIKELY_PASS
        else:
            # UNCERTAIN: 1 indicator from sequence heuristics is a weaker
            # signal than 1 indicator from structural analysis.
            verdict = Verdict.UNCERTAIN
    elif n_indicators == 2:
...
def _check_sequence_hydrophobicity(
    ...
    we cannot determine burial without structure).
    """
    protein = protein.upper()
    n = len(protein)
    if n == 0:
