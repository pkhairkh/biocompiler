...
from biocompiler.engines.base import EngineTimer
...
class PredicateResultDict(TypedDict, total=False):
    """Type for normalized predicate result dicts."""
...
class StructureQualityDict(TypedDict, total=False):
    """Type for structure quality result dicts."""

    mean_plddt: float
    per_residue_plddt: list[float]
    method: str
    verdict: str
    predicate_result: PredicateResultDict


class StabilityDict(TypedDict, total=False):
    """Type for stability result dicts."""
...
class SolubilityDict(TypedDict, total=False):
    """Type for solubility result dicts."""
...
class ImmunogenicityDict(TypedDict, total=False):
    """Type for immunogenicity result dicts."""
...
class MhcBindingDict(TypedDict, total=False):
    """Type for MHC binding result dicts."""

    strong_binders: list[dict]
    weak_binders: list[dict]
    verdict: str
    predicate_result: PredicateResultDict


class EpitopeDict(TypedDict, total=False):
    """Type for epitope prediction result dicts."""
...
class ReportDict(TypedDict, total=False):
    """Type for the JSON-serializable report dict."""

    protein: str
    organism: str
    structure_quality: StructureQualityDict | None
    stability: StabilityDict | None
    solubility: SolubilityDict | None
    immunogenicity: ImmunogenicityDict | None
    mhc_binding: MhcBindingDict | None
    epitope: EpitopeDict | None
    predicate_results: list[PredicateResultDict]
    overall_verdict: str
...
def _to_dict(obj: object) -> object:
    ...
    if isinstance(obj, dict):
        return obj
    try:
        # Check if it is a dataclass instance
...
class ProteinAssessmentReport:
    ...
    protein: str
    organism: str
    structure_quality: StructureQualityDict | None = None
    stability: StabilityDict | None = None
    solubility: SolubilityDict | None = None
    immunogenicity: ImmunogenicityDict | None = None
    mhc_binding: MhcBindingDict | None = None
    epitope: EpitopeDict | None = None
    predicate_results: list[PredicateResultDict] = field(default_factory=list)
...
def _extract_predicate(result_dict: object) -> PredicateResultDict | None:
    """Extract and normalize the predicate_result from a dict, or return None."""
    if not isinstance(result_dict, dict):
        return None
    pred = result_dict.get("predicate_result")
    if pred is not None:
        return _normalize_predicate(pred)
    return None


def _run_structure_quality_step(
    protein: str, pdb_string: str | None
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run structure quality analysis step.

    Returns (result_dict, predicate_or_None, error_or_None).
    """
    try:
        from .quality import compute_structure_quality
        sq_result = compute_structure_quality(protein, pdb_string=pdb_string)
        result_dict = _to_dict(sq_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _run_stability_step(
    protein: str, pdb_string: str | None
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run stability (FoldX) analysis step.
    ...
    try:
        ...
        result_dict = _to_dict(stab_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _run_solubility_step(
    protein: str,
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run solubility (CamSol) analysis step.
    ...
    try:
        ...
        result_dict = _to_dict(sol_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _run_immunogenicity_step(
    protein: str, organism: str
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run immunogenicity analysis step.
    ...
    try:
        ...
        result_dict = _to_dict(imm_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _run_mhc_binding_step(
    protein: str,
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run MHC binding prediction step.

    Returns (result_dict, predicate_or_None, error_or_None).
    """
    try:
        from biocompiler.immunogenicity.core import predict_all as predict_mhc_all
        mhc_result = predict_mhc_all(protein)
        result_dict = _to_dict(mhc_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _run_epitope_step(
    protein: str,
) -> tuple[object | None, PredicateResultDict | None, str | None]:
    """Run epitope prediction step.

    Returns (result_dict, predicate_or_None, error_or_None).
    """
    try:
        from biocompiler.immunogenicity.core import predict_epitopes
        epi_result = predict_epitopes(protein)
        result_dict = _to_dict(epi_result)
        return result_dict, _extract_predicate(result_dict), None
    except ImportError:
...
def _apply_step_result(
    ...
    attr_name: str,
    result_dict: object | None,
    predicate: PredicateResultDict | None,
    error: str | None,
    predicate_results: list[PredicateResultDict],
) -> None:
    """Apply a single analysis step result to the report."""
    if result_dict is not None:
        setattr(report, attr_name, result_dict)
...
def assess_protein(
    ...
    with EngineTimer() as timer:
        if run_structure:
            r, p, e = _run_structure_quality_step(protein, pdb_string)
            _apply_step_result(report, "structure_quality", r, p, e, predicate_results)

        if run_stability:
            r, p, e = _run_stability_step(protein, pdb_string)
            _apply_step_result(report, "stability", r, p, e, predicate_results)

        if run_solubility:
            r, p, e = _run_solubility_step(protein)
            _apply_step_result(report, "solubility", r, p, e, predicate_results)

        if run_immunogenicity:
            r, p, e = _run_immunogenicity_step(protein, organism)
            _apply_step_result(report, "immunogenicity", r, p, e, predicate_results)

            r, p, e = _run_mhc_binding_step(protein)
            _apply_step_result(report, "mhc_binding", r, p, e, predicate_results)

            r, p, e = _run_epitope_step(protein)
            _apply_step_result(report, "epitope", r, p, e, predicate_results)
...
def _normalize_predicate(pred: dict | TypeCheckResult) -> PredicateResultDict:
...
def compute_overall_verdict(predicate_results: list[PredicateResultDict]) -> str:
...
def _recommend_structure_quality(sq: StructureQualityDict | None) -> list[str]:
    """Generate recommendations for structure quality results."""
    recs: list[str] = []
...
def _recommend_stability(stab: StabilityDict | None) -> list[str]:
    """Generate recommendations for stability results."""
    recs: list[str] = []
...
def _recommend_solubility(sol: SolubilityDict | None) -> list[str]:
    """Generate recommendations for solubility results."""
    recs: list[str] = []
...
def _recommend_immunogenicity(
    imm: ImmunogenicityDict | None,
    mhc: MhcBindingDict | None,
    epi: EpitopeDict | None,
) -> list[str]:
    """Generate recommendations for immunogenicity, MHC binding, and epitope results."""
    recs: list[str] = []
...
def generate_recommendations(report: ProteinAssessmentReport) -> list[str]:
    ...
    recs.extend(_recommend_structure_quality(report.structure_quality))
    recs.extend(_recommend_stability(report.stability))
    recs.extend(_recommend_solubility(report.solubility))
    recs.extend(_recommend_immunogenicity(
        report.immunogenicity, report.mhc_binding, report.epitope
    ))
...
def _format_structure_text(
    sq: StructureQualityDict | None, lines: list[str]
) -> None:
    """Append structure quality text section to lines."""
    lines.append("-" * 60)
...
def _format_stability_text(
    stab: StabilityDict | None, lines: list[str]
) -> None:
    """Append stability text section to lines."""
    lines.append("")
    lines.append("-" * 60)
...
def _format_solubility_text(
    sol: SolubilityDict | None, lines: list[str]
) -> None:
    """Append solubility text section to lines."""
    lines.append("")
    lines.append("-" * 60)
...
def _format_immuno_text(
    imm: ImmunogenicityDict | None,
    mhc: MhcBindingDict | None,
    epi: EpitopeDict | None,
    lines: list[str],
) -> None:
    """Append immunogenicity, MHC binding, and epitope text sections to lines."""
    lines.append("")
    lines.append("-" * 60)
...
def _format_predicates_text(
    predicates: list[PredicateResultDict], lines: list[str]
) -> None:
    """Append predicate results text section to lines."""
    lines.append("")
    lines.append("-" * 60)
...
def _format_recommendations_text(recs: list[str], lines: list[str]) -> None:
    """Append wrapped recommendation text section to lines."""
    if not recs:
        return
    lines.append("")
    lines.append("-" * 60)
...
def format_assessment_text(report: ProteinAssessmentReport) -> str:
    ...
    _format_structure_text(report.structure_quality, lines)
    _format_stability_text(report.stability, lines)
    _format_solubility_text(report.solubility, lines)
    _format_immuno_text(
        report.immunogenicity, report.mhc_binding, report.epitope, lines
    )
    _format_predicates_text(report.predicate_results, lines)
    ...
    _format_recommendations_text(report.recommendations, lines)
...
def _report_to_dict(report: ProteinAssessmentReport) -> ReportDict:
...
def _html_header_block(
    report: ProteinAssessmentReport,
    verdict_css: str,
    verdict_symbol: str,
) -> str:
    """Build the HTML header block with protein info and verdict badge."""
...
def _html_summary_block(report: ProteinAssessmentReport) -> str:
    """Build the HTML summary statistics section."""
    pass_count = sum(1 for p in report.predicate_results if p.get('verdict') == 'PASS')
    return f"""
...
def _html_footer_and_script(version: str) -> str:
    """Build the HTML footer and JavaScript section."""
    return f"""
<div class="footer">
    Generated by BioCompiler v{version} &mdash; Protein Structure Assessment
</div>
...
def format_assessment_html(report: ProteinAssessmentReport) -> str:
    ...
    header = _html_header_block(report, verdict_css, verdict_symbol)
    summary = _html_summary_block(report)
    footer = _html_footer_and_script(__version__)
...
def _build_solubility_gauge(score: float, score_display: str) -> str:
    """Build the SVG gauge HTML for a solubility score."""
...
def _html_solubility_section(report: ProteinAssessmentReport) -> str:
    ...
    if isinstance(score, (int, float)):
        gauge_html = _build_solubility_gauge(score, score_display)
...
def _build_mhc_binding_table(strong: list[dict]) -> str:
    """Build an HTML table for MHC binding results."""
    rows = ""
...
def _build_epitope_table(epitopes: list[dict]) -> str:
    """Build an HTML table for epitope prediction results."""
    epi_rows = ""
...
def _html_immunogenicity_section(report: ProteinAssessmentReport) -> str:
    ...
    if mhc and isinstance(mhc, dict):
        ...
        if isinstance(strong, list) and strong:
            parts.append(_build_mhc_binding_table(strong))
    ...
    if epi and isinstance(epi, dict):
        ...
        if isinstance(epitopes, list) and epitopes:
            parts.append(_build_epitope_table(epitopes))
...
def _build_plddt_bars(
    ...
    """Build the SVG rect elements for pLDDT bar chart."""
    bars = ""
...
def _build_plddt_x_ticks(
    n: int, bar_width: float, margin_left: float, height: float
) -> str:
    """Build X-axis tick labels for pLDDT chart."""
    x_ticks = ""
    step = max(1, n // 10)
...
def plot_plddt_bar_svg(
    ...
    bars = _build_plddt_bars(plddt_scores, bar_width, margin_left, margin_top, plot_h)
    ...
    # X-axis tick labels
    x_ticks = _build_plddt_x_ticks(n, bar_width, margin_left, height)

    return _assemble_plddt_svg(
        width, height, margin_left, margin_right, margin_top, margin_top,
        plot_h, y90, y70, y50, bars, x_ticks, title,
    )


def _assemble_plddt_svg(
    width: int, height: int, margin_left: int, margin_right: int,
    margin_top: int, margin_bottom: int, plot_h: float,
    y90: float, y70: float, y50: float,
    bars: str, x_ticks: str, title: str,
) -> str:
    """Assemble the final SVG string for the pLDDT bar chart."""
...
def _build_solubility_area_paths(
    per_residue_scores: list[float],
    map_x: object,
    map_y: object,
    zero_y: float,
) -> tuple[str, str, str]:
    """Build the SVG area fill paths and line path for solubility profile.

    Returns (blue_area_path, red_area_path, line_path).
    """
    blue_points = []
    red_points = []
    line_points = []
    for i, s in enumerate(per_residue_scores):
        x = map_x(i)
        y_blue = map_y(max(s, 0))
        y_red = map_y(min(s, 0))
        y_line = map_y(s)
        blue_points.append(f"{x:.1f},{y_blue:.1f}")
        red_points.append(f"{x:.1f},{y_red:.1f}")
    ...
    return blue_area, red_area, line_path
...
def plot_solubility_profile_svg(
    ...
    blue_area, red_area, line_path = _build_solubility_area_paths(
        per_residue_scores, map_x, map_y, zero_y
    )
    ...
    return _assemble_solubility_svg(
        width, height, margin_left, margin_right, margin_top, plot_h,
        zero_y, y_max, y_min, blue_area, red_area, line_path, x_ticks, title,
    )


def _assemble_solubility_svg(
    width: int, height: int, margin_left: int, margin_right: int,
    margin_top: int, plot_h: float, zero_y: float,
    y_max: float, y_min: float,
    blue_area: str, red_area: str, line_path: str,
    x_ticks: str, title: str,
) -> str:
    """Assemble the final SVG string for the solubility profile chart."""
    return f"""<svg width="{width}" height="{height}" xmlns="http://www.w3.org/2000/svg">
    <rect width="100%" height="100%" fill="white"/>
