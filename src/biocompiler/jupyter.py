...
def display_sequence(
    ...
    from .scanner import gc_content as _gc_content, validate_dna_sequence
...
def display_optimization_result(result) -> None:
    ...
    if result.fallback_used:
        ...
        <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:4px;padding:8px 12px;margin-top:8px;font-family:sans-serif;font-size:13px;">
            ⚠️ Greedy fallback optimizer was used (z3 not available or sequence too long)
        </div>
        """
...
def interactive_optimize(
        ...
        with output:
            ...
            try:
                ...
                if result.satisfied_predicates:
                    print(f"\n  ✓ {', '.join(result.satisfied_predicates)}")
                if result.failed_predicates:
                    print(f"\n  ✗ {', '.join(result.failed_predicates)}")

                if result.fallback_used:
                    print("\n  ⚠️ Greedy fallback was used")
    ...
    controls = widgets.VBox([
        widgets.HTML("<b>🔧 Optimization Parameters</b>"),
        gc_lo_slider,
        gc_hi_slider,
        cai_slider,
        optimize_button,
