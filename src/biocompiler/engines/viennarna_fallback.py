...
try:
    from biocompiler.engines.viennarna import MFEResult, StemLoop, AccessibilityResult
except ImportError:
...
def compute_approx_dg(
    ...
    **NUMBA acceleration**: when ``biocompiler.optimizer.numba_kernels``
    is importable and ``USE_NUMBA=True`` (default), the inner O(n²)
    scan is dispatched to the JIT-compiled
    ``_numba_compute_approx_dg`` kernel.  This eliminates 125K+ pure-
    Python helper calls (``_can_pair``, ``_pair_energy``,
    ``_loop_penalty``) and yields ~25× speedup on 1.4 kb sequences.
    The pure-Python implementation is retained as a fallback for
    environments without NUMBA and as the source of truth for tests.
    ...
    """
    # ── NUMBA fast-path ────────────────────────────────────────────
    # The numba kernel produces byte-identical results to the pure-
    # Python implementation below (verified in
    # tests/test_numba_kernels.py).  We dispatch to it whenever
    # available because the pure-Python inner loop accounts for ~20%
    # of total optimization time on 466-aa proteins.
    try:
        from biocompiler.optimizer.numba_kernels import (
            USE_NUMBA as _USE_NUMBA,
            _numba_compute_approx_dg as _numba_dg,
            seq_to_bytes as _seq_to_bytes,
        )
        if _USE_NUMBA:
            return _numba_dg(_seq_to_bytes(dna_sequence.upper()))
    except Exception:
        logger.debug("NUMBA fast-path unavailable; falling back to pure Python",
                     exc_info=True)
