"""
DEPRECATED — This module is superseded by ``blast_screening.py``.

The ``BlastScreener`` class in ``blast_screening.py`` provides equivalent
functionality with a cleaner API and better fallback behavior.  New code
should use ``from biocompiler.biosecurity.blast_screening import BlastScreener``.
...
def check_biosecurity_blast(
    ...
    """
    from . import get_biosecurity_mode
