...
Migrated from species.py (v1.0.0):
...
- CAI table unification: SPECIES['ecoli']['cai_weights'] now uses
  ...
  agree on optimal codons. CAI values will differ from legacy versions (now correct).
...
1.0.0 changes (Task 27 — organisms cleanup):
...
from .escherichia import E_COLI_CODON_USAGE as E_COLI_CODON_USAGE
from .escherichia import E_COLI_CODON_ADAPTIVENESS as E_COLI_CODON_ADAPTIVENESS
from .escherichia import E_COLI_PREFERRED_CODONS as E_COLI_PREFERRED_CODONS
from .escherichia import E_COLI_CODON_PAIR_BIAS as E_COLI_CODON_PAIR_BIAS
from .escherichia import E_COLI_EXPRESSION_OPTIMIZATION_PARAMS as E_COLI_EXPRESSION_OPTIMIZATION_PARAMS
from .escherichia import ECOLI_CODON_USAGE as ECOLI_CODON_USAGE
from .escherichia import compute_codon_pair_bias as compute_codon_pair_bias
...
# Now that we have unified on CODON_ADAPTIVENESS_TABLES, SPECIES is kept
...
class _DeprecatedSpeciesDict(dict):  # type: ignore[misc]
    ...
    _warned_keys: set[str]  # Track keys we have already warned about
