...
ORGANISM_CONFIGS: dict[str, OrganismConfig] = {
    ...
    "Komagataella_phaffii": OrganismConfig(
        ...
        domain="eukaryote",
        constraint_profile="yeast",
        preferred_codons=_build_preferred_codons("pichia"),
        avoided_motifs=_YEAST_AVOIDED_MOTIFS,
        max_homopolymer_run=5,
        mrna_degradation_model="simple",
    ),
...
def get_organism_config(organism: str) -> OrganismConfig:
    ...
    # ── Canonical name resolution via the central registry ─────
    # Delegate to ``resolve_organism`` from :mod:`biocompiler.organisms`
    # (240+ alias entries) BEFORE consulting the local _ALIASES map below
    # (11 entries).  This unifies the two divergent name-resolution
    # systems, so callers passing any alias supported by the central
    # registry are routed to the correct OrganismConfig.  The lazy import
    # avoids a circular dependency: ``organisms/__init__.py`` does not
    # import ``organisms.config`` at module load time, but
    # ``biocompiler.type_system`` (and others) do.
    try:
        from biocompiler.organisms import resolve_organism as _resolve_canonical
        resolved = _resolve_canonical(organism, strict=False)
    except Exception:  # pragma: no cover — defensive
        resolved = organism

    if resolved != organism:
        # The central registry recognised the alias.  Try the resolved
        # canonical name directly, then via _ALIASES (which handles the
        # Escherichia_coli -> E_coli_K12 mapping that the registry does
        # not encode because ORGANISM_CONFIGS uses the strain key).
        if resolved in ORGANISM_CONFIGS:
            logger.info(
                "Resolved organism alias %r -> %r (via resolve_organism)",
                organism, resolved,
            )
            return ORGANISM_CONFIGS[resolved]

    # ── Legacy alias resolution (11 entries; strain-key mapping) ──
    ...
    # Try the resolved canonical name first (covers e.g. Escherichia_coli
    # resolved from "ecoli" -> needs _ALIASES to map to E_coli_K12).
    for key in (resolved, organism):
        canonical = _ALIASES.get(key)
        if canonical and canonical in ORGANISM_CONFIGS:
            logger.info(
                "Resolved organism alias %r -> %r", key, canonical,
            )
    ...
    # misclassified as eukaryotes even if they are not in the registry.
