...
def _check_mhcflurry_available() -> bool:
    ...
    # Try the deeper adapter check — but do not crash if the package
    # init has issues.
    try:
        from biocompiler.immunogenicity.mhcflurry_adapter import is_mhcflurry_available
        return is_mhcflurry_available()
    except Exception:
        # mhcflurry is importable but the adapter is not reachable;
...
def _lookup_binding_peptides(
    ...
    .. deprecated:: C20 remediation (prior fix)
        The ``precomputed/`` subpackage was deleted (see
        :meth:`biocompiler.immunogenicity.mhcflurry_adapter.MHCflurryClient._lookup_precomputed`
        for rationale).  This helper now always returns an empty list.
        Population-coverage computation itself is unaffected — it uses
        :data:`EXPANDED_POPULATION_COVERAGE` (allele frequency data), not
        the deleted binding database.  The function is retained so existing
        call sites in :func:`get_population_coverage` continue to work
