...
from ..engines.base import validate_protein_sequence
from biocompiler.shared.exceptions import ImmunogenicityError

# `MHCBindingResult` is canonically defined in `_models` (which carries the
# `confidence` field).  Previously this module re-declared a separate,
# `confidence`-less dataclass that shadowed the canonical one via the
# `__init__.py` wildcard-import order, so `MHCBindingResult(..., confidence=...)`
# raised `TypeError` (CRITICAL issue C10).  Instead of duplicating the field
# definitions, we now subclass the canonical dataclass and only override the
# NetMHCpan-specific defaults.  The `confidence` field (and every other field)
# is inherited, so the exported class is fully compatible with code that
# expects the `_models` version.
from ._models import MHCBindingResult as _MHCBindingResultBase  # noqa: E402
...
@dataclass
class MHCBindingResult(_MHCBindingResultBase):
    """NetMHCpan-specific :class:`MHCBindingResult`.

    Subclass of the canonical :class:`biocompiler.immunogenicity._models.MHCBindingResult`
    that overrides defaults for fields which the NetMHCpan client always
    populates explicitly (or, for non-binder fallbacks, leaves at sensible
    NetMHCpan defaults).  All fields — including `confidence` — are inherited
    from the base class, so this subclass is field-compatible with the
    canonical definition and `MHCBindingResult(..., confidence=0.95)` works.
    """
    ...
    def _poll_for_results(self, result_url: str) -> str:
        ...
        for poll_attempt in range(MAX_POLL_ATTEMPTS):
            try:
                ...
                # If we cannot determine the state, wait and retry
                time.sleep(POLL_INTERVAL)
