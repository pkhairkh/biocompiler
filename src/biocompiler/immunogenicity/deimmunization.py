"""
BioCompiler Deimmunization Engine v1.0.0
==========================================
...
.. warning::
    ...
    HONESTY NOTE — FABRICATED PSSM SCORES (TIGHTEN-4)
    -------------------------------------------------
    The deimmunization algorithm relies on the immunogenicity module's
    MHC binding prediction, whose default PSSMs are APPROXIMATE, not real
    binding data (see ``immunogenicity/core.py`` — the comments literally
    say "guessed/approximate scores, NOT scores derived from experimental
    binding data").

    Consequently:

    * The ``optimized_immunogenicity`` and ``original_immunogenicity``
      scores on :class:`DeimmunizationResult` are *reference numbers*
      derived from fabricated PSSMs, NOT measurements.
    * A successful deimmunization run (``success=True``) means the
      algorithm *reduced the fabricated score*; it does NOT mean the
      protein is genuinely less immunogenic in a real immune system.
    * For a verified deimmunization claim, install NetMHCpan or
      MHCflurry and call ``deimmunize(protein, use_real_data=True)``.
      Without a real predictor, ``use_real_data=True`` will return a
      result whose ``verdict`` is :attr:`~biocompiler.shared.types.Verdict.UNCERTAIN`.

    This module never returns PASS for a "low immunogenicity" claim on
    the basis of fabricated scores.
...
Key fixes (Task 26):
  ...
  Fix 2: Anchor position priority — MHC-I anchors (allele-specific from
         _ALLELE_ANCHOR_POSITIONS, default P2/P9), MHC-II anchors
         (allele-specific, default P1/P4/P6/P9) receive 1.5× binding_reduction weight.
         WARNING: Anchor positions now use allele-specific lookup when available;
         see _ALLELE_ANCHOR_POSITIONS and _DEFAULT_MHC_I_ANCHORS / _DEFAULT_MHC_II_ANCHORS.
...
from biocompiler.shared.constants import BLOSUM62, HYDROPATHY, STANDARD_AAS
...
#
# WARNING KNOWN LIMITATION: This assumes ALL MHC-I alleles anchor at P2/P9.
# Real anchor positions vary by allele:
#   - HLA-A*02:01 anchors at P2/P9 (correct for this allele)
#   - HLA-B*07:02 anchors at P1/P2 (Pro at P2, hydrophobic at P1)
#   - HLA-B*08:01 anchors at P3/P9 (Lys at P3, Leu/Ile at P9)
#   - HLA-A*01:01 anchors at P2/P9 but with different preferences (T/S at P2, Y at P9)
#   - HLA-B*44:02 anchors at P2/P9 but prefers Glu at P2
#
# For MHC-II, anchor positions also vary significantly by allele and
# the binding register (frame) within the peptide is not fixed.
#
# Allele-specific anchor position lookup table derived from IEDB data.
# Keys are MHC class ("mhc_i" / "mhc_ii"), values map allele names to
# the set of 0-indexed anchor positions within the peptide core.
# This replaces the previous hardcoded P2/P9 default with per-allele
# specificity where data is available.
_ALLELE_ANCHOR_POSITIONS: dict[str, dict[str, set[int]]] = {
    "mhc_i": {
        "HLA-A*01:01": {8},          # P9 is primary anchor (Y/F)
        "HLA-A*02:01": {1, 8},        # P2 and P9 (L/M/I/V and V/L/I/A)
        "HLA-A*03:01": {1, 8},        # P2 (K/T) and P9 (Y/F/L)
        "HLA-B*07:02": {1},           # P2 is primary anchor (P)
        "HLA-B*08:01": {1, 8},        # P2 and P9
        "HLA-B*27:05": {1, 8},        # P2 (R/K/Q) and P9
    },
    "mhc_ii": {
        "HLA-DRB1*01:01": {0, 3, 5, 8},  # P1/P4/P6/P9
        "HLA-DRB1*04:01": {0, 3, 5, 8},
    },
}

# Default anchor positions used as fallback when the allele is unknown
# or not present in the lookup table above.  These are the classic
# P2/P9 for MHC-I and P1/P4/P6/P9 for MHC-II.
_DEFAULT_MHC_I_ANCHORS: set[int] = {1, 8}

# MHC class II default anchor positions (0-indexed within the 9-mer core).
# Positions 1, 4, 6, 9 (1-indexed) are the key anchor residues
# that interact with the MHC-II binding groove pockets.
#
# WARNING KNOWN LIMITATION: MHC-II anchor positions depend on the binding
# register, which varies per allele and is not predictable by a
# simple fixed-position model. NetMHCIIpan determines the optimal
# register for each peptide-allele pair.
_DEFAULT_MHC_II_ANCHORS: set[int] = {0, 3, 5, 8}
...
class DeimmunizationResult(BaseEngineResult):
    ...
    # Deimmunization reduces a *fabricated* PSSM score by default.  The
    # `verdict` field signals whether the underlying binding prediction
    # was based on real data (NetMHCpan / MHCflurry) or on the guessed
    # PSSMs.  Default UNCERTAIN / fabricated_scores — only
    # `deimmunize(protein, use_real_data=True)` with a real predictor
    # installed can produce PASS / FAIL.
    verdict: Verdict = Verdict.UNCERTAIN
    reason: str = "fabricated_scores"
    message: str = (
        "Deimmunization scores use approximate PSSMs, NOT real binding "
        "data. Verdict is UNCERTAIN. Install NetMHCpan or MHCflurry for "
        "verified deimmunization."
    )
...
def _estimate_solubility_impact(
    ...
    if protein is not None and position is not None:
        try:
            from ..engines.camsol import compute_intrinsic_solubility as _camsol_score
            mutated_protein = protein[:position] + mutant + protein[position + 1:]
...
def _is_anchor_position(
    ...
    and allele.  When the allele is provided and present in the lookup
    table, allele-specific anchor positions are used.  Otherwise, the
    default anchor sets are used as fallback.
    ...
    Args:
        ...
        allele: MHC allele name (e.g. "HLA-A*02:01").  When provided,
            allele-specific anchors are looked up from
            ``_ALLELE_ANCHOR_POSITIONS``.
    ...
    if mhc_class == 1:
        if allele:
            anchors = _ALLELE_ANCHOR_POSITIONS.get("mhc_i", {}).get(allele)
            if anchors is not None:
                return pos_in_peptide in anchors
        return pos_in_peptide in _DEFAULT_MHC_I_ANCHORS
    else:
        if allele:
            anchors = _ALLELE_ANCHOR_POSITIONS.get("mhc_ii", {}).get(allele)
            if anchors is not None:
                return pos_in_peptide in anchors
        return pos_in_peptide in _DEFAULT_MHC_II_ANCHORS
...
def find_epitope_disrupting_mutations(
    ...
    # Determine the dominant allele for allele-specific anchor lookup.
    # Use the first allele matching the dominant MHC class as the representative.
    dominant_allele = ""
    if mhc_alleles:
        for a in mhc_alleles:
            if _detect_mhc_class(a) == dominant_mhc_class:
                dominant_allele = a
                break
    ...
    for pos in range(max(0, epitope_start), min(len(protein), epitope_end)):
        ...
        for pep_start in range(
            ...
            if not (pep_start <= pos < pep_start + _MHC_I_PEPTIDE_LENGTH):
                continue
            if _is_anchor_position(pos, pep_start, dominant_mhc_class, dominant_allele):
                is_anchor = True
                break
...
def _compute_immunogenicity_score(
    ...
    .. warning:: TIGHTEN-4
        ...
        By default the returned score is derived from the **guessed PSSMs**
        in :mod:`biocompiler.immunogenicity.core` (NOT real binding data).
        It is a *reference number*, not a measurement.  Pass
        ``use_real_data=True`` to require a real binding-data predictor
        (NetMHCpan / MHCflurry); if none is installed, the underlying
        :func:`~biocompiler.immunogenicity.core.compute_immunogenicity`
        will return an UNCERTAIN verdict and this function still returns
        the fabricated reference score (with a logged warning).

    Args:
        protein: Protein sequence.
        organism: Target organism.
        mhc_alleles: MHC alleles to test.
        use_real_data: If True, require a real binding-data predictor.
    ...
    try:
        ...
        # TIGHTEN-4: warn if the score is fabricated.
        if getattr(result, "verdict", None) == Verdict.UNCERTAIN and                 getattr(result, "reason", "") == "fabricated_scores":
            logger.warning(
                "_compute_immunogenicity_score: returned score %.4f is "
                "derived from FABRICATED PSSMs (not real binding data). "
                "Treat as a reference number only — not a measurement.",
                result.immunogenicity_score,
            )
...
def _count_t_cell_epitopes(
    ...
    .. warning:: TIGHTEN-4
        ...
        By default the count is derived from the **guessed PSSMs** in
        :mod:`biocompiler.immunogenicity.core` (NOT real binding data).
        Pass ``use_real_data=True`` to require a real binding-data
        predictor.
    ...
    try:
        ...
        # TIGHTEN-4: warn if the count is fabricated.
        if getattr(result, "verdict", None) == Verdict.UNCERTAIN and                 getattr(result, "reason", "") == "fabricated_scores":
            logger.warning(
                "_count_t_cell_epitopes: count %d derived from FABRICATED "
                "PSSMs (not real binding data).",
                len(result.t_cell_epitopes),
            )
        return len(result.t_cell_epitopes)
    except ImportError:
        logger.warning("immunogenicity.compute_immunogenicity unavailable, using local fallback")
    except Exception:
        logger.warning("immunogenicity.compute_immunogenicity failed, using local fallback")
...
def _compute_solubility_score(protein: str) -> float:
    ...
    """
    try:
        from ..engines.camsol import compute_intrinsic_solubility
...
def deimmunize(
    ...
    Args:
        ...
        max_iterations: Maximum number of iteration rounds (Task 27 Fix 2).
            ...
            Default is 10.
        use_real_data: If True, require a real binding-data predictor
            (NetMHCpan / MHCflurry).  If none is installed, the returned
            :class:`DeimmunizationResult` will carry
            ``verdict=UNCERTAIN`` and ``reason="no_real_predictor"``;
            the optimized scores are still computed from the guessed
            PSSMs and exposed for reference.  Default False (guessed
            PSSMs — verdict UNCERTAIN with ``reason="fabricated_scores"``).
    ...
    Returns:
        ...
        could be removed.  The result carries honesty fields
        (``verdict``, ``reason``, ``message``, ``data_source``)
        indicating whether the underlying binding prediction was based
        on real data or on the guessed PSSMs.
    ...
    with EngineTimer() as timer:
        ...
        original_immunogenicity = _compute_immunogenicity_score(
            protein, organism, allele_list, use_real_data=use_real_data
        )
        original_epitope_count = _count_t_cell_epitopes(
            protein, organism, allele_list, use_real_data=use_real_data
        )
        ...
        # The deimmunization scores are derived from the immunogenicity
        # module's PSSMs, which are hand-crafted approximations unless a
        # real binding-data predictor (NetMHCpan / MHCflurry) is
        # available.  We mirror the verdict of the underlying
        # compute_immunogenicity() call so callers can distinguish a
        # verified deimmunization from a fabricated-score one.
        from .core import _check_real_binding_data_available
        ...
        if use_real_data and real_data_available:
            ...
            reason = "real_data_deimmunization"
            message = (
                "Deimmunization verdict based on real binding-data "
                "predictor (NetMHCpan / MHCflurry)."
            )
            data_source = "real_binding_data"
            ...
            message = (
                ...
                "UNCERTAIN. The optimized score below is a fabricated-PSSM "
            ...
            message = (
                "Deimmunization scores use approximate PSSMs, NOT real "
                "binding data. Verdict is UNCERTAIN. Install NetMHCpan or "
                "MHCflurry for verified deimmunization."
            )
