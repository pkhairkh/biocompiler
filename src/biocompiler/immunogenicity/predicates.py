...
def _check_anchor_match(peptide: str, allele: str) -> bool:
    ...
    P9 (index 8). If the peptide does not match the preferred anchors,
    ...
    Returns:
        ...
        False if it clearly does not match.
    """
    anchors = _MHC_I_ANCHOR_POSITIONS.get(allele)
    if anchors is None:
        # Unknown allele → cannot verify anchors → assume match
        return True

    peptide = peptide.upper()
    if len(peptide) < 9:
        return True  # Cannot check anchors for short peptides
...
def _is_self_protein(organism: str, source_organism: str | None = None) -> bool:
    ...
    if source_organism is None:
        # If no source organism is specified, assume it is from the host
        return True
...
def evaluate_low_immunogenicity(
    ...
    **kwargs: Any,
    ...
    # ── H19 honesty fix: respect the underlying verdict ──────
    # compute_immunogenicity() returns verdict=UNCERTAIN with
    # reason="fabricated_scores" (or "no_real_predictor") whenever
    # the scores are PSSM-derived rather than real binding data.
    # Issuing PASS / LIKELY_PASS / LIKELY_FAIL / FAIL on the basis
    # of fabricated scores would be dishonest — it would imply the
    # PSSM-derived score is trustworthy enough to support a definite
    # verdict.  Propagate UNCERTAIN instead.  The score-based bands
    # below are only reachable when result.verdict is NOT UNCERTAIN
    # (i.e. a real binding-data predictor actually produced the
    # scores).
    if getattr(result, "verdict", None) == Verdict.UNCERTAIN:
        return PredicateResult(
            predicate="LowImmunogenicity",
            verdict=Verdict.UNCERTAIN,
            derivation=derivation,
            violation=None,
            knowledge_gap=(
                "Immunogenicity scores are PSSM-derived "
                "(fabricated), not real binding data; a "
                "definite PASS/FAIL verdict is not justified. "
                f"Underlying reason: {getattr(result, 'reason', 'unknown')}"
            ),
            details=(
                "compute_immunogenicity returned UNCERTAIN "
                f"(reason={getattr(result, 'reason', 'n/a')}); "
                "predicate verdict propagated as UNCERTAIN."
            ),
        )
...
def evaluate_no_strong_t_cell_epitope(
    ...
    **kwargs: Any,
    ...
    # Re-classify epitopes that do not match MHC anchor positions.
