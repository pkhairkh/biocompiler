...
De Morgan Negation:
  The five_valued_not function implements De Morgan negation on the
  5-valued lattice, swapping PASS ↔ FAIL and LIKELY_PASS ↔ LIKELY_FAIL
  while fixing UNCERTAIN ↔ UNCERTAIN.  This is an involution:
  five_valued_not(five_valued_not(v)) == v for all v.
...
def five_valued_not(v: FiveValuedVerdict) -> FiveValuedVerdict:
    """De Morgan negation: PASS↔FAIL, LIKELY_PASS↔LIKELY_FAIL, UNCERTAIN↔UNCERTAIN."""
    _NEGATION = {
        FiveValuedVerdict.PASS: FiveValuedVerdict.FAIL,
        FiveValuedVerdict.LIKELY_PASS: FiveValuedVerdict.LIKELY_FAIL,
        FiveValuedVerdict.UNCERTAIN: FiveValuedVerdict.UNCERTAIN,
        FiveValuedVerdict.LIKELY_FAIL: FiveValuedVerdict.LIKELY_PASS,
        FiveValuedVerdict.FAIL: FiveValuedVerdict.PASS,
    }
    return _NEGATION[v]
...
def refinement_is_sound(verdict: FiveValuedVerdict, actual_condition: bool) -> bool:
    ...
    For any 5-valued verdict v and actual condition c:
      ...
      - If to_three_valued(v) = UNCERTAIN, soundness is vacuously preserved
    ...
    if three_v == Verdict.PASS:
        ...
        # Only FiveValuedVerdict.PASS refines to Verdict.PASS, so this
        # branch exactly mirrors the soundness contract for PASS.
        return actual_condition is True
    elif three_v == Verdict.FAIL:
        # FAIL in 3-valued model is conservative: both LIKELY_FAIL and
        # FAIL refine to Verdict.FAIL.  For either source the soundness
        # requirement is identical — actual_condition must be False
        # (the prior `if verdict == LIKELY_FAIL:` arm returned the same
        # value, so the conditional was dead code).
        return actual_condition is False
    else:
...
def certificate_level_with_uncertainty(
    verdicts: list[FiveValuedVerdict],
) -> Literal["GOLD", "SILVER", "BRONZE"]:
    """Determine certificate level accounting for UNCERTAIN verdicts.

    Rules:
    - GOLD: all verdicts are definite (PASS/FAIL) or near-definite (LIKELY_PASS/LIKELY_FAIL)
    - SILVER: exactly 1 UNCERTAIN verdict
    - BRONZE: 2+ UNCERTAIN verdicts OR any FAIL

    This matches the implementation in certificate.py and provides
    a pure-logic interface for the Lean4 proof correspondence.

    Args:
        verdicts: List of FiveValuedVerdict values.

    Returns:
        Certificate level as "GOLD", "SILVER", or "BRONZE".
    """
    uncertain_count = sum(1 for v in verdicts if v == FiveValuedVerdict.UNCERTAIN)
    has_fail = any(v == FiveValuedVerdict.FAIL for v in verdicts)

    if has_fail or uncertain_count >= 2:
        return "BRONZE"
