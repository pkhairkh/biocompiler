...
.. warning:: CIRCULAR VALIDATION — NOT INDEPENDENT (C13 honesty correction)

   NOTE (prior fix / C13): Despite the name, this module is **NOT an
   independent validation** of the production
   ``biocompiler.sequence.maxentscan`` parameters.  The frequency tables
   below (``DONOR_MONO_FREQ``, ``DONOR_COND_FREQ``, ``ACCEPTOR_MONO_FREQ``,
   ``ACCEPTOR_COND_FREQ``) are **byte-for-byte copies** of the tables in
   ``biocompiler.sequence.maxentscan``.  Therefore
   ``cross_validate_against_production()`` is a *circular* consistency
   check: it verifies that the two duplicated copies still agree with
   each other (catching copy-paste drift), but it does **NOT** validate
   the parameters against Yeo & Burge (2004)'s published scores or against
   real ``me2x3.pl`` output.

   Furthermore (see the module-level WARNING in
   ``biocompiler.sequence.maxentscan``), those parameters are themselves
   **APPROXIMATIONS derived from mononucleotide marginals with first-order
   dependency adjustments**, not the real Yeo & Burge 2004 trained
   conditional frequencies.  ~44% of donor rows and ~57% of acceptor rows
   equal the marginals exactly — an impossibility in real trained data.

   Consequence: the "Score Ranges (bits)" advertised below (donors 8–12,
   acceptors 8–14) are aspirational targets calibrated to match the real
   MaxEntScan output range, NOT empirically validated against it.  Treat
   any "validated" label on these scores with appropriate scepticism.  To
   perform a real independent validation, download the original
   ``me2x3.pl`` / ``maxentscan`` distribution and compare per-site scores
   on a held-out set of verified splice sites.

Independent cross-validation implementation of maximum entropy splice site
scoring following Yeo & Burge (2004) "Maximum entropy modeling of short
sequence motifs with applications to RNA splicing" J. Comput. Biol.
11(2-3):377-394.

Role
----
The production ``biocompiler.sequence.maxentscan`` module **now also uses
the first-order Markov model** (as of the Markov migration).  This module's
role has therefore shifted from "reference implementation" to **independent
cross-validation**: it computes scores using its own copy of the Markov
parameters and scoring logic, entirely independently of the production
module.  Its purpose is to catch regressions — if someone changes the
production module's scoring code or parameters, this independent
implementation should still agree, and any disagreement signals a bug.

This module does **NOT** re-export or delegate to the production module.
Both implementations must be kept in sync manually; the
:func:`cross_validate_against_production` function automates the
comparison.

.. note:: Circularity caveat (C13): because the parameter tables in this
   module are manual copies of the production tables (see the WARNING at
   the top of this file), :func:`cross_validate_against_production` only
   proves the two copies agree. It does **not** prove the parameters match
   Yeo & Burge (2004) — see the honesty note in
   ``biocompiler.sequence.maxentscan``.
...
__all__ = [
    ...
    "is_strong_acceptor",
    # Cross-validation
    "cross_validate_against_production",
]
...
# ==============================================================================
# 6. Cross-Validation Against Production Module
# ==============================================================================


def cross_validate_against_production(tolerance: float = 0.1) -> List[str]:
    """Cross-validate this module's scores against the production module.

    Since both modules now use the first-order Markov model, their scores
    should agree within floating-point tolerance. Any disagreement indicates
    ...
    Returns:
        List of error messages. Empty list means all scores match.
    """
    from biocompiler.sequence.maxentscan import score_donor as prod_donor
    from biocompiler.sequence.maxentscan import score_acceptor as prod_acceptor

    errors = []

    # Test all reference donor sequences
    for seq, score in MAXENTSCAN_DONOR_SCORES.items():
        gt_pos = 3  # GT starts at index 3 in 9-mers
        computed = score_donor_maxentscan(seq)
        prod = prod_donor(seq, gt_pos)
        diff = abs(computed - prod)
        if diff > tolerance:
            errors.append(f"Donor cross-validation mismatch for '{seq}': "
                         f"benchmark={computed:.2f}, production={prod:.2f}, diff={diff:.4f}")

    # Test all reference acceptor sequences
    for seq, score in MAXENTSCAN_ACCEPTOR_SCORES.items():
        ag_pos = 18  # A of AG is at index 18 in 23-mers (18 upstream + AG + 3 downstream)
        computed = score_acceptor_maxentscan(seq)
        prod = prod_acceptor(seq, ag_pos)
        diff = abs(computed - prod)
        if diff > tolerance:
            errors.append(f"Acceptor cross-validation mismatch for '{seq}': "
                         f"benchmark={computed:.2f}, production={prod:.2f}, diff={diff:.4f}")
