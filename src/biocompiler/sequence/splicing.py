...
- MaxEntScan-based scoring for cryptic splice site detection using the
  first-order Markov (maximum-entropy) model (Yeo & Burge 2004)
...
.. note::
   ...
   The production scoring in ``maxentscan.py`` now uses the first-order
   Markov model, which captures dinucleotide dependencies and produces
   higher absolute scores than the previous PWM approximation.  The
   deprecated ``maxent_score()`` below still uses the simplified PWM;
   ``maxent_score_v2()`` correctly delegates to ``score_donor()`` which
   uses the Markov model.
"""
...
from biocompiler.sequence.maxentscan import (
    CRYPTIC_SPLICE_THRESHOLD as _CRYPTIC_SPLICE_THRESHOLD,
)
...
# The high threshold is imported from maxentscan.CRYPTIC_SPLICE_THRESHOLD
# to ensure a single source of truth across the project.
from biocompiler.sequence.maxentscan import CRYPTIC_SPLICE_THRESHOLD as _CRYPTIC_SPLICE_THRESHOLD
_DEFAULT_LOW_THRESH: float = 3.0     # Below this score → PASS
_DEFAULT_HIGH_THRESH: float = _CRYPTIC_SPLICE_THRESHOLD    # At or above this score → FAIL
# With the first-order Markov model: canonical donors score ~11, weak
# cryptic sites score < 8.  The threshold of 8.0 correctly catches strong
# canonical-like sites while letting weak cryptic sites through.
...
_DEFAULT_CRYPTIC_SCORE_THRESHOLD: float = _CRYPTIC_SPLICE_THRESHOLD  # Minimum score for cryptic sites to be included
...
def maxent_score_v2(context: str) -> float:
    ...
    It delegates to ``maxentscan.score_donor()`` which now uses the
    first-order Markov (maximum-entropy) model from Yeo & Burge 2004,
    producing scores that are positively correlated with true splice site
    strength.

    Unlike the deprecated ``maxent_score()``, higher scores here genuinely
    indicate stronger splice signals (canonical donors score ~11 with the
    Markov model, weak donors 0-5, non-donors < 0).
    ...
    """
    from biocompiler.sequence.maxentscan import score_donor as _score_donor  # same-package import
...
def score_splice_sites(
    ...
    from biocompiler.type_system import SpliceVerdict  # noqa: F811 — runtime import for enum values
...
from biocompiler.shared.types import SpliceIsoform
from biocompiler.shared.constants import MIN_INTRON_LENGTH
...
from biocompiler.organisms.tissue_data import get_tissue_weights as _get_gtex_weights
...
def _get_tissue_weights(cellular_context: str) -> dict[str, float]:
    ...
    try:
        ...
        from biocompiler.organisms.tissue_data import GTEX_TISSUE_WEIGHTS
